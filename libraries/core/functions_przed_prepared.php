...
//	createSymlink('portlets');
	...
	function QP1($query, $types = null, $values = array(), $log = false) {
		$result = Q($query, $log, true, $types, $values);
		if ($result) {
...
//			$pdf->setHeaderData('/../../../media/liver/logo_podziekowania.jpg', 0, '', '', array(255, 255, 255), array(255, 255, 255));
	...
	/*  */
	function prepareChildrenToSubmit($pobId, &$pobsToSkip = array()) {
...
WHERE pob_pob_id = ?
";
		$rows = QPR($query, "issssii", array($page->getId(), ''.$pobId, '%,'.$pobId, $pobId.',%', '%,'.$pobId.',%', $pobId, $pobId));
		$children = array();
		if ($rows && count($rows) > 0) {
			foreach ($rows as $row) {
				$doOdsId = $row['do_odswiezenia'];
				if (!in_array($doOdsId, $pobsToSkip)) {
					$children[$doOdsId] = prepareChildrenToSubmit($doOdsId, $pobsToSkip);
				}
			}
		}
		return $children;
	...
	function JSFS($pobId = null, $mode = "Submit", $submitChildren = true, $pobsToSkip = array(), $noPortletUpdate = false, $action = null, $args = array()) {
		...
		return "var childrenToSubmit = " . $childrenToSubmitJSString . "; jsfs({$pobId}, '{$mode}', childrenToSubmit, {$noPortletUpdateString}, '{$action}', new Array());";
	}
