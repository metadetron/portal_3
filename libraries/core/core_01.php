...
function localeSettings() {	
	$locale = virgoLanguage::getCurrentLanguageCode();
...
//
//
function showProfilerInfo($time_start) {
?>
...
function executePortletAction($pob, $portletAction, $databaseHandler) {
		...
		if (isset($portletAction) && trim($portletAction) != "") {
			...
			$className = $pdf->getAlias();
			if (!class_exists($className)) {
				require_once(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.$pdf->getNamespace().DIRECTORY_SEPARATOR.$pdf->getAlias().DIRECTORY_SEPARATOR.'controller.php');
			}
			$class = new $className();
...
// portlets must echo their results in their code, not here as with json...						
   ...
   "http://www.w3.org/TR/html4/strict.dtd">'; 
		...
		/* UWAGA! W tej chwili jest tak, ze zmiany w bazie dokonane w getContent i tak sie nie 
		  zakomituja! Tylkow akcji mozna zmieniac, getContent tylko do wyswietlania */
			$_SESSION['current_portlet_object_id'] = $pob->getId();
			echo $pob->getContent();
			unset($_SESSION['current_portlet_object_id']);
			if (isset($_REQUEST['profiler'])) {
...
onclick="
	window.open(
		...
		'portlets',
		'width=750,height=500,resizable=no,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no,copyhistory=no')
	"
JAVASCRIPT;
	} else {
		$configurable = "";
