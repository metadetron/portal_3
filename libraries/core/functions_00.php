...
//	createSymlink('portlets');
	...
	function QR($query, $log = false, $showError = true, $columnName = null) {
		$rows = array();
		$result = Q($query, $log);
		if ($result) {
...
//		}
	...
	function E($text, $P = array(), $stringContent = true, $default = null, $isParam = true) {
		$content = $isParam ? PP($text, $default) : $text;
	...
	function JSVA($name, $value, $action) {
		JSF();
		echo "f.$name.value = '$value'; f.portlet_action.value = '$action'; f.submit()";
	}
	
	function JSVAU($name, $value, $action, $url) {
		JSF();
		echo "f.$name.value = '$value'; f.portlet_action.value = '$action'; f.action='$url'; f.submit()";
	}
...
<?php
	...
	function getFormValue($name, $value = null) {
		...
		return isset($value) ? $value : R($name);
	}
...
<?php		
	...
	function PP($parameterName, $defaultValue = null) {
		$ret = virgoPortal::getParameterValue($parameterName, $defaultValue);
		if (!isset($ret)) {
