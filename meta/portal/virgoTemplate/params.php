...
<?php
			$options = array();
			$options["tmp_id"] = "id";
			$options["tmp_name"] = "Name";

			$options["tmp_code"] = "Code";
...
?>
		<tr>
			<td title="eg. tmp_abc_id in (select abc_id from...)) ">extra_ajax_filter</td>
			<td>
...
<?php
			$types = "s";
			$values = array('short_text_size_code');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_short_text_size_code = $res;
			$postValue = $_POST['pdf_short_text_size_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_short_text_size_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(short_text_size_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_short_text_size_code)) {
						$values = array($postValue, short_text_size_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(short_text_size_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_short_text_size_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_short_text_size_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(short_text_size_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_short_text_size_code)) {
						$values = array($postValue, short_text_size_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(short_text_size_code, $postValue);
...
?>
		<tr>
			<td>short_text_size_code</td>
			<td>
<input type="text" name="pdf_short_text_size_code" value="<?php echo $pdf_short_text_size_code ?>"/>
			</td>
			<td>
<input type="text" name="pob_short_text_size_code" value="<?php echo $pob_short_text_size_code ?>"/>
			</td>
			<td>
				<input type="submit" value="Store"/>
			</td>			
...
<?php
			$types = "s";
			$values = array('short_text_size_css');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_short_text_size_css = $res;
			$postValue = $_POST['pdf_short_text_size_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_short_text_size_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(short_text_size_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_short_text_size_css)) {
						$values = array($postValue, short_text_size_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(short_text_size_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_short_text_size_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_short_text_size_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(short_text_size_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_short_text_size_css)) {
						$values = array($postValue, short_text_size_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(short_text_size_css, $postValue);
...
?>
		<tr>
			<td>short_text_size_css</td>
			<td>
<input type="text" name="pdf_short_text_size_css" value="<?php echo $pdf_short_text_size_css ?>"/>
			</td>
			<td>
<input type="text" name="pob_short_text_size_css" value="<?php echo $pob_short_text_size_css ?>"/>
			</td>
			<td>
				<input type="submit" value="Store"/>
			</td>			
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_code = $res;
			$postValue = $_POST['pdf_show_table_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_code)) {
						$values = array($postValue, show_table_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_code" >
						<option value="" <?php echo !isset($pdf_show_table_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_code) && strrpos(",".$pdf_show_table_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_code" value="<?php echo $pdf_show_table_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_code" >
						<option value="" <?php echo !isset($pob_show_table_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_code) && strrpos(",".$pob_show_table_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_code" value="<?php echo $pob_show_table_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_css = $res;
			$postValue = $_POST['pdf_show_table_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_css)) {
						$values = array($postValue, show_table_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_css" >
						<option value="" <?php echo !isset($pdf_show_table_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_css) && strrpos(",".$pdf_show_table_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_css" value="<?php echo $pdf_show_table_css ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_css" >
						<option value="" <?php echo !isset($pob_show_table_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_css) && strrpos(",".$pob_show_table_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_css" value="<?php echo $pob_show_table_css ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_table_portals');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_portals = $res;
			$postValue = $_POST['pdf_show_table_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_portals)) {
						$values = array($postValue, show_table_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_portals, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_portals)) {
						$values = array($postValue, show_table_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_portals, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_portals" >
						<option value="" <?php echo !isset($pdf_show_table_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_portals) && strrpos(",".$pdf_show_table_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_portals" value="<?php echo $pdf_show_table_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_portals" >
						<option value="" <?php echo !isset($pob_show_table_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_portals) && strrpos(",".$pob_show_table_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_portals" value="<?php echo $pob_show_table_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_create_code = $res;
			$values = array("show_create_code_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_code_obligatory = $res;
			$postValue = $_POST['pdf_show_create_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_code)) {
						$values = array($postValue, show_create_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_code_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_code_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_code_obligatory);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_code_obligatory)) {
						$values = array($postValue, show_create_code_obligatory);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_code_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_create_code_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_code_obligatory = $res;
			$values = array("show_create_code_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_code_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_code" >
						<option value="" <?php echo !isset($pdf_show_create_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_code) && strrpos(",".$pdf_show_create_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_code" value="<?php echo $pdf_show_create_code ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_create_code_obligatory" value="1" <?php echo $pob_show_create_code_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_code" >
						<option value="" <?php echo !isset($pob_show_create_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_code) && strrpos(",".$pob_show_create_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_code" value="<?php echo $pob_show_create_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_create_css = $res;
			$values = array("show_create_css_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_css_obligatory = $res;
			$postValue = $_POST['pdf_show_create_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_css)) {
						$values = array($postValue, show_create_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_css_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_css_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_css_obligatory);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_css_obligatory)) {
						$values = array($postValue, show_create_css_obligatory);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_css_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_create_css_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_css_obligatory = $res;
			$values = array("show_create_css_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_css_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_css" >
						<option value="" <?php echo !isset($pdf_show_create_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_css) && strrpos(",".$pdf_show_create_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_css" value="<?php echo $pdf_show_create_css ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_create_css_obligatory" value="1" <?php echo $pob_show_create_css_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_css" >
						<option value="" <?php echo !isset($pob_show_create_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_css) && strrpos(",".$pob_show_create_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_css" value="<?php echo $pob_show_create_css ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_portals');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_create_portals = $res;
			$postValue = $_POST['pdf_show_create_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_portals)) {
						$values = array($postValue, show_create_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_portals, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_portals)) {
						$values = array($postValue, show_create_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_portals, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_portals" >
						<option value="" <?php echo !isset($pdf_show_create_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_portals) && strrpos(",".$pdf_show_create_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_portals" value="<?php echo $pdf_show_create_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_portals" >
						<option value="" <?php echo !isset($pob_show_create_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_portals) && strrpos(",".$pob_show_create_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_portals" value="<?php echo $pob_show_create_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_code = $res;
			$values = array("show_form_code_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_code_obligatory = $res;
			$postValue = $_POST['pdf_show_form_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_code)) {
						$values = array($postValue, show_form_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_code_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_code_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_code_obligatory);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_code_obligatory)) {
						$values = array($postValue, show_form_code_obligatory);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_code_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_form_code_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_code_obligatory = $res;
			$values = array("show_form_code_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_code_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_code" >
						<option value="" <?php echo !isset($pdf_show_form_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_code) && strrpos(",".$pdf_show_form_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_code" value="<?php echo $pdf_show_form_code ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_form_code_obligatory" value="1" <?php echo $pob_show_form_code_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_code" >
						<option value="" <?php echo !isset($pob_show_form_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_code) && strrpos(",".$pob_show_form_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_code" value="<?php echo $pob_show_form_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_css = $res;
			$values = array("show_form_css_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_css_obligatory = $res;
			$postValue = $_POST['pdf_show_form_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_css)) {
						$values = array($postValue, show_form_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_css_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_css_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_css_obligatory);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_css_obligatory)) {
						$values = array($postValue, show_form_css_obligatory);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_css_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_form_css_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_css_obligatory = $res;
			$values = array("show_form_css_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_css_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_css" >
						<option value="" <?php echo !isset($pdf_show_form_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_css) && strrpos(",".$pdf_show_form_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_css" value="<?php echo $pdf_show_form_css ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_form_css_obligatory" value="1" <?php echo $pob_show_form_css_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_css" >
						<option value="" <?php echo !isset($pob_show_form_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_css) && strrpos(",".$pob_show_form_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_css" value="<?php echo $pob_show_form_css ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_portals');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_portals = $res;
			$postValue = $_POST['pdf_show_form_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_portals)) {
						$values = array($postValue, show_form_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_portals, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_portals)) {
						$values = array($postValue, show_form_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_portals, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_portals" >
						<option value="" <?php echo !isset($pdf_show_form_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_portals) && strrpos(",".$pdf_show_form_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_portals" value="<?php echo $pdf_show_form_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_portals" >
						<option value="" <?php echo !isset($pob_show_form_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_portals) && strrpos(",".$pob_show_form_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_portals" value="<?php echo $pob_show_form_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_code = $res;
			$postValue = $_POST['pdf_show_view_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_code)) {
						$values = array($postValue, show_view_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_code" >
						<option value="" <?php echo !isset($pdf_show_view_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_code) && strrpos(",".$pdf_show_view_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_code" value="<?php echo $pdf_show_view_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_code" >
						<option value="" <?php echo !isset($pob_show_view_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_code) && strrpos(",".$pob_show_view_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_code" value="<?php echo $pob_show_view_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_css = $res;
			$postValue = $_POST['pdf_show_view_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_css)) {
						$values = array($postValue, show_view_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_css" >
						<option value="" <?php echo !isset($pdf_show_view_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_css) && strrpos(",".$pdf_show_view_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_css" value="<?php echo $pdf_show_view_css ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_css" >
						<option value="" <?php echo !isset($pob_show_view_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_css) && strrpos(",".$pob_show_view_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_css" value="<?php echo $pob_show_view_css ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_portals');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_portals = $res;
			$postValue = $_POST['pdf_show_view_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_portals)) {
						$values = array($postValue, show_view_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_portals, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_portals)) {
						$values = array($postValue, show_view_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_portals, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_portals" >
						<option value="" <?php echo !isset($pdf_show_view_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_portals) && strrpos(",".$pdf_show_view_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_portals" value="<?php echo $pdf_show_view_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_portals" >
						<option value="" <?php echo !isset($pob_show_view_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_portals) && strrpos(",".$pob_show_view_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_portals" value="<?php echo $pob_show_view_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_search_code = $res;
			$postValue = $_POST['pdf_show_search_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_code)) {
						$values = array($postValue, show_search_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_code" >
						<option value="" <?php echo !isset($pdf_show_search_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_code) && strrpos(",".$pdf_show_search_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_code" value="<?php echo $pdf_show_search_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_code" >
						<option value="" <?php echo !isset($pob_show_search_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_code) && strrpos(",".$pob_show_search_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_code" value="<?php echo $pob_show_search_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_search_css = $res;
			$postValue = $_POST['pdf_show_search_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_css)) {
						$values = array($postValue, show_search_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_css" >
						<option value="" <?php echo !isset($pdf_show_search_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_css) && strrpos(",".$pdf_show_search_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_css" value="<?php echo $pdf_show_search_css ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_css" >
						<option value="" <?php echo !isset($pob_show_search_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_css) && strrpos(",".$pob_show_search_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_css" value="<?php echo $pob_show_search_css ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_portals');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_search_portals = $res;
			$postValue = $_POST['pdf_show_search_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_portals)) {
						$values = array($postValue, show_search_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_portals, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_portals)) {
						$values = array($postValue, show_search_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_portals, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_portals" >
						<option value="" <?php echo !isset($pdf_show_search_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_portals) && strrpos(",".$pdf_show_search_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_portals" value="<?php echo $pdf_show_search_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_portals" >
						<option value="" <?php echo !isset($pob_show_search_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_portals) && strrpos(",".$pob_show_search_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_portals" value="<?php echo $pob_show_search_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_code = $res;
			$postValue = $_POST['pdf_show_pdf_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_code)) {
						$values = array($postValue, show_pdf_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_code" >
						<option value="" <?php echo !isset($pdf_show_pdf_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_code) && strrpos(",".$pdf_show_pdf_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_code" value="<?php echo $pdf_show_pdf_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_code" >
						<option value="" <?php echo !isset($pob_show_pdf_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_code) && strrpos(",".$pob_show_pdf_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_code" value="<?php echo $pob_show_pdf_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_css = $res;
			$postValue = $_POST['pdf_show_pdf_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_css)) {
						$values = array($postValue, show_pdf_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_css" >
						<option value="" <?php echo !isset($pdf_show_pdf_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_css) && strrpos(",".$pdf_show_pdf_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_css" value="<?php echo $pdf_show_pdf_css ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_css" >
						<option value="" <?php echo !isset($pob_show_pdf_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_css) && strrpos(",".$pob_show_pdf_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_css" value="<?php echo $pob_show_pdf_css ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_portals = $res;
			$postValue = $_POST['pdf_show_pdf_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_portals)) {
						$values = array($postValue, show_pdf_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_portals, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_portals" >
						<option value="" <?php echo !isset($pdf_show_pdf_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_portals) && strrpos(",".$pdf_show_pdf_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_portals" value="<?php echo $pdf_show_pdf_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_portals" >
						<option value="" <?php echo !isset($pob_show_pdf_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_portals) && strrpos(",".$pob_show_pdf_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_portals" value="<?php echo $pob_show_pdf_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_export_code = $res;
			$postValue = $_POST['pdf_show_export_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_code)) {
						$values = array($postValue, show_export_code);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_code, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_code'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_code != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_code);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_code" >
						<option value="" <?php echo !isset($pdf_show_export_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_code) && strrpos(",".$pdf_show_export_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_code" value="<?php echo $pdf_show_export_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_code" >
						<option value="" <?php echo !isset($pob_show_export_code) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_code) && strrpos(",".$pob_show_export_code.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_code" value="<?php echo $pob_show_export_code ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_export_css = $res;
			$postValue = $_POST['pdf_show_export_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_css)) {
						$values = array($postValue, show_export_css);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_css, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_css'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_css != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_css);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_css" >
						<option value="" <?php echo !isset($pdf_show_export_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_css) && strrpos(",".$pdf_show_export_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_css" value="<?php echo $pdf_show_export_css ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_css" >
						<option value="" <?php echo !isset($pob_show_export_css) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_css) && strrpos(",".$pob_show_export_css.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_css" value="<?php echo $pob_show_export_css ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_portals');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_export_portals = $res;
			$postValue = $_POST['pdf_show_export_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_portals)) {
						$values = array($postValue, show_export_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_portals, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_portals'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_portals != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_portals);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_portals)) {
						$values = array($postValue, show_export_portals);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_portals, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_portals" >
						<option value="" <?php echo !isset($pdf_show_export_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_portals) && strrpos(",".$pdf_show_export_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_portals" value="<?php echo $pdf_show_export_portals ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_portals" >
						<option value="" <?php echo !isset($pob_show_export_portals) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_portals) && strrpos(",".$pob_show_export_portals.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_portals" value="<?php echo $pob_show_export_portals ?>">
<?php
	}
?>				
				</div>
...
?>
		<tr>
			<td title="Custom SQL condition (eg. tmp_abc_id in (select abc_id from...)) You can use classes $currentUser and $currentPage (eg. tmp_usr_created_id = {$user->getId()})">custom_sql_condition</td>
			<td>
...
<?php 
	if (isset($_POST['HINT_template'])) {
		$newHint = $_POST['HINT_template'];
	...
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_template'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
			...
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_template', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_template</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_template"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_template_name'])) {
		$newHint = $_POST['HINT_template_name'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_template_name'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_template_name', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_template_name</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_template_name"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_template_code'])) {
		$newHint = $_POST['HINT_template_code'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_template_code'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_template_code', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_template_code</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_template_code"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_template_css'])) {
		$newHint = $_POST['HINT_template_css'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_template_css'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_template_css', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_template_css</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_template_css"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
...
<tr><td></td><td></td><td></td><td></td></tr>
<?php 
	if (isset($_POST['HINT_template_pages'])) {
		$newHint = $_POST['HINT_template_pages'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_template_pages'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_template_pages', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_template_pages</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_template_pages"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_template_portlet_parameters'])) {
		$newHint = $_POST['HINT_template_portlet_parameters'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_template_portlet_parameters'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_template_portlet_parameters', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_template_portlet_parameters</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_template_portlet_parameters"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_template_portals'])) {
		$newHint = $_POST['HINT_template_portals'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_template_portals'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_template_portals', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_template_portals</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_template_portals"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
