...
WHERE 
  ...
  AND pdf_namespace = ?
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoLogChannel', 'portal'));
			foreach ($result as $row) {
				$options[$row['pob_id']] = "{$row['pdf_name']} {$row['pob_custom_title']} [{$row['pge_title']}]";
			}
?>		
...
WHERE 
  ...
  AND pob_id != {$pobId}
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoChannelLevel', 'portal'));
			foreach ($result as $row) {
				$options[$row['pob_id']] = "{$row['pdf_name']} {$row['pob_custom_title']} [{$row['pge_title']}]";
			}
			$tmpOptions = $options;
...
<?php
			...
			$options["log_channel"] = "Log channel";
?>
<?php
			$types = "s";
...
?>
		<tr>
			<td title="eg. clv_abc_id in (select abc_id from...)) ">extra_ajax_filter</td>
			<td>
...
<?php
			$types = "s";
			$values = array('show_table_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_table_log_channel = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_table_log_channel = $res;
			$postValue = $_POST['pdf_show_table_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_log_channel)) {
						$values = array($postValue, show_table_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_log_channel)) {
						$values = array($postValue, show_table_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_log_channel" >
						<option value="" <?php echo !isset($pdf_show_table_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_log_channel) && strrpos(",".$pdf_show_table_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_log_channel" value="<?php echo $pdf_show_table_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_log_channel" >
						<option value="" <?php echo !isset($pob_show_table_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_log_channel) && strrpos(",".$pob_show_table_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_log_channel" value="<?php echo $pob_show_table_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_create_log_channel = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_log_channel = $res;
			$postValue = $_POST['pdf_show_create_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_log_channel)) {
						$values = array($postValue, show_create_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_log_channel)) {
						$values = array($postValue, show_create_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_log_channel" >
						<option value="" <?php echo !isset($pdf_show_create_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_log_channel) && strrpos(",".$pdf_show_create_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_log_channel" value="<?php echo $pdf_show_create_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_log_channel" >
						<option value="" <?php echo !isset($pob_show_create_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_log_channel) && strrpos(",".$pob_show_create_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_log_channel" value="<?php echo $pob_show_create_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('create_default_value_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_create_default_value_log_channel = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_create_default_value_log_channel = $res;
			$postValue = $_POST['pdf_create_default_value_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_create_default_value_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(create_default_value_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_create_default_value_log_channel)) {
						$values = array($postValue, create_default_value_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(create_default_value_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_create_default_value_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_create_default_value_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(create_default_value_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_create_default_value_log_channel)) {
						$values = array($postValue, create_default_value_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(create_default_value_log_channel, $postValue);
...
?>
		<tr>
			<td title="">
					create_default_value_log_channel
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_create_default_value_log_channel" >
						<option value="" <?php echo !isset($pdf_create_default_value_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_create_default_value_log_channel) && strrpos(",".$pdf_create_default_value_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_create_default_value_log_channel" value="<?php echo $pdf_create_default_value_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_create_default_value_log_channel" >
						<option value="" <?php echo !isset($pob_create_default_value_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_create_default_value_log_channel) && strrpos(",".$pob_create_default_value_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_create_default_value_log_channel" value="<?php echo $pob_create_default_value_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_log_channel = $res;
			$postValue = $_POST['pdf_show_form_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_log_channel)) {
						$values = array($postValue, show_form_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_log_channel)) {
						$values = array($postValue, show_form_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_log_channel" >
						<option value="" <?php echo !isset($pdf_show_form_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_log_channel) && strrpos(",".$pdf_show_form_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_log_channel" value="<?php echo $pdf_show_form_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_log_channel" >
						<option value="" <?php echo !isset($pob_show_form_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_log_channel) && strrpos(",".$pob_show_form_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_log_channel" value="<?php echo $pob_show_form_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_log_channel = $res;
			$postValue = $_POST['pdf_show_view_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_log_channel)) {
						$values = array($postValue, show_view_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_log_channel)) {
						$values = array($postValue, show_view_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_log_channel" >
						<option value="" <?php echo !isset($pdf_show_view_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_log_channel) && strrpos(",".$pdf_show_view_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_log_channel" value="<?php echo $pdf_show_view_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_log_channel" >
						<option value="" <?php echo !isset($pob_show_view_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_log_channel) && strrpos(",".$pob_show_view_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_log_channel" value="<?php echo $pob_show_view_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_search_log_channel = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_search_log_channel = $res;
			$postValue = $_POST['pdf_show_search_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_log_channel)) {
						$values = array($postValue, show_search_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_log_channel)) {
						$values = array($postValue, show_search_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_log_channel" >
						<option value="" <?php echo !isset($pdf_show_search_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_log_channel) && strrpos(",".$pdf_show_search_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_log_channel" value="<?php echo $pdf_show_search_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_log_channel" >
						<option value="" <?php echo !isset($pob_show_search_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_log_channel) && strrpos(",".$pob_show_search_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_log_channel" value="<?php echo $pob_show_search_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_pdf_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_log_channel = $res;
			$postValue = $_POST['pdf_show_pdf_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_log_channel)) {
						$values = array($postValue, show_pdf_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_pdf_log_channel)) {
						$values = array($postValue, show_pdf_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_log_channel" >
						<option value="" <?php echo !isset($pdf_show_pdf_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_log_channel) && strrpos(",".$pdf_show_pdf_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_log_channel" value="<?php echo $pdf_show_pdf_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_log_channel" >
						<option value="" <?php echo !isset($pob_show_pdf_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_log_channel) && strrpos(",".$pob_show_pdf_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_log_channel" value="<?php echo $pob_show_pdf_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_export_log_channel = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_export_log_channel = $res;
			$postValue = $_POST['pdf_show_export_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_log_channel)) {
						$values = array($postValue, show_export_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_log_channel)) {
						$values = array($postValue, show_export_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_log_channel" >
						<option value="" <?php echo !isset($pdf_show_export_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_log_channel) && strrpos(",".$pdf_show_export_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_log_channel" value="<?php echo $pdf_show_export_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_log_channel" >
						<option value="" <?php echo !isset($pob_show_export_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_log_channel) && strrpos(",".$pob_show_export_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_log_channel" value="<?php echo $pob_show_export_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('limit_to_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_limit_to_log_channel = $res;
			$postValue = $_POST['pdf_limit_to_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_limit_to_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(limit_to_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_limit_to_log_channel)) {
						$values = array($postValue, limit_to_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(limit_to_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_limit_to_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_limit_to_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(limit_to_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_limit_to_log_channel)) {
						$values = array($postValue, limit_to_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(limit_to_log_channel, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_limit_to_log_channel[]" multiple='multiple'>
						<option value="" <?php echo !isset($pdf_limit_to_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_limit_to_log_channel) && strrpos(",".$pdf_limit_to_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_limit_to_log_channel" value="<?php echo $pdf_limit_to_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_limit_to_log_channel[]" multiple='multiple'>
						<option value="" <?php echo !isset($pob_limit_to_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_limit_to_log_channel) && strrpos(",".$pob_limit_to_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_limit_to_log_channel" value="<?php echo $pob_limit_to_log_channel ?>">
<?php
	}
?>				
				</div>
...
?>
		<tr>
			<td title="Custom SQL condition (eg. clv_abc_id in (select abc_id from...)) You can use classes $currentUser and $currentPage (eg. clv_usr_created_id = {$user->getId()})">custom_sql_condition</td>
			<td>
...
<?php
			$types = "s";
			$values = array('import_default_value_log_channel');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_import_default_value_log_channel = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_import_default_value_log_channel = $res;
			$postValue = $_POST['pdf_import_default_value_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_import_default_value_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(import_default_value_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_import_default_value_log_channel)) {
						$values = array($postValue, import_default_value_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(import_default_value_log_channel, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_import_default_value_log_channel'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_import_default_value_log_channel != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(import_default_value_log_channel);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_import_default_value_log_channel)) {
						$values = array($postValue, import_default_value_log_channel);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(import_default_value_log_channel, $postValue);
...
?>
		<tr>
			<td title="">
					import_default_value_log_channel
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_import_default_value_log_channel" >
						<option value="" <?php echo !isset($pdf_import_default_value_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_import_default_value_log_channel) && strrpos(",".$pdf_import_default_value_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_import_default_value_log_channel" value="<?php echo $pdf_import_default_value_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_import_default_value_log_channel" >
						<option value="" <?php echo !isset($pob_import_default_value_log_channel) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_import_default_value_log_channel) && strrpos(",".$pob_import_default_value_log_channel.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_import_default_value_log_channel" value="<?php echo $pob_import_default_value_log_channel ?>">
<?php
	}
?>				
				</div>
...
<?php 
	if (isset($_POST['HINT_channel_level'])) {
		$newHint = $_POST['HINT_channel_level'];
	...
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_channel_level'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
			...
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_channel_level', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_channel_level</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_channel_level"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<tr><td></td><td></td><td></td><td></td></tr>
<?php 
	if (isset($_POST['HINT_channel_level_log_level'])) {
		$newHint = $_POST['HINT_channel_level_log_level'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_channel_level_log_level'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_channel_level_log_level', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_channel_level_log_level</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_channel_level_log_level"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_channel_level_log_channel'])) {
		$newHint = $_POST['HINT_channel_level_log_channel'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_channel_level_log_channel'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_channel_level_log_channel', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_channel_level_log_channel</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_channel_level_log_channel"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
