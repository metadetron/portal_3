...
WHERE 
  ...
  AND pob_id != {$pobId}
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoLanguage', 'portal'));
			foreach ($result as $row) {
...
<?php
			$options = array();
			$options["lng_id"] = "id";
			$options["lng_code"] = "Code";

			$options["lng_name"] = "Name";

			$options["lng_default"] = "Default";

			$options["lng_public"] = "Public";

			$options["lng_order"] = "Order";
...
?>
		<tr>
			<td title="eg. lng_abc_id in (select abc_id from...)) ">extra_ajax_filter</td>
			<td>
...
<?php
	...
	actionPermissions("AddPortletObject", $id, $name, $pobId);
...
<?php					
	...
	setPermissions("AddPortletObject", $id, $pobId);
...
?>				
					<select name="pdf_show_table_code" >
						<option value="" <?php echo !isset($pdf_show_table_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
?>				
					<select name="pob_show_table_code" >
						<option value="" <?php echo !isset($pob_show_table_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
<?php
			$types = "s";
			$values = array('show_table_public');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_public = $res;
			$postValue = $_POST['pdf_show_table_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_public)) {
						$values = array($postValue, show_table_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_public, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_public)) {
						$values = array($postValue, show_table_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_public, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_public" >
						<option value="" <?php echo !isset($pdf_show_table_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_public) && strrpos(",".$pdf_show_table_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_public" value="<?php echo $pdf_show_table_public ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_public" >
						<option value="" <?php echo !isset($pob_show_table_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_public) && strrpos(",".$pob_show_table_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_public" value="<?php echo $pob_show_table_public ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_table_translations');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_table_translations = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_table_translations = $res;
			$postValue = $_POST['pdf_show_table_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_translations)) {
						$values = array($postValue, show_table_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_translations, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_translations)) {
						$values = array($postValue, show_table_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_translations, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_translations" >
						<option value="" <?php echo !isset($pdf_show_table_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_translations) && strrpos(",".$pdf_show_table_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_translations" value="<?php echo $pdf_show_table_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_translations" >
						<option value="" <?php echo !isset($pob_show_table_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_translations) && strrpos(",".$pob_show_table_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_translations" value="<?php echo $pob_show_table_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_table_html_contents');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_table_html_contents = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_table_html_contents = $res;
			$postValue = $_POST['pdf_show_table_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_html_contents)) {
						$values = array($postValue, show_table_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_html_contents, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_table_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_html_contents)) {
						$values = array($postValue, show_table_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_html_contents, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_html_contents" >
						<option value="" <?php echo !isset($pdf_show_table_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_html_contents) && strrpos(",".$pdf_show_table_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_html_contents" value="<?php echo $pdf_show_table_html_contents ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_html_contents" >
						<option value="" <?php echo !isset($pob_show_table_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_html_contents) && strrpos(",".$pob_show_table_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_html_contents" value="<?php echo $pob_show_table_html_contents ?>">
<?php
	}
?>				
				</div>
...
?>				
					<select name="pdf_show_create_code" >
						<option value="" <?php echo !isset($pdf_show_create_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
?>				
					<select name="pob_show_create_code" >
						<option value="" <?php echo !isset($pob_show_create_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
<?php
			$types = "s";
			$values = array('show_create_public');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_create_public = $res;
			$values = array("show_create_public_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_public_obligatory = $res;
			$postValue = $_POST['pdf_show_create_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_public)) {
						$values = array($postValue, show_create_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_public, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_public)) {
						$values = array($postValue, show_create_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_public, $postValue);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_public_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_public_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_public_obligatory);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_public_obligatory)) {
						$values = array($postValue, show_create_public_obligatory);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_public_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_create_public_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_public_obligatory = $res;
			$values = array("show_create_public_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_public_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_public" >
						<option value="" <?php echo !isset($pdf_show_create_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_public) && strrpos(",".$pdf_show_create_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_public" value="<?php echo $pdf_show_create_public ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_create_public_obligatory" value="1" <?php echo $pob_show_create_public_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_public" >
						<option value="" <?php echo !isset($pob_show_create_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_public) && strrpos(",".$pob_show_create_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_public" value="<?php echo $pob_show_create_public ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_translations');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_create_translations = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_translations = $res;
			$postValue = $_POST['pdf_show_create_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_translations)) {
						$values = array($postValue, show_create_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_translations, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_translations)) {
						$values = array($postValue, show_create_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_translations, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_translations" >
						<option value="" <?php echo !isset($pdf_show_create_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_translations) && strrpos(",".$pdf_show_create_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_translations" value="<?php echo $pdf_show_create_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_translations" >
						<option value="" <?php echo !isset($pob_show_create_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_translations) && strrpos(",".$pob_show_create_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_translations" value="<?php echo $pob_show_create_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_html_contents');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_create_html_contents = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_html_contents = $res;
			$postValue = $_POST['pdf_show_create_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_html_contents)) {
						$values = array($postValue, show_create_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_html_contents, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_create_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_html_contents)) {
						$values = array($postValue, show_create_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_html_contents, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_html_contents" >
						<option value="" <?php echo !isset($pdf_show_create_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_html_contents) && strrpos(",".$pdf_show_create_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_html_contents" value="<?php echo $pdf_show_create_html_contents ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_html_contents" >
						<option value="" <?php echo !isset($pob_show_create_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_html_contents) && strrpos(",".$pob_show_create_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_html_contents" value="<?php echo $pob_show_create_html_contents ?>">
<?php
	}
?>				
				</div>
...
?>				
					<select name="pdf_show_form_code" >
						<option value="" <?php echo !isset($pdf_show_form_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
?>				
					<select name="pob_show_form_code" >
						<option value="" <?php echo !isset($pob_show_form_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_public = $res;
			$values = array("show_form_public_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_public_obligatory = $res;
			$postValue = $_POST['pdf_show_form_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_public)) {
						$values = array($postValue, show_form_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_public, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_public_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_public_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_public_obligatory);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_public_obligatory)) {
						$values = array($postValue, show_form_public_obligatory);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_public_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_form_public_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_public_obligatory = $res;
			$values = array("show_form_public_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_public_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_public" >
						<option value="" <?php echo !isset($pdf_show_form_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_public) && strrpos(",".$pdf_show_form_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_public" value="<?php echo $pdf_show_form_public ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_form_public_obligatory" value="1" <?php echo $pob_show_form_public_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_public" >
						<option value="" <?php echo !isset($pob_show_form_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_public) && strrpos(",".$pob_show_form_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_public" value="<?php echo $pob_show_form_public ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_translations');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_form_translations = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_translations = $res;
			$postValue = $_POST['pdf_show_form_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_translations)) {
						$values = array($postValue, show_form_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_translations, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_translations)) {
						$values = array($postValue, show_form_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_translations, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_translations" >
						<option value="" <?php echo !isset($pdf_show_form_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_translations) && strrpos(",".$pdf_show_form_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_translations" value="<?php echo $pdf_show_form_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_translations" >
						<option value="" <?php echo !isset($pob_show_form_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_translations) && strrpos(",".$pob_show_form_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_translations" value="<?php echo $pob_show_form_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_html_contents');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_form_html_contents = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_html_contents = $res;
			$postValue = $_POST['pdf_show_form_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_html_contents)) {
						$values = array($postValue, show_form_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_html_contents, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_form_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_html_contents)) {
						$values = array($postValue, show_form_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_html_contents, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_html_contents" >
						<option value="" <?php echo !isset($pdf_show_form_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_html_contents) && strrpos(",".$pdf_show_form_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_html_contents" value="<?php echo $pdf_show_form_html_contents ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_html_contents" >
						<option value="" <?php echo !isset($pob_show_form_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_html_contents) && strrpos(",".$pob_show_form_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_html_contents" value="<?php echo $pob_show_form_html_contents ?>">
<?php
	}
?>				
				</div>
...
?>				
					<select name="pdf_show_view_code" >
						<option value="" <?php echo !isset($pdf_show_view_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
?>				
					<select name="pob_show_view_code" >
						<option value="" <?php echo !isset($pob_show_view_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_public = $res;
			$postValue = $_POST['pdf_show_view_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_public)) {
						$values = array($postValue, show_view_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_public, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_public" >
						<option value="" <?php echo !isset($pdf_show_view_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_public) && strrpos(",".$pdf_show_view_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_public" value="<?php echo $pdf_show_view_public ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_public" >
						<option value="" <?php echo !isset($pob_show_view_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_public) && strrpos(",".$pob_show_view_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_public" value="<?php echo $pob_show_view_public ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_translations');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_view_translations = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_view_translations = $res;
			$postValue = $_POST['pdf_show_view_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_translations)) {
						$values = array($postValue, show_view_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_translations, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_translations)) {
						$values = array($postValue, show_view_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_translations, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_translations" >
						<option value="" <?php echo !isset($pdf_show_view_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_translations) && strrpos(",".$pdf_show_view_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_translations" value="<?php echo $pdf_show_view_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_translations" >
						<option value="" <?php echo !isset($pob_show_view_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_translations) && strrpos(",".$pob_show_view_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_translations" value="<?php echo $pob_show_view_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_html_contents');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_view_html_contents = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_view_html_contents = $res;
			$postValue = $_POST['pdf_show_view_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_html_contents)) {
						$values = array($postValue, show_view_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_html_contents, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_view_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_html_contents)) {
						$values = array($postValue, show_view_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_html_contents, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_html_contents" >
						<option value="" <?php echo !isset($pdf_show_view_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_html_contents) && strrpos(",".$pdf_show_view_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_html_contents" value="<?php echo $pdf_show_view_html_contents ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_html_contents" >
						<option value="" <?php echo !isset($pob_show_view_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_html_contents) && strrpos(",".$pob_show_view_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_html_contents" value="<?php echo $pob_show_view_html_contents ?>">
<?php
	}
?>				
				</div>
...
?>				
					<select name="pdf_show_search_code" >
						<option value="" <?php echo !isset($pdf_show_search_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
?>				
					<select name="pob_show_search_code" >
						<option value="" <?php echo !isset($pob_show_search_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
<?php
			$types = "s";
			$values = array('show_search_public');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_search_public = $res;
			$postValue = $_POST['pdf_show_search_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_public)) {
						$values = array($postValue, show_search_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_public, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_public)) {
						$values = array($postValue, show_search_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_public, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_public" >
						<option value="" <?php echo !isset($pdf_show_search_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_public) && strrpos(",".$pdf_show_search_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_public" value="<?php echo $pdf_show_search_public ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_public" >
						<option value="" <?php echo !isset($pob_show_search_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_public) && strrpos(",".$pob_show_search_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_public" value="<?php echo $pob_show_search_public ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_translations');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_search_translations = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_search_translations = $res;
			$postValue = $_POST['pdf_show_search_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_translations)) {
						$values = array($postValue, show_search_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_translations, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_translations)) {
						$values = array($postValue, show_search_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_translations, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_translations" >
						<option value="" <?php echo !isset($pdf_show_search_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_translations) && strrpos(",".$pdf_show_search_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_translations" value="<?php echo $pdf_show_search_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_translations" >
						<option value="" <?php echo !isset($pob_show_search_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_translations) && strrpos(",".$pob_show_search_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_translations" value="<?php echo $pob_show_search_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_html_contents');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_search_html_contents = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_search_html_contents = $res;
			$postValue = $_POST['pdf_show_search_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_html_contents)) {
						$values = array($postValue, show_search_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_html_contents, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_search_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_html_contents)) {
						$values = array($postValue, show_search_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_html_contents, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_html_contents" >
						<option value="" <?php echo !isset($pdf_show_search_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_html_contents) && strrpos(",".$pdf_show_search_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_html_contents" value="<?php echo $pdf_show_search_html_contents ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_html_contents" >
						<option value="" <?php echo !isset($pob_show_search_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_html_contents) && strrpos(",".$pob_show_search_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_html_contents" value="<?php echo $pob_show_search_html_contents ?>">
<?php
	}
?>				
				</div>
...
?>				
					<select name="pdf_show_pdf_code" >
						<option value="" <?php echo !isset($pdf_show_pdf_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
?>				
					<select name="pob_show_pdf_code" >
						<option value="" <?php echo !isset($pob_show_pdf_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_public = $res;
			$postValue = $_POST['pdf_show_pdf_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_public)) {
						$values = array($postValue, show_pdf_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_public, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_public" >
						<option value="" <?php echo !isset($pdf_show_pdf_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_public) && strrpos(",".$pdf_show_pdf_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_public" value="<?php echo $pdf_show_pdf_public ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_public" >
						<option value="" <?php echo !isset($pob_show_pdf_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_public) && strrpos(",".$pob_show_pdf_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_public" value="<?php echo $pob_show_pdf_public ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_pdf_translations');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_translations = $res;
			$postValue = $_POST['pdf_show_pdf_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_translations)) {
						$values = array($postValue, show_pdf_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_translations, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_pdf_translations)) {
						$values = array($postValue, show_pdf_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_translations, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_translations" >
						<option value="" <?php echo !isset($pdf_show_pdf_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_translations) && strrpos(",".$pdf_show_pdf_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_translations" value="<?php echo $pdf_show_pdf_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_translations" >
						<option value="" <?php echo !isset($pob_show_pdf_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_translations) && strrpos(",".$pob_show_pdf_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_translations" value="<?php echo $pob_show_pdf_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_pdf_html_contents');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_pdf_html_contents = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_pdf_html_contents = $res;
			$postValue = $_POST['pdf_show_pdf_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_html_contents)) {
						$values = array($postValue, show_pdf_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_html_contents, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_pdf_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_pdf_html_contents)) {
						$values = array($postValue, show_pdf_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_html_contents, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_html_contents" >
						<option value="" <?php echo !isset($pdf_show_pdf_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_html_contents) && strrpos(",".$pdf_show_pdf_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_html_contents" value="<?php echo $pdf_show_pdf_html_contents ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_html_contents" >
						<option value="" <?php echo !isset($pob_show_pdf_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_html_contents) && strrpos(",".$pob_show_pdf_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_html_contents" value="<?php echo $pob_show_pdf_html_contents ?>">
<?php
	}
?>				
				</div>
...
?>				
					<select name="pdf_show_export_code" >
						<option value="" <?php echo !isset($pdf_show_export_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
?>				
					<select name="pob_show_export_code" >
						<option value="" <?php echo !isset($pob_show_export_code) ? "selected " : "" ?>>!!!OBLIGATORY!!!</option>
<?php
			foreach ($options as $key => $val) {
?>				
...
<?php
			$types = "s";
			$values = array('show_export_public');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_export_public = $res;
			$postValue = $_POST['pdf_show_export_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_public)) {
						$values = array($postValue, show_export_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_public, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_public'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_public != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_public);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_public)) {
						$values = array($postValue, show_export_public);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_public, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_public" >
						<option value="" <?php echo !isset($pdf_show_export_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_public) && strrpos(",".$pdf_show_export_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_public" value="<?php echo $pdf_show_export_public ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_public" >
						<option value="" <?php echo !isset($pob_show_export_public) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_public) && strrpos(",".$pob_show_export_public.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_public" value="<?php echo $pob_show_export_public ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_translations');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_export_translations = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_export_translations = $res;
			$postValue = $_POST['pdf_show_export_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_translations)) {
						$values = array($postValue, show_export_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_translations, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_translations'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_translations != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_translations);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_translations)) {
						$values = array($postValue, show_export_translations);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_translations, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_translations" >
						<option value="" <?php echo !isset($pdf_show_export_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_translations) && strrpos(",".$pdf_show_export_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_translations" value="<?php echo $pdf_show_export_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_translations" >
						<option value="" <?php echo !isset($pob_show_export_translations) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_translations) && strrpos(",".$pob_show_export_translations.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_translations" value="<?php echo $pob_show_export_translations ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_html_contents');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_export_html_contents = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_export_html_contents = $res;
			$postValue = $_POST['pdf_show_export_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_html_contents)) {
						$values = array($postValue, show_export_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_html_contents, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_html_contents'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_html_contents != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array(show_export_html_contents);
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_html_contents)) {
						$values = array($postValue, show_export_html_contents);
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_html_contents, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_html_contents" >
						<option value="" <?php echo !isset($pdf_show_export_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_html_contents) && strrpos(",".$pdf_show_export_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_html_contents" value="<?php echo $pdf_show_export_html_contents ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_html_contents" >
						<option value="" <?php echo !isset($pob_show_export_html_contents) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_html_contents) && strrpos(",".$pob_show_export_html_contents.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_html_contents" value="<?php echo $pob_show_export_html_contents ?>">
<?php
	}
?>				
				</div>
...
?>
		<tr>
			<td title="Custom SQL condition (eg. lng_abc_id in (select abc_id from...)) You can use classes $currentUser and $currentPage (eg. lng_usr_created_id = {$user->getId()})">custom_sql_condition</td>
			<td>
...
<?php 
	if (isset($_POST['HINT_language'])) {
		$newHint = $_POST['HINT_language'];
	...
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
			...
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_language_code'])) {
		$newHint = $_POST['HINT_language_code'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language_code'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language_code', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language_code</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language_code"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_language_name'])) {
		$newHint = $_POST['HINT_language_name'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language_name'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language_name', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language_name</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language_name"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_language_default'])) {
		$newHint = $_POST['HINT_language_default'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language_default'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language_default', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language_default</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language_default"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_language_public'])) {
		$newHint = $_POST['HINT_language_public'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language_public'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language_public', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language_public</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language_public"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_language_order'])) {
		$newHint = $_POST['HINT_language_order'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language_order'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language_order', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language_order</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language_order"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
...
<tr><td></td><td></td><td></td><td></td></tr>
<?php 
	if (isset($_POST['HINT_language_translations'])) {
		$newHint = $_POST['HINT_language_translations'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language_translations'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language_translations', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language_translations</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language_translations"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_language_html_contents'])) {
		$newHint = $_POST['HINT_language_html_contents'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_language_html_contents'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_language_html_contents', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_language_html_contents</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_language_html_contents"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
