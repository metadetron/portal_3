...
<?php
			$options = array();
			$options["gtw_id"] = "id";
			$options["gtw_nazwa"] = "Nazwa";

			$options["gtw_opis"] = "Opis";
...
?>
		<tr>
			<td title="eg. gtw_abc_id in (select abc_id from...)) ">extra_ajax_filter</td>
			<td>
...
<?php
			$types = "s";
			$values = array('show_table_towary');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_towary = $res;
			$postValue = $_POST['pdf_show_table_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_towary)) {
						$values = array($postValue, 'show_table_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_towary, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_towary)) {
						$values = array($postValue, 'show_table_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_towary, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_towary" >
						<option value="" <?php echo !isset($pdf_show_table_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_towary) && strrpos(",".$pdf_show_table_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_towary" value="<?php echo $pdf_show_table_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_towary" >
						<option value="" <?php echo !isset($pob_show_table_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_towary) && strrpos(",".$pob_show_table_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_towary" value="<?php echo $pob_show_table_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_towary');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_create_towary = $res;
			$postValue = $_POST['pdf_show_create_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_towary)) {
						$values = array($postValue, 'show_create_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_towary, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_towary)) {
						$values = array($postValue, 'show_create_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_towary, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_towary" >
						<option value="" <?php echo !isset($pdf_show_create_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_towary) && strrpos(",".$pdf_show_create_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_towary" value="<?php echo $pdf_show_create_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_towary" >
						<option value="" <?php echo !isset($pob_show_create_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_towary) && strrpos(",".$pob_show_create_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_towary" value="<?php echo $pob_show_create_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_towary = $res;
			$postValue = $_POST['pdf_show_form_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_towary)) {
						$values = array($postValue, 'show_form_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_towary, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_towary" >
						<option value="" <?php echo !isset($pdf_show_form_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_towary) && strrpos(",".$pdf_show_form_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_towary" value="<?php echo $pdf_show_form_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_towary" >
						<option value="" <?php echo !isset($pob_show_form_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_towary) && strrpos(",".$pob_show_form_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_towary" value="<?php echo $pob_show_form_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_towary = $res;
			$postValue = $_POST['pdf_show_view_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_towary)) {
						$values = array($postValue, 'show_view_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_towary, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_towary" >
						<option value="" <?php echo !isset($pdf_show_view_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_towary) && strrpos(",".$pdf_show_view_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_towary" value="<?php echo $pdf_show_view_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_towary" >
						<option value="" <?php echo !isset($pob_show_view_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_towary) && strrpos(",".$pob_show_view_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_towary" value="<?php echo $pob_show_view_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_towary');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_search_towary = $res;
			$postValue = $_POST['pdf_show_search_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_towary)) {
						$values = array($postValue, 'show_search_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_towary, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_towary)) {
						$values = array($postValue, 'show_search_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_towary, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_towary" >
						<option value="" <?php echo !isset($pdf_show_search_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_towary) && strrpos(",".$pdf_show_search_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_towary" value="<?php echo $pdf_show_search_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_towary" >
						<option value="" <?php echo !isset($pob_show_search_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_towary) && strrpos(",".$pob_show_search_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_towary" value="<?php echo $pob_show_search_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_towary = $res;
			$postValue = $_POST['pdf_show_pdf_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_towary)) {
						$values = array($postValue, 'show_pdf_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_towary, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_towary" >
						<option value="" <?php echo !isset($pdf_show_pdf_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_towary) && strrpos(",".$pdf_show_pdf_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_towary" value="<?php echo $pdf_show_pdf_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_towary" >
						<option value="" <?php echo !isset($pob_show_pdf_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_towary) && strrpos(",".$pob_show_pdf_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_towary" value="<?php echo $pob_show_pdf_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_towary');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_export_towary = $res;
			$postValue = $_POST['pdf_show_export_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_towary)) {
						$values = array($postValue, 'show_export_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_towary, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_towary'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_towary != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_towary');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_towary)) {
						$values = array($postValue, 'show_export_towary');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_towary, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_towary" >
						<option value="" <?php echo !isset($pdf_show_export_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_towary) && strrpos(",".$pdf_show_export_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_towary" value="<?php echo $pdf_show_export_towary ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_towary" >
						<option value="" <?php echo !isset($pob_show_export_towary) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_towary) && strrpos(",".$pob_show_export_towary.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_towary" value="<?php echo $pob_show_export_towary ?>">
<?php
	}
?>				
				</div>
...
?>
		<tr>
			<td title="Custom SQL condition (eg. gtw_abc_id in (select abc_id from...)) You can use classes $currentUser and $currentPage (eg. gtw_usr_created_id = {$user->getId()})">custom_sql_condition</td>
			<td>
...
<?php 
	if (isset($_POST['HINT_grupa_towaru'])) {
		$newHint = $_POST['HINT_grupa_towaru'];
	...
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_grupa_towaru'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
			...
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_grupa_towaru', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_grupa_towaru</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_grupa_towaru"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_grupa_towaru_nazwa'])) {
		$newHint = $_POST['HINT_grupa_towaru_nazwa'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_grupa_towaru_nazwa'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_grupa_towaru_nazwa', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_grupa_towaru_nazwa</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_grupa_towaru_nazwa"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_grupa_towaru_opis'])) {
		$newHint = $_POST['HINT_grupa_towaru_opis'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_grupa_towaru_opis'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_grupa_towaru_opis', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_grupa_towaru_opis</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_grupa_towaru_opis"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
...
<tr><td></td><td></td><td></td><td></td></tr>
<?php 
	if (isset($_POST['HINT_grupa_towaru_towary'])) {
		$newHint = $_POST['HINT_grupa_towaru_towary'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_grupa_towaru_towary'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_grupa_towaru_towary', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_grupa_towaru_towary</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_grupa_towaru_towary"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
