...
<?php
	...
	$codeActionStrings[] = "?";
	$types .= "s";
	$codeActions[] = "StoreNewWydanie";
	$codeActionStrings[] = "?";
	$types .= "s";
	$codeActions[] = "StoreNewZamowienie";
	$codeActionStrings[] = "?";
	$types .= "s";
...
WHERE 
  ...
  AND pdf_namespace = ?
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoZamowienie', 'sealock'));
			foreach ($result as $row) {
...
WHERE 
  ...
  AND pdf_namespace = ?
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoWydanie', 'sealock'));
			foreach ($result as $row) {
				$options[$row['pob_id']] = "{$row['pdf_name']} {$row['pob_custom_title']} [{$row['pge_title']}]";
			}
?>		
...
WHERE 
  ...
  AND pdf_namespace = ?
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoOdbiorca', 'sealock'));
			foreach ($result as $row) {
				$options[$row['pob_id']] = "{$row['pdf_name']} {$row['pob_custom_title']} [{$row['pge_title']}]";
			}
			$query = <<<SQL
...
WHERE 
  ...
  AND pdf_namespace = ?
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoProdukt', 'sealock'));
			foreach ($result as $row) {
				$options[$row['pob_id']] = "{$row['pdf_name']} {$row['pob_custom_title']} [{$row['pge_title']}]";
			}
?>		
...
WHERE 
  ...
  AND pob_id != {$pobId}
SQL;
			$result = $databaseHandler->queryPrepared($query, false, "ss", array('virgoPozycjaZamowienia', 'sealock'));
			foreach ($result as $row) {
				$options[$row['pob_id']] = "{$row['pdf_name']} {$row['pob_custom_title']} [{$row['pge_title']}]";
			}
			$tmpOptions = $options;
...
<?php
			$options = array();
			$options["pzm_id"] = "id";
			$options["pzm_ilosc"] = "Ilość";

			$options["pzm_cena_netto"] = "Cena netto";

			$options["pzm_cena_brutto"] = "Cena brutto";

			$options["zamowienie"] = "Zamówienie";
			$options["towar"] = "Towar";
			$options["wydanie"] = "Wydanie";
?>
<?php
			$types = "s";
...
?>
		<tr>
			<td title="eg. pzm_abc_id in (select abc_id from...)) ">extra_ajax_filter</td>
			<td>
...
<?php
	...
	actionPermissions("StoreNewWydanie", $id, $name, $pobId);
	actionPermissions("StoreNewZamowienie", $id, $name, $pobId);
...
<?php					
	...
	setPermissions("StoreNewWydanie", $id, $pobId);
	setPermissions("StoreNewZamowienie", $id, $pobId);
...
<?php
			$types = "s";
			$values = array('show_table_cena_netto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_cena_netto = $res;
			$postValue = $_POST['pdf_show_table_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_cena_netto)) {
						$values = array($postValue, 'show_table_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_cena_netto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_cena_netto)) {
						$values = array($postValue, 'show_table_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_cena_netto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_cena_netto" >
						<option value="" <?php echo !isset($pdf_show_table_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_cena_netto) && strrpos(",".$pdf_show_table_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_cena_netto" value="<?php echo $pdf_show_table_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_cena_netto" >
						<option value="" <?php echo !isset($pob_show_table_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_cena_netto) && strrpos(",".$pob_show_table_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_cena_netto" value="<?php echo $pob_show_table_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_table_cena_brutto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_table_cena_brutto = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_table_cena_brutto = $res;
			$postValue = $_POST['pdf_show_table_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_cena_brutto)) {
						$values = array($postValue, 'show_table_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_cena_brutto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_cena_brutto)) {
						$values = array($postValue, 'show_table_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_cena_brutto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_cena_brutto" >
						<option value="" <?php echo !isset($pdf_show_table_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_cena_brutto) && strrpos(",".$pdf_show_table_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_cena_brutto" value="<?php echo $pdf_show_table_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_cena_brutto" >
						<option value="" <?php echo !isset($pob_show_table_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_cena_brutto) && strrpos(",".$pob_show_table_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_cena_brutto" value="<?php echo $pob_show_table_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_table_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_zamowienie = $res;
			$postValue = $_POST['pdf_show_table_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_zamowienie)) {
						$values = array($postValue, 'show_table_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_zamowienie)) {
						$values = array($postValue, 'show_table_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_zamowienie" >
						<option value="" <?php echo !isset($pdf_show_table_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_zamowienie) && strrpos(",".$pdf_show_table_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_zamowienie" value="<?php echo $pdf_show_table_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_zamowienie" >
						<option value="" <?php echo !isset($pob_show_table_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_zamowienie) && strrpos(",".$pob_show_table_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_zamowienie" value="<?php echo $pob_show_table_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_table_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_table_wydanie = $res;
			$postValue = $_POST['pdf_show_table_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_table_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_table_wydanie)) {
						$values = array($postValue, 'show_table_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_table_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_table_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_table_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_table_wydanie)) {
						$values = array($postValue, 'show_table_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_table_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_table_wydanie" >
						<option value="" <?php echo !isset($pdf_show_table_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_table_wydanie) && strrpos(",".$pdf_show_table_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_table_wydanie" value="<?php echo $pdf_show_table_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_table_wydanie" >
						<option value="" <?php echo !isset($pob_show_table_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_table_wydanie) && strrpos(",".$pob_show_table_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_table_wydanie" value="<?php echo $pob_show_table_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_cena_netto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_create_cena_netto = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_netto = $res;
			$values = array("show_create_cena_netto_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_netto_obligatory = $res;
			$postValue = $_POST['pdf_show_create_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_cena_netto)) {
						$values = array($postValue, 'show_create_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_cena_netto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_cena_netto)) {
						$values = array($postValue, 'show_create_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_cena_netto, $postValue);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_cena_netto_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_cena_netto_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_cena_netto_obligatory');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_cena_netto_obligatory)) {
						$values = array($postValue, 'show_create_cena_netto_obligatory');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_cena_netto_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_create_cena_netto_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_netto_obligatory = $res;
			$values = array("show_create_cena_netto_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_netto_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
					show_create_cena_netto
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_cena_netto" >
						<option value="" <?php echo !isset($pdf_show_create_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_cena_netto) && strrpos(",".$pdf_show_create_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_cena_netto" value="<?php echo $pdf_show_create_cena_netto ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_create_cena_netto_obligatory" value="1" <?php echo $pob_show_create_cena_netto_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_cena_netto" >
						<option value="" <?php echo !isset($pob_show_create_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_cena_netto) && strrpos(",".$pob_show_create_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_cena_netto" value="<?php echo $pob_show_create_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_cena_brutto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_create_cena_brutto = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_brutto = $res;
			$values = array("show_create_cena_brutto_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_brutto_obligatory = $res;
			$postValue = $_POST['pdf_show_create_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_cena_brutto)) {
						$values = array($postValue, 'show_create_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_cena_brutto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_cena_brutto)) {
						$values = array($postValue, 'show_create_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_cena_brutto, $postValue);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_cena_brutto_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_cena_brutto_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_cena_brutto_obligatory');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_cena_brutto_obligatory)) {
						$values = array($postValue, 'show_create_cena_brutto_obligatory');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_cena_brutto_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_create_cena_brutto_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_brutto_obligatory = $res;
			$values = array("show_create_cena_brutto_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_cena_brutto_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
					show_create_cena_brutto
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_cena_brutto" >
						<option value="" <?php echo !isset($pdf_show_create_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_cena_brutto) && strrpos(",".$pdf_show_create_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_cena_brutto" value="<?php echo $pdf_show_create_cena_brutto ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_create_cena_brutto_obligatory" value="1" <?php echo $pob_show_create_cena_brutto_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_cena_brutto" >
						<option value="" <?php echo !isset($pob_show_create_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_cena_brutto) && strrpos(",".$pob_show_create_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_cena_brutto" value="<?php echo $pob_show_create_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_create_zamowienie = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_create_zamowienie = $res;
			$postValue = $_POST['pdf_show_create_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_zamowienie)) {
						$values = array($postValue, 'show_create_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_zamowienie)) {
						$values = array($postValue, 'show_create_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_zamowienie" >
						<option value="" <?php echo !isset($pdf_show_create_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_zamowienie) && strrpos(",".$pdf_show_create_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_zamowienie" value="<?php echo $pdf_show_create_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_zamowienie" >
						<option value="" <?php echo !isset($pob_show_create_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_zamowienie) && strrpos(",".$pob_show_create_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_zamowienie" value="<?php echo $pob_show_create_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('create_default_value_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_create_default_value_zamowienie = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_create_default_value_zamowienie = $res;
			$postValue = $_POST['pdf_create_default_value_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_create_default_value_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('create_default_value_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_create_default_value_zamowienie)) {
						$values = array($postValue, 'create_default_value_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(create_default_value_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_create_default_value_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_create_default_value_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('create_default_value_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_create_default_value_zamowienie)) {
						$values = array($postValue, 'create_default_value_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(create_default_value_zamowienie, $postValue);
...
?>
		<tr>
			<td title="">
					create_default_value_zamowienie
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_create_default_value_zamowienie" >
						<option value="" <?php echo !isset($pdf_create_default_value_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_create_default_value_zamowienie) && strrpos(",".$pdf_create_default_value_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_create_default_value_zamowienie" value="<?php echo $pdf_create_default_value_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_create_default_value_zamowienie" >
						<option value="" <?php echo !isset($pob_create_default_value_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_create_default_value_zamowienie) && strrpos(",".$pob_create_default_value_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_create_default_value_zamowienie" value="<?php echo $pob_create_default_value_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_create_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_create_wydanie = $res;
			$postValue = $_POST['pdf_show_create_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_create_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_create_wydanie)) {
						$values = array($postValue, 'show_create_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_create_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_create_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_create_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_create_wydanie)) {
						$values = array($postValue, 'show_create_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_create_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_create_wydanie" >
						<option value="" <?php echo !isset($pdf_show_create_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_create_wydanie) && strrpos(",".$pdf_show_create_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_create_wydanie" value="<?php echo $pdf_show_create_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_create_wydanie" >
						<option value="" <?php echo !isset($pob_show_create_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_create_wydanie) && strrpos(",".$pob_show_create_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_create_wydanie" value="<?php echo $pob_show_create_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('create_default_value_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_create_default_value_wydanie = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_create_default_value_wydanie = $res;
			$postValue = $_POST['pdf_create_default_value_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_create_default_value_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('create_default_value_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_create_default_value_wydanie)) {
						$values = array($postValue, 'create_default_value_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(create_default_value_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_create_default_value_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_create_default_value_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('create_default_value_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_create_default_value_wydanie)) {
						$values = array($postValue, 'create_default_value_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(create_default_value_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_create_default_value_wydanie" >
						<option value="" <?php echo !isset($pdf_create_default_value_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_create_default_value_wydanie) && strrpos(",".$pdf_create_default_value_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_create_default_value_wydanie" value="<?php echo $pdf_create_default_value_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_create_default_value_wydanie" >
						<option value="" <?php echo !isset($pob_create_default_value_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_create_default_value_wydanie) && strrpos(",".$pob_create_default_value_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_create_default_value_wydanie" value="<?php echo $pob_create_default_value_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_cena_netto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_cena_netto = $res;
			$values = array("show_form_cena_netto_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_cena_netto_obligatory = $res;
			$postValue = $_POST['pdf_show_form_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_cena_netto)) {
						$values = array($postValue, 'show_form_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_cena_netto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_cena_netto)) {
						$values = array($postValue, 'show_form_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_cena_netto, $postValue);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_cena_netto_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_cena_netto_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_cena_netto_obligatory');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_cena_netto_obligatory)) {
						$values = array($postValue, 'show_form_cena_netto_obligatory');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_cena_netto_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_form_cena_netto_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_cena_netto_obligatory = $res;
			$values = array("show_form_cena_netto_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_cena_netto_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
					show_form_cena_netto
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_cena_netto" >
						<option value="" <?php echo !isset($pdf_show_form_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_cena_netto) && strrpos(",".$pdf_show_form_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_cena_netto" value="<?php echo $pdf_show_form_cena_netto ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_form_cena_netto_obligatory" value="1" <?php echo $pob_show_form_cena_netto_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_cena_netto" >
						<option value="" <?php echo !isset($pob_show_form_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_cena_netto) && strrpos(",".$pob_show_form_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_cena_netto" value="<?php echo $pob_show_form_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_cena_brutto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_cena_brutto = $res;
			$values = array("show_form_cena_brutto_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_cena_brutto_obligatory = $res;
			$postValue = $_POST['pdf_show_form_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_cena_brutto)) {
						$values = array($postValue, 'show_form_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_cena_brutto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_cena_brutto)) {
						$values = array($postValue, 'show_form_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_cena_brutto, $postValue);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_cena_brutto_obligatory'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_cena_brutto_obligatory != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_cena_brutto_obligatory');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_cena_brutto_obligatory)) {
						$values = array($postValue, 'show_form_cena_brutto_obligatory');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_cena_brutto_obligatory, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pob_id) VALUES (?, ?, {$pobId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pdf_show_form_cena_brutto_obligatory = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_cena_brutto_obligatory = $res;
			$values = array("show_form_cena_brutto_obligatory_obligatory");
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_form_cena_brutto_obligatory_obligatory = $res;
			}
?>
		<tr>
			<td title="">
					show_form_cena_brutto
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_cena_brutto" >
						<option value="" <?php echo !isset($pdf_show_form_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_cena_brutto) && strrpos(",".$pdf_show_form_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_cena_brutto" value="<?php echo $pdf_show_form_cena_brutto ?>">
<?php
	}
?>				
				</div>
			...
			<td>
				<div style="max-width: 200px; overflow: auto;">
					Obl:<input type="checkbox" name="pob_show_form_cena_brutto_obligatory" value="1" <?php echo $pob_show_form_cena_brutto_obligatory == "1" ? " checked='checked' " : "" ?>/>
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_cena_brutto" >
						<option value="" <?php echo !isset($pob_show_form_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_cena_brutto) && strrpos(",".$pob_show_form_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_cena_brutto" value="<?php echo $pob_show_form_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_zamowienie = $res;
			$postValue = $_POST['pdf_show_form_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_zamowienie)) {
						$values = array($postValue, 'show_form_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_zamowienie)) {
						$values = array($postValue, 'show_form_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_zamowienie" >
						<option value="" <?php echo !isset($pdf_show_form_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_zamowienie) && strrpos(",".$pdf_show_form_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_zamowienie" value="<?php echo $pdf_show_form_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_zamowienie" >
						<option value="" <?php echo !isset($pob_show_form_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_zamowienie) && strrpos(",".$pob_show_form_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_zamowienie" value="<?php echo $pob_show_form_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_form_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_form_wydanie = $res;
			$postValue = $_POST['pdf_show_form_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_form_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_form_wydanie)) {
						$values = array($postValue, 'show_form_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_form_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_form_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_form_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_form_wydanie)) {
						$values = array($postValue, 'show_form_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_form_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_form_wydanie" >
						<option value="" <?php echo !isset($pdf_show_form_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_form_wydanie) && strrpos(",".$pdf_show_form_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_form_wydanie" value="<?php echo $pdf_show_form_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_form_wydanie" >
						<option value="" <?php echo !isset($pob_show_form_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_form_wydanie) && strrpos(",".$pob_show_form_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_form_wydanie" value="<?php echo $pob_show_form_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_cena_netto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_cena_netto = $res;
			$postValue = $_POST['pdf_show_view_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_cena_netto)) {
						$values = array($postValue, 'show_view_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_cena_netto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_cena_netto)) {
						$values = array($postValue, 'show_view_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_cena_netto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_cena_netto" >
						<option value="" <?php echo !isset($pdf_show_view_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_cena_netto) && strrpos(",".$pdf_show_view_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_cena_netto" value="<?php echo $pdf_show_view_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_cena_netto" >
						<option value="" <?php echo !isset($pob_show_view_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_cena_netto) && strrpos(",".$pob_show_view_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_cena_netto" value="<?php echo $pob_show_view_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_cena_brutto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_cena_brutto = $res;
			$postValue = $_POST['pdf_show_view_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_cena_brutto)) {
						$values = array($postValue, 'show_view_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_cena_brutto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_cena_brutto)) {
						$values = array($postValue, 'show_view_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_cena_brutto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_cena_brutto" >
						<option value="" <?php echo !isset($pdf_show_view_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_cena_brutto) && strrpos(",".$pdf_show_view_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_cena_brutto" value="<?php echo $pdf_show_view_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_cena_brutto" >
						<option value="" <?php echo !isset($pob_show_view_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_cena_brutto) && strrpos(",".$pob_show_view_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_cena_brutto" value="<?php echo $pob_show_view_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_zamowienie = $res;
			$postValue = $_POST['pdf_show_view_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_zamowienie)) {
						$values = array($postValue, 'show_view_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_zamowienie)) {
						$values = array($postValue, 'show_view_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_zamowienie" >
						<option value="" <?php echo !isset($pdf_show_view_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_zamowienie) && strrpos(",".$pdf_show_view_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_zamowienie" value="<?php echo $pdf_show_view_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_zamowienie" >
						<option value="" <?php echo !isset($pob_show_view_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_zamowienie) && strrpos(",".$pob_show_view_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_zamowienie" value="<?php echo $pob_show_view_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_view_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_view_wydanie = $res;
			$postValue = $_POST['pdf_show_view_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_view_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_view_wydanie)) {
						$values = array($postValue, 'show_view_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_view_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_view_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_view_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_view_wydanie)) {
						$values = array($postValue, 'show_view_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_view_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_view_wydanie" >
						<option value="" <?php echo !isset($pdf_show_view_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_view_wydanie) && strrpos(",".$pdf_show_view_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_view_wydanie" value="<?php echo $pdf_show_view_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_view_wydanie" >
						<option value="" <?php echo !isset($pob_show_view_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_view_wydanie) && strrpos(",".$pob_show_view_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_view_wydanie" value="<?php echo $pob_show_view_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_cena_netto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_search_cena_netto = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_search_cena_netto = $res;
			$postValue = $_POST['pdf_show_search_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_cena_netto)) {
						$values = array($postValue, 'show_search_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_cena_netto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_cena_netto)) {
						$values = array($postValue, 'show_search_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_cena_netto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_cena_netto" >
						<option value="" <?php echo !isset($pdf_show_search_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_cena_netto) && strrpos(",".$pdf_show_search_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_cena_netto" value="<?php echo $pdf_show_search_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_cena_netto" >
						<option value="" <?php echo !isset($pob_show_search_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_cena_netto) && strrpos(",".$pob_show_search_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_cena_netto" value="<?php echo $pob_show_search_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_cena_brutto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_search_cena_brutto = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_search_cena_brutto = $res;
			$postValue = $_POST['pdf_show_search_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_cena_brutto)) {
						$values = array($postValue, 'show_search_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_cena_brutto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_cena_brutto)) {
						$values = array($postValue, 'show_search_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_cena_brutto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_cena_brutto" >
						<option value="" <?php echo !isset($pdf_show_search_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_cena_brutto) && strrpos(",".$pdf_show_search_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_cena_brutto" value="<?php echo $pdf_show_search_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_cena_brutto" >
						<option value="" <?php echo !isset($pob_show_search_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_cena_brutto) && strrpos(",".$pob_show_search_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_cena_brutto" value="<?php echo $pob_show_search_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_search_zamowienie = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_search_zamowienie = $res;
			$postValue = $_POST['pdf_show_search_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_zamowienie)) {
						$values = array($postValue, 'show_search_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_zamowienie)) {
						$values = array($postValue, 'show_search_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_zamowienie" >
						<option value="" <?php echo !isset($pdf_show_search_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_zamowienie) && strrpos(",".$pdf_show_search_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_zamowienie" value="<?php echo $pdf_show_search_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_zamowienie" >
						<option value="" <?php echo !isset($pob_show_search_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_zamowienie) && strrpos(",".$pob_show_search_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_zamowienie" value="<?php echo $pob_show_search_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_search_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_search_wydanie = $res;
			$postValue = $_POST['pdf_show_search_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_search_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_search_wydanie)) {
						$values = array($postValue, 'show_search_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_search_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_search_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_search_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_search_wydanie)) {
						$values = array($postValue, 'show_search_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_search_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_search_wydanie" >
						<option value="" <?php echo !isset($pdf_show_search_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_search_wydanie) && strrpos(",".$pdf_show_search_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_search_wydanie" value="<?php echo $pdf_show_search_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_search_wydanie" >
						<option value="" <?php echo !isset($pob_show_search_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_search_wydanie) && strrpos(",".$pob_show_search_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_search_wydanie" value="<?php echo $pob_show_search_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_pdf_cena_netto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_cena_netto = $res;
			$postValue = $_POST['pdf_show_pdf_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_cena_netto)) {
						$values = array($postValue, 'show_pdf_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_cena_netto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_pdf_cena_netto)) {
						$values = array($postValue, 'show_pdf_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_cena_netto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_cena_netto" >
						<option value="" <?php echo !isset($pdf_show_pdf_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_cena_netto) && strrpos(",".$pdf_show_pdf_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_cena_netto" value="<?php echo $pdf_show_pdf_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_cena_netto" >
						<option value="" <?php echo !isset($pob_show_pdf_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_cena_netto) && strrpos(",".$pob_show_pdf_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_cena_netto" value="<?php echo $pob_show_pdf_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_pdf_cena_brutto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_cena_brutto = $res;
			$postValue = $_POST['pdf_show_pdf_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_cena_brutto)) {
						$values = array($postValue, 'show_pdf_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_cena_brutto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_pdf_cena_brutto)) {
						$values = array($postValue, 'show_pdf_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_cena_brutto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_cena_brutto" >
						<option value="" <?php echo !isset($pdf_show_pdf_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_cena_brutto) && strrpos(",".$pdf_show_pdf_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_cena_brutto" value="<?php echo $pdf_show_pdf_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_cena_brutto" >
						<option value="" <?php echo !isset($pob_show_pdf_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_cena_brutto) && strrpos(",".$pob_show_pdf_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_cena_brutto" value="<?php echo $pob_show_pdf_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_pdf_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_zamowienie = $res;
			$postValue = $_POST['pdf_show_pdf_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_zamowienie)) {
						$values = array($postValue, 'show_pdf_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_pdf_zamowienie)) {
						$values = array($postValue, 'show_pdf_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_zamowienie" >
						<option value="" <?php echo !isset($pdf_show_pdf_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_zamowienie) && strrpos(",".$pdf_show_pdf_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_zamowienie" value="<?php echo $pdf_show_pdf_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_zamowienie" >
						<option value="" <?php echo !isset($pob_show_pdf_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_zamowienie) && strrpos(",".$pob_show_pdf_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_zamowienie" value="<?php echo $pob_show_pdf_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_pdf_wydanie = $res;
			$postValue = $_POST['pdf_show_pdf_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_pdf_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_pdf_wydanie)) {
						$values = array($postValue, 'show_pdf_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_pdf_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_pdf_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_pdf_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_pdf_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_pdf_wydanie" >
						<option value="" <?php echo !isset($pdf_show_pdf_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_pdf_wydanie) && strrpos(",".$pdf_show_pdf_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_pdf_wydanie" value="<?php echo $pdf_show_pdf_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_pdf_wydanie" >
						<option value="" <?php echo !isset($pob_show_pdf_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_pdf_wydanie) && strrpos(",".$pob_show_pdf_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_pdf_wydanie" value="<?php echo $pob_show_pdf_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_cena_netto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_export_cena_netto = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_export_cena_netto = $res;
			$postValue = $_POST['pdf_show_export_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_cena_netto)) {
						$values = array($postValue, 'show_export_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_cena_netto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_cena_netto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_cena_netto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_cena_netto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_cena_netto)) {
						$values = array($postValue, 'show_export_cena_netto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_cena_netto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_cena_netto" >
						<option value="" <?php echo !isset($pdf_show_export_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_cena_netto) && strrpos(",".$pdf_show_export_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_cena_netto" value="<?php echo $pdf_show_export_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_cena_netto" >
						<option value="" <?php echo !isset($pob_show_export_cena_netto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_cena_netto) && strrpos(",".$pob_show_export_cena_netto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_cena_netto" value="<?php echo $pob_show_export_cena_netto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_cena_brutto');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_export_cena_brutto = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_export_cena_brutto = $res;
			$postValue = $_POST['pdf_show_export_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_cena_brutto)) {
						$values = array($postValue, 'show_export_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_cena_brutto, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_cena_brutto'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_cena_brutto != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_cena_brutto');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_cena_brutto)) {
						$values = array($postValue, 'show_export_cena_brutto');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_cena_brutto, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_cena_brutto" >
						<option value="" <?php echo !isset($pdf_show_export_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_cena_brutto) && strrpos(",".$pdf_show_export_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_cena_brutto" value="<?php echo $pdf_show_export_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_cena_brutto" >
						<option value="" <?php echo !isset($pob_show_export_cena_brutto) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_cena_brutto) && strrpos(",".$pob_show_export_cena_brutto.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_cena_brutto" value="<?php echo $pob_show_export_cena_brutto ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_show_export_zamowienie = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_show_export_zamowienie = $res;
			$postValue = $_POST['pdf_show_export_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_zamowienie)) {
						$values = array($postValue, 'show_export_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_zamowienie)) {
						$values = array($postValue, 'show_export_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_zamowienie" >
						<option value="" <?php echo !isset($pdf_show_export_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_zamowienie) && strrpos(",".$pdf_show_export_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_zamowienie" value="<?php echo $pdf_show_export_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_zamowienie" >
						<option value="" <?php echo !isset($pob_show_export_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_zamowienie) && strrpos(",".$pob_show_export_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_zamowienie" value="<?php echo $pob_show_export_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('show_export_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_show_export_wydanie = $res;
			$postValue = $_POST['pdf_show_export_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_show_export_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_show_export_wydanie)) {
						$values = array($postValue, 'show_export_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_show_export_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_show_export_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('show_export_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_show_export_wydanie)) {
						$values = array($postValue, 'show_export_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(show_export_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_show_export_wydanie" >
						<option value="" <?php echo !isset($pdf_show_export_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_show_export_wydanie) && strrpos(",".$pdf_show_export_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_show_export_wydanie" value="<?php echo $pdf_show_export_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_show_export_wydanie" >
						<option value="" <?php echo !isset($pob_show_export_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_show_export_wydanie) && strrpos(",".$pob_show_export_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_show_export_wydanie" value="<?php echo $pob_show_export_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('limit_to_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_limit_to_zamowienie = $res;
			$postValue = $_POST['pdf_limit_to_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_limit_to_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('limit_to_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_limit_to_zamowienie)) {
						$values = array($postValue, 'limit_to_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(limit_to_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_limit_to_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_limit_to_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('limit_to_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_limit_to_zamowienie)) {
						$values = array($postValue, 'limit_to_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(limit_to_zamowienie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_limit_to_zamowienie[]" multiple='multiple'>
						<option value="" <?php echo !isset($pdf_limit_to_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_limit_to_zamowienie) && strrpos(",".$pdf_limit_to_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_limit_to_zamowienie" value="<?php echo $pdf_limit_to_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_limit_to_zamowienie[]" multiple='multiple'>
						<option value="" <?php echo !isset($pob_limit_to_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_limit_to_zamowienie) && strrpos(",".$pob_limit_to_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_limit_to_zamowienie" value="<?php echo $pob_limit_to_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$pob_limit_to_wydanie = $res;
			$postValue = $_POST['pdf_limit_to_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_limit_to_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('limit_to_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_limit_to_wydanie)) {
						$values = array($postValue, 'limit_to_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(limit_to_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_limit_to_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_limit_to_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('limit_to_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_limit_to_wydanie[]" multiple='multiple'>
						<option value="" <?php echo !isset($pdf_limit_to_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_limit_to_wydanie) && strrpos(",".$pdf_limit_to_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_limit_to_wydanie" value="<?php echo $pdf_limit_to_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_limit_to_wydanie[]" multiple='multiple'>
						<option value="" <?php echo !isset($pob_limit_to_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_limit_to_wydanie) && strrpos(",".$pob_limit_to_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_limit_to_wydanie" value="<?php echo $pob_limit_to_wydanie ?>">
<?php
	}
?>				
				</div>
...
?>
		<tr>
			<td title="Custom SQL condition (eg. pzm_abc_id in (select abc_id from...)) You can use classes $currentUser and $currentPage (eg. pzm_usr_created_id = {$user->getId()})">custom_sql_condition</td>
			<td>
...
<?php
			$types = "s";
			$values = array('import_default_value_zamowienie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_import_default_value_zamowienie = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_import_default_value_zamowienie = $res;
			$postValue = $_POST['pdf_import_default_value_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_import_default_value_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('import_default_value_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_import_default_value_zamowienie)) {
						$values = array($postValue, 'import_default_value_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(import_default_value_zamowienie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_import_default_value_zamowienie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_import_default_value_zamowienie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('import_default_value_zamowienie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_import_default_value_zamowienie)) {
						$values = array($postValue, 'import_default_value_zamowienie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(import_default_value_zamowienie, $postValue);
...
?>
		<tr>
			<td title="">
					import_default_value_zamowienie
			</td>
			<td>
				<div style="max-width: 200px; overflow: auto;">
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_import_default_value_zamowienie" >
						<option value="" <?php echo !isset($pdf_import_default_value_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_import_default_value_zamowienie) && strrpos(",".$pdf_import_default_value_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_import_default_value_zamowienie" value="<?php echo $pdf_import_default_value_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_import_default_value_zamowienie" >
						<option value="" <?php echo !isset($pob_import_default_value_zamowienie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_import_default_value_zamowienie) && strrpos(",".$pob_import_default_value_zamowienie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_import_default_value_zamowienie" value="<?php echo $pob_import_default_value_zamowienie ?>">
<?php
	}
?>				
				</div>
...
<?php
			$types = "s";
			$values = array('import_default_value_wydanie');
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pdf_import_default_value_wydanie = $res;
			$query = "SELECT ppr_value FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
	$result = $databaseHandler->queryPrepared($query, false, $types, $values);
	if ($result !== false) {
		$res = null;
		...
		$res = null;
			...
			$pob_import_default_value_wydanie = $res;
			$postValue = $_POST['pdf_import_default_value_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pdf_import_default_value_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('import_default_value_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pdf_import_default_value_wydanie)) {
						$values = array($postValue, 'import_default_value_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pdf_id = {$pdfId} AND ppr_name = ? ";
					} else {
						$values = array(import_default_value_wydanie, $postValue);
						$query = "INSERT INTO prt_portlet_parameters (ppr_name, ppr_value, ppr_pdf_id) VALUES (?, ?, {$pdfId})";
					}
				}
				$databaseHandler->queryPrepared($query, false, $types, $values);
	...
	if ($result !== false) {
		...
		$res = null;
			...
			$postValue = $_POST['pob_import_default_value_wydanie'];
			if (isset($postValue) && is_array($postValue)) {
				$postValue = implode(',', $postValue);
			}
			if (isset($_POST['pobId']) && $pob_import_default_value_wydanie != $postValue) {
				if (!isset($postValue) || $postValue == "") {
					$types = "s";
					$values = array('import_default_value_wydanie');
					$query = "DELETE FROM prt_portlet_parameters WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					...
					$types = "ss";
					if (isset($pob_import_default_value_wydanie)) {
						$values = array($postValue, 'import_default_value_wydanie');
						$query = "UPDATE prt_portlet_parameters SET ppr_value = ? WHERE ppr_pob_id = {$pobId} AND ppr_name = ? ";
					} else {
						$values = array(import_default_value_wydanie, $postValue);
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pdf_import_default_value_wydanie" >
						<option value="" <?php echo !isset($pdf_import_default_value_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pdf_import_default_value_wydanie) && strrpos(",".$pdf_import_default_value_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pdf_import_default_value_wydanie" value="<?php echo $pdf_import_default_value_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php
	if (count($options) <= 100) {
?>				
					<select name="pob_import_default_value_wydanie" >
						<option value="" <?php echo !isset($pob_import_default_value_wydanie) ? "selected " : "" ?>></option>
<?php
			foreach ($options as $key => $val) {
?>				
						<option value="<?php echo $key ?>" <?php echo isset($pob_import_default_value_wydanie) && strrpos(",".$pob_import_default_value_wydanie.",", ",".$key.",") !== false ? "selected " : "" ?>><?php echo $val ?></option>
<?php
			}
?>				
					</select>
<?php
		...
		/* podawanie z palca. na razie bez multi */
?>
	Za duzo rekordów, podaj ID: <input type="text" name="pob_import_default_value_wydanie" value="<?php echo $pob_import_default_value_wydanie ?>">
<?php
	}
?>				
				</div>
...
<?php 
	if (isset($_POST['HINT_pozycja_zamowienia'])) {
		$newHint = $_POST['HINT_pozycja_zamowienia'];
	...
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_pozycja_zamowienia'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
			...
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_pozycja_zamowienia', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_pozycja_zamowienia</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_pozycja_zamowienia"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_pozycja_zamowienia_ilosc'])) {
		$newHint = $_POST['HINT_pozycja_zamowienia_ilosc'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_pozycja_zamowienia_ilosc'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_pozycja_zamowienia_ilosc', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_pozycja_zamowienia_ilosc</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_pozycja_zamowienia_ilosc"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_pozycja_zamowienia_cena_netto'])) {
		$newHint = $_POST['HINT_pozycja_zamowienia_cena_netto'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_pozycja_zamowienia_cena_netto'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_pozycja_zamowienia_cena_netto', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_pozycja_zamowienia_cena_netto</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_pozycja_zamowienia_cena_netto"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_pozycja_zamowienia_cena_brutto'])) {
		$newHint = $_POST['HINT_pozycja_zamowienia_cena_brutto'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_pozycja_zamowienia_cena_brutto'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_pozycja_zamowienia_cena_brutto', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_pozycja_zamowienia_cena_brutto</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_pozycja_zamowienia_cena_brutto"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<tr><td></td><td></td><td></td><td></td></tr>
<?php 
	if (isset($_POST['HINT_pozycja_zamowienia_zamowienie'])) {
		$newHint = $_POST['HINT_pozycja_zamowienia_zamowienie'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_pozycja_zamowienia_zamowienie'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_pozycja_zamowienia_zamowienie', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_pozycja_zamowienia_zamowienie</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_pozycja_zamowienia_zamowienie"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_pozycja_zamowienia_towar'])) {
		$newHint = $_POST['HINT_pozycja_zamowienia_towar'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_pozycja_zamowienia_towar'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_pozycja_zamowienia_towar', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_pozycja_zamowienia_towar</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_pozycja_zamowienia_towar"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
<?php 
	if (isset($_POST['HINT_pozycja_zamowienia_wydanie'])) {
		$newHint = $_POST['HINT_pozycja_zamowienia_wydanie'];
		$newHint = trim($newHint);
		if ($newHint == "") {
			$newHint = null;
		}
	...
	$hintText = null;
	$query = " SELECT trn_id, trn_text FROM prt_translations WHERE trn_token = ? AND trn_lng_id = " . $lngId;
	$result = $databaseHandler->queryPrepared($query, false, "s", array('HINT_pozycja_zamowienia_wydanie'));
	foreach ($result as $res) {
		$hintId = $row[trn_id];
		$hintText = $row[trn_text];
	}
			if (isset($hintId)) {
				if ($newHint != $hintText) {
					...
					$query = " INSERT INTO prt_translations (trn_lng_id, trn_token, trn_text) VALUES (" . $lngId . ", 'HINT_pozycja_zamowienia_wydanie', '" . $newHint . "')";
			}
		} else {
	$hintId = null;
	$hintText = null;
...
?>
	<tr>
		<td>HINT_pozycja_zamowienia_wydanie</td>
		<td colspan="2"><textarea cols="60" rows="4" name="HINT_pozycja_zamowienia_wydanie"><?php echo $text ?></textarea></td>
		<td>
			<input type="submit" value="Store"/>
		</td>			
	</tr>
