...
//	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoLanguage'.DIRECTORY_SEPARATOR.'controller.php')) require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoLanguage'.DIRECTORY_SEPARATOR.'controller.php');
	$componentParams = null; //&JComponentHelper::getParams('com_prt_translation');
...
?>
<link rel="stylesheet" href="<?php echo $live_site ?>/components/com_prt_translation/portal.css" type="text/css" /> 
<?php
	}
?>
<?php
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoTranslation'.DIRECTORY_SEPARATOR.'prt_trn.css')) {
?>
<link rel="stylesheet" href="<?php echo $_SESSION['portal_url'] ?>/portlets/portal/virgoTranslation/prt_trn.css" type="text/css" /> 
<?php
	}
?>
<style>
...
<div class="virgo_container_portal virgo_container_entity_translation" style="border: none;">
...
//		$classNames["unique_visit"] = "virgoUniqueVisit";
		...
		$ancestors = array();
		$contextId = null;		
			$resultTranslation = virgoTranslation::createGuiAware();
			$contextId = $resultTranslation->getContextId();
			if (isset($contextId)) {
				if (virgoTranslation::getDisplayMode() != "CREATE" || R('portlet_action') == "Duplicate") {
					$resultTranslation->load($contextId);
				}
			}
?>
<?php
...
<?php
	...
	if (isset($masterPobId)) {
		...
		if ($className == "virgoTranslation") {
			$masterObject = new $className();
			$tmpId = $masterObject->getRemoteContextId($masterPobId);
			if (isset($tmpId)) {
				$resultTranslation = new virgoTranslation($tmpId);
				virgoTranslation::setDisplayMode("FORM");
			} else {
				$resultTranslation = new virgoTranslation();
				virgoTranslation::setDisplayMode("CREATE");
			}
		}
	} else {
		if (P('form_only', "0") == "5") {
			if (is_null($resultTranslation->getId())) { 
				if (P('only_private_records', "0") == "1") {
					$allPrivateRecords = $resultTranslation->selectAll();
					if (sizeof($allPrivateRecords) > 0) {
						$resultTranslation = new virgoTranslation($allPrivateRecords[0]['trn_id']);
						$resultTranslation->putInContext(false);
					} else {
						$resultTranslation = new virgoTranslation();
					}
				} else {
					...
					if (isset($customSQL) && trim($customSQL) != '') {
						...
						$records = $resultTranslation->selectAll($customSQL);
						if (sizeof($records) > 0) {
							$resultTranslation = new virgoTranslation($records[0]['trn_id']);
							$resultTranslation->putInContext(false);
						} else {
							$resultTranslation = new virgoTranslation();
						}
			...
			$resultTranslation = new virgoTranslation(virgoUser::getUserId());
			$resultTranslation->putInContext(false);
		}
	}
?>
...
?>
<?php
	$translationDisplayMode = virgoTranslation::getDisplayMode();
//	if ($translationDisplayMode == "" || $translationDisplayMode == "TABLE") {
//		$resultTranslation = $resultTranslation->portletActionForm();
//	}
?>
		<div class="form">
<?php
		$parentContextInfos = $resultTranslation->getParentsInContext();
...
<?php
		...
		if (isset($criteriaTranslation["text"])) {
			...
			if ($fieldCriteriaText["is_null"] != 0) {
				$countTmp = $countTmp + 1;
			} else {
...
?>
				...
				<td class="db_criteria_value">
					<button class="button btn btn-mini" onclick="
						this.form.virgo_filter_column.value='text';
						this.form.portlet_action.value='RemoveCriterium';
						<?php echo JSFS() ?>
					">
<?php echo T('REMOVE_CRITERIA') ?>					</button>
...
<?php
PROFILE('token');
	if (isset($resultTranslation)) {
		$tmpId = is_array($resultTranslation) ? $resultTranslation['trn_id'] : $resultTranslation->getId();
		if (isset($tmpId)) {
?>
...
?>
<?php
	if ($translationDisplayMode != "TABLE") {
		...
		if (
			...
			$pob = $resultTranslation->getMyPortletObject();
			...
			if (isset($invokedPortletId) && $invokedPortletId == $_SESSION['current_portlet_object_id'] && isset($reloadFromRequest) && $reloadFromRequest == "1") { 
				...
				$resultTranslation->loadFromRequest();
			} else {
				if (P('form_only', "0") == "1" && isset($contextId)) {
					if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoTranslation'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
						require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoTranslation'.DIRECTORY_SEPARATOR.'create_store_message.php');
						$translationDisplayMode = "-empty-";
					}
				}
			}
		}
	}
if (!$resultTranslation->hideContentDueToNoParentSelected()) {
...
/* MILESTONE 1.1 Form */
	...
	if ($translationDisplayMode == "FORM") {
?>
<script language="JavaScript">
  ...
  function confirmExit() {
	  ...
	  for (var i=0; i < checks.length; i++) {
		  ...
		  if (check.form.name == "virgo_form_translation") {
			  if (check.value == 'T') {
...
?>
					<label align="right" nowrap class="fieldlabel  obligatory " for="trn_language_<?php echo isset($resultTranslation->trn_id) ? $resultTranslation->trn_id : '' ?>">
 *
...
//		$limit_language = $componentParams->get('limit_to_language');
		$limit_language = null;
		$tmpId = portal\virgoTranslation::getParentInContext("portal\\virgoLanguage");
		$readOnly = "";
		if (isset($tmpId) || P('show_form_language', "1") == "2") {
// to powinno bylo juz byc zrobione w createGuiAware()			
//			if (!is_null($tmpId)) {
//				$resultTranslation->trn_lng__id = $tmpId;
//			}
			if (!is_null($resultTranslation->getLngId())) {
				$parentId = $resultTranslation->getLngId();
...
?>
						<input type="hidden" id="trn_language_<?php echo $resultTranslation->getId() ?>" name="trn_language_<?php echo $resultTranslation->getId() ?>" value="<?php echo $parentId ?>">
						<span class="inputbox readonly"><?php echo isset($parentValue) ? $parentValue : '' ?></span>
<?php
		} else {
?>
<?php
	$hint = TE('HINT_TRANSLATION_LANGUAGE');
?>
<?php
	$whereList = "";
	...
	$showAjaxtrn = P('show_form_language', "1") == "3" || $parentCount > 100;
	if (!$showAjaxtrn) {
?>
    						<select 
							class="inputbox  obligatory " 
							id="trn_language_<?php echo !is_null($resultTranslation->getId()) ? $resultTranslation->getId() : '' ?>" 
							name="trn_language_<?php echo !is_null($resultTranslation->getId()) ? $resultTranslation->getId() : '' ?>" 
<?php
	if (isset($hint)) {
?>
							title="<?php echo $hint ?>"
...
<?php 
				echo (!is_null($resultTranslation->getLngId()) && $id == $resultTranslation->getLngId() ? "selected='selected'" : "");
?>
							>
								<?php echo $label ?>
							</option>
...
?>
	...
	<input 
		...
		id="trn_language_dropdown_<?php echo $resultTranslation->getId() ?>" 
		autocomplete="off" 
...
$(function() {
        $( "#trn_language_dropdown_<?php echo $resultTranslation->getId() ?>" ).autocomplete({
            delay: 0,
            ...
            select: function( event, ui ) {
				if (ui.item.value != '') {
					$('#trn_language_<?php echo $resultTranslation->getId() ?>').val(ui.item.value);
				  	$('#trn_language_dropdown_<?php echo $resultTranslation->getId() ?>').val(ui.item.label);
				  	$('#trn_language_dropdown_<?php echo $resultTranslation->getId() ?>').addClass("locked");
				}
            ...
            open: function() {
				$('#trn_language_<?php echo $resultTranslation->getId() ?>').val('');
				$('#trn_language_dropdown_<?php echo $resultTranslation->getId() ?>').removeClass("locked");		
...
?>						
<script type="text/javascript">
$('#trn_language_dropdown_<?php echo $resultTranslation->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["lng"])) {
		...
		$parentValue = $resultTranslation->trn_lng_id;
	}
	
?>
				<input type="hidden" id="trn_language_<?php echo $resultTranslation->trn_id ?>" name="trn_language_<?php echo $resultTranslation->trn_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<?php
...
?>
					<label 
						nowrap="nowrap"
						class="fieldlabel  obligatory   token varchar" 
						for="trn_token_<?php echo $resultTranslation->getId() ?>"
					>* <?php echo T('TOKEN') ?>
</label>
<?php
	if (!$formsInTable) {
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultTranslation->getToken(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="token" name="trn_token_<?php echo $resultTranslation->getId() ?>" value="<?php echo htmlentities($resultTranslation->getToken(), ENT_QUOTES, "UTF-8") ?>"> 
...
<?php
	if (!isset($resultTranslation)) {
		$resultTranslation = new portal\virgoTranslation();
	}
?>
						<input 
							class="inputbox  obligatory   medium  " 
							type="text"
							id="trn_token_<?php echo $resultTranslation->getId() ?>" 
							name="trn_token_<?php echo $resultTranslation->getId() ?>"
							maxlength="255"
							size="50" 
							value="<?php echo htmlentities($resultTranslation->getToken(), ENT_QUOTES, "UTF-8") ?>" 
							onchange="this.form.virgo_changed.value='T'"
<?php
	if (isset($tabIndex) && $tabIndex > 0) {
?>
...
<?php
	...
	$hint = TE('HINT_TRANSLATION_TOKEN');
	if (isset($hint)) {
?>
							title="<?php echo $hint ?>"
<?php
...
?>						
<script type="text/javascript">
$('#trn_token_<?php echo $resultTranslation->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (P('show_form_text', "1") == "1" || P('show_form_text', "1") == "2") {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label 
						nowrap="nowrap"
						class="fieldlabel  <?php echo P('show_form_text_obligatory', "0") == "1" ? " obligatory " : "" ?>   text varchar" 
						for="trn_text_<?php echo $resultTranslation->getId() ?>"
					> <?php echo P('show_form_text_obligatory', "0") == "1" ? "* " : "" ?><?php echo T('TEXT') ?>
</label>
<?php
	if (!$formsInTable) {
?>
...
?>
					<span align="left" nowrap>
<?php
	if (P('show_form_text', "1") == "2") {
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultTranslation->getText(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="text" name="trn_text_<?php echo $resultTranslation->getId() ?>" value="<?php echo htmlentities($resultTranslation->getText(), ENT_QUOTES, "UTF-8") ?>"> 
...
?>
						<input 
							class="inputbox  <?php echo P('show_form_text_obligatory', "0") == "1" ? " obligatory " : "" ?>   long  " 
							type="text"
							id="trn_text_<?php echo $resultTranslation->getId() ?>" 
							name="trn_text_<?php echo $resultTranslation->getId() ?>"
							maxlength="2000"
							size="50" 
							value="<?php echo htmlentities($resultTranslation->getText(), ENT_QUOTES, "UTF-8") ?>" 
							onchange="this.form.virgo_changed.value='T'"
<?php
	if (isset($tabIndex) && $tabIndex > 0) {
?>
...
?>						
<script type="text/javascript">
$('#trn_text_<?php echo $resultTranslation->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
if (P('hide_audit', '0') == '0') {
	$showFieldset = 0;
	if ($resultTranslation->getDateCreated()) {
		if ($resultTranslation->getUsrCreatedId() == 0) {
			$username = "anonymous";
		} else {
			$createdByUser = new virgoUser((int)$resultTranslation->getUsrCreatedId());
			$username = $createdByUser->getUsername();
		}
		$createdOn = $resultTranslation->getDateCreated();
		$showFieldset = 1;
?>
		...
		<legend><?php echo T('AUDIT_DATA') ?>:</legend>
			<ul>
				<li>
					...
					<input type="hidden" id="usr_created_id" name="usr_created_id" value="<?php echo $resultTranslation->getUsrCreatedId() ?>"	>
				</li>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('CREATION_DATE') ?></span>
					<span class="readonly"><i><?php echo $createdOn ?></i></span>
					<input type="hidden" id="date_created" name="date_created" value="<?php echo $resultTranslation->getDateCreated() ?>"	>
				</li>
<?php
	}
?>
<?php
	if ($resultTranslation->getDateModified()) {
		if ($resultTranslation->getUsrModifiedId() == 0) {
			$username = "anonymous";
		} else {
			$modifiedByUser = new virgoUser((int)$resultTranslation->getUsrModifiedId());
			$username = $modifiedByUser->getUsername();
		}
		$modifiedOn = $resultTranslation->getDateModified();
?>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('MODIFIED_BY') ?></span>
					<span class="readonly"><b><?php echo $username ?></b></span>
					<input type="hidden" id="usr_modified_id" name="usr_modified_id" value="<?php echo $resultTranslation->getUsrModifiedId() ?>"	>
				</li>
				<li>
					...
					<input type="hidden" id="date_modified" name="date_modified" value="<?php echo $resultTranslation->getDateModified() ?>"	>
				</li>
<?php
	}
	if ($showFieldset == 1) {
...
?>
				...
				<div class="buttons form-actions">
						<input type="text" name="trn_id_<?php echo $this->getId() ?>" id="trn_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
 ...
 <div class="button_wrapper button_wrapper_store inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'trn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultTranslation) ? (is_array($resultTranslation) ? $resultTranslation['trn_id'] : $resultTranslation->getId()) : '' ?>');
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("TRANSLATION"), "\\'".rawurlencode($resultTranslation->getVirgoTitle())."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
...
// document.forms['portlet_form_<?php echo $this->getId() ?>'].elements['trn_language_<?php echo $resultTranslation->getId() ?>'].focus();
</script>
	</div>
<?php
/* MILESTONE 1.2 Create */
	} elseif ($translationDisplayMode == "CREATE") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
...
?>
<?php
		if (isset($resultTranslation->trn_id)) {
			$resultTranslation->trn_id = null;
		}
		$parentAjaxRendered = "0";
?>
<?php
...
if (isset($defaultValue) && trim($defaultValue) != "") {
	...
	$resultTranslation->setLngId($defaultValue);
}
	}
?>
																										<?php
...
<?php
	...
	$parentCount = portal\virgoLanguage::getVirgoListSize($whereList);
	$showAjaxtrn = P('show_create_language', "1") == "3" || $parentCount > 100;
	if (!$showAjaxtrn) {
?>
    						<select 
							class="inputbox  obligatory " 
...
?>
					<label nowrap class="fieldlabel  obligatory " for="trn_token_<?php echo $resultTranslation->getId() ?>">
* <?php echo T('TOKEN') ?>
					</label>
<?php
	if (!$formsInTable) {
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "token") {
				$resultTranslation->setToken($virgoSelectedDay);
			}
		}
	}
...
<?php
	if (P('show_create_text', "1") == "1" || P('show_create_text', "1") == "2") {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label nowrap class="fieldlabel  <?php echo P('show_create_text_obligatory', "0") == "1" ? " obligatory " : "" ?> " for="trn_text_<?php echo $resultTranslation->getId() ?>">
 <?php echo P('show_create_text_obligatory', "0") == "1" ? "* " : "" ?><?php echo T('TEXT') ?>
					</label>
<?php
	if (!$formsInTable) {
?>
...
<?php
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "text") {
				$resultTranslation->setText($virgoSelectedDay);
			}
		}
	}
	if (P('show_create_text', "1") == "2") {
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultTranslation->getText(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
...
/* MILESTONE 1.3 Search */
	} elseif ($translationDisplayMode == "SEARCH") {
?>
...
<?php echo T('TEXT') ?>
		...
		<span align="left" nowrap>
						<input 
							class="inputbox" 
							type="text"
							id="virgo_search_text" 
							name="virgo_search_text"
							style="border: yellow 1 solid;" 
							maxlength="2000"
							size="50" 
							value="<?php echo isset($dataTypeCriteria["default"]) ? htmlentities($dataTypeCriteria["default"], ENT_QUOTES, "UTF-8") : "" ?>" 
...
?>
		...
		<span class="virgo_search_empty_values" align="left" nowrap style="font-size: 0.7em;">
			<?php echo T('VALUE') ?>			<select 
				id="virgo_search_text_is_null" 
				name="virgo_search_text_is_null"
			>
				<option value=""
<?php
		if (isset($fieldCriteriaText) && $fieldCriteriaText["is_null"] == 0) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="not_null"
<?php
		if (isset($fieldCriteriaText) && $fieldCriteriaText["is_null"] == 1) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="null"
<?php
		if (isset($fieldCriteriaText) && $fieldCriteriaText["is_null"] == 2) {
?>
				selected="selected"
<?php
		}
...
<?php
	if (P('show_search_language', '1') == "1") {
		if (isset($criteriaTranslation["language"])) {
			$fieldCriteriaLanguage = $criteriaTranslation["language"];
		}
?>
				<li
					<?php echo (!$formsInTable && $floatingFields) ? "style='display: inline-block; float: left;'" : '' ?>
...
<?php
/* MILESTONE 1.4 View */
	} elseif ($translationDisplayMode == "VIEW") {
?>
	<div class="form_view">
<?php
...
<?php
			...
			$readOnly = "";
			if ($resultTranslation->getId() == 0) {
// przesuac do createforgui				$resultTranslation->trn_lng__id = $tmpId;
			}
			$parentId = $resultTranslation->getLanguageId();
			$parentValue = portal\virgoLanguage::lookup($parentId);
				
?>
					<input
						type="hidden"
						id="trn_language_<?php echo isset($resultTranslation->trn_id) ? $resultTranslation->trn_id : '' ?>" 
						name="trn_language_<?php echo isset($resultTranslation->trn_id) ? $resultTranslation->trn_id : '' ?>" 						
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("TRANSLATION"), "\\'".$resultTranslation->getVirgoTitle()."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'trn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultTranslation) ? (is_array($resultTranslation) ? $resultTranslation['trn_id'] : $resultTranslation->getId()) : '' ?>');
...
<?php
/* MILESTONE 1.5 Table */
	} elseif ($translationDisplayMode == "TABLE") {
PROFILE('TABLE');
		if (P('form_only') == "3") {
?>
<?php
	...
	$resultCount = -1;
	$filterApplied = false;
	$resultTranslation->setShowPage(1); 
	$resultTranslation->setShowRows('all'); 	
	$resultsTranslation = $resultTranslation->getTableData($resultCount, $filterApplied);
	$events = array();
	foreach ($resultsTranslation as $resultTranslation) {
		if (isset($resultTranslation[$eventColumn]) && isset($events[substr($resultTranslation[$eventColumn], 0, 10)])) {
			$eventsInDay = $events[substr($resultTranslation[$eventColumn], 0, 10)];
		} else {
			$eventsInDay = array();
		}
		$eventObject = new virgoTranslation($resultTranslation['trn_id']);
		$eventsInDay[] = $eventObject;
		$events[substr($resultTranslation[$eventColumn], 0, 10)] = $eventsInDay;
	}
?>
<style type="text/css">
...
?>			
			...
			<tr class='virgo_calendar_week'>
				...
				<input type='hidden' name='trn_id_<?php echo $this->getId() ?>' value=''/>
<?php
	$tmpWeekDay = 1;
	$safety = 0;
	if ($tmpDay["wday"] > 0) {
...
<?php
		...
		if (isset($events[$tmpStringDay])) {
			...
			foreach ($eventsInDay as $resultTranslation) {
?>
<?php
PROFILE('token');
	if (isset($resultTranslation)) {
...
<div class="button_wrapper button_wrapper_view inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'trn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultTranslation) ? (is_array($resultTranslation) ? $resultTranslation['trn_id'] : $resultTranslation->getId()) : '' ?>');
 								...
 								form.target = '';
							" 
							value="<?php echo T($resultTranslation->getVirgoTitle()) ?>"
						/><div class="button_right"></div></div><?php
	}
?>
<?php				
//				echo "<span class='virgo_calendar_event' onclick='var form=document.getElementById(\"portlet_form_".$this->getId()."\");form.portlet_action.value=\"View\";form.trn_id_".$this->getId().".value=\"".$eventInDay->getId()."\";form.submit();'>" . $eventInDay->getVirgoTitle() . "</span>";
			}
		}
		echo "</td>";
...
?>
		<script type="text/javascript">
			var translationChildrenDivOpen = '';
			
			function childrenButtonClicked(clickedDivId) {
				var div = document.getElementById(clickedDivId);
				if (clickedDivId == translationChildrenDivOpen) {
					div.style.display = 'none';
					translationChildrenDivOpen = '';
				} else {
					if (translationChildrenDivOpen != '') {
						document.getElementById(translationChildrenDivOpen).style.display = 'none';
					}
					div.style.display = 'block';
					translationChildrenDivOpen = clickedDivId;
				}
			}
...
PROFILE('table_01');
PROFILE('table_02');
			$showPage = $resultTranslation->getShowPage(); 
			$showRows = $resultTranslation->getShowRows(); 
?>
						<input type="text" name="trn_id_<?php echo $this->getId() ?>" id="trn_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>			
				<tr><td colspan="99" class="nav-header"><?php echo T('Translations') ?></td></tr>
<?php
			}
?>			
<?php
PROFILE('table_02');
PROFILE('main select');
			$virgoOrderColumn = $resultTranslation->getOrderColumn();
			$virgoOrderMode = $resultTranslation->getOrderMode();
			$resultCount = -1;
			$filterApplied = false;
			$resultsTranslation = $resultTranslation->getTableData($resultCount, $filterApplied);
PROFILE('main select');
...
?>
<input name='trn_Language_id_<?php echo $this->getId() ?>' id='trn_Language_id_<?php echo $this->getId() ?>' type="hidden"/>
<?php		
	}
?>
				<th align="center" valign="middle" rowspan=" 2"><!-- bylo: nowrap -->
<?php
	$oc = $resultTranslation->getOrderColumn(); 
	$om = $resultTranslation->getOrderMode();
...
<?php
	if (P('show_table_filter', '0') == '1') {
		...
		if ($parentsCount < 51) {
			...
			$parentFilter = virgoTranslation::getLocalSessionValue('VirgoFilterLanguage', null);
?>
						<select 
...
<?php		
			...
			$parentFilter = virgoTranslation::getLocalSessionValue('VirgoFilterTitleLanguage', null);
?>
						<input
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'trn_token');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('TOKEN') ?>							<?php echo ($oc == "trn_token" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoTranslation::getLocalSessionValue('VirgoFilterToken', null);
?>
						<input
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'trn_text');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('TEXT') ?>							<?php echo ($oc == "trn_text" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoTranslation::getLocalSessionValue('VirgoFilterText', null);
?>
						<input
							name="virgo_filter_text"
							class="virgo_filter"
							onChange="<?php echo JSFS(null, "Submit", true, array(), false, "SetVirgoTableFilter") ?>"
							value="<?php echo $tableFilter ?>"
						/>
...
<?php
			...
			if ($resultCount != 0) {
				if (((int)$showRows) * (((int)$showPage)-1) == $resultCount ) {
					...
					$resultTranslation->setShowPage($showPage);
				}
				$index = 0;
PROFILE('table_04');
...
?>
<?php
$fileNameToInclude = PORTAL_PATH . "/portlets/portal/virgoTranslation/modules/renderTableRow_{$_SESSION['current_portlet_object_id']}.php";
...
<?php
			if (P('form_only') != "4" || (P('chessboard_width') != 0 && $index % P('chessboard_width') == 1)) {
				if (is_null($firstRowId)) {
					$firstRowId = $resultTranslation['trn_id'];
				}
				$displayClass = ' displayClass ';
				$tmpContextId = virgoTranslation::getContextId();
				if (is_null($tmpContextId)) {
					$forceContextOnFirstRow = P('force_context_on_first_row', "1");
					if ($forceContextOnFirstRow == "1") {
						virgoTranslation::setContextId($resultTranslation['trn_id'], false);
						$tmpContextId = $resultTranslation['trn_id'];
					}
				}
				if (isset($tmpContextId) && $resultTranslation['trn_id'] == $tmpContextId) {
					if (P('form_only') != "4") {
...
?>
			<tr 
				id="<?php echo $this->getId() ?>_<?php echo isset($resultTranslation['trn_id']) ? $resultTranslation['trn_id'] : "" ?>" 
...
?>
 language_<?php echo isset($resultTranslation['trn_lng_id']) ? $resultTranslation['trn_lng_id'] : "" ?>
<?php
				}
?>
			" 
...
?>
				<input type="checkbox" class="checkbox" style="float: right;" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultTranslation['trn_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
				<input type="checkbox" class="checkbox" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultTranslation['trn_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
			<a  
				...
				onclick="
					...
					form.portlet_action.value='View'; 
					form.trn_id_<?php echo $this->getId() ?>.value='<?php echo isset($resultTranslation['trn_id']) ? $resultTranslation['trn_id'] : '' ?>'; 
					form.target = ''; 
<?php
					if (isset($ajax) && !$ajax) {
?>					
...
<?php 
	if (P('show_table_language', "1") == "1") {
		if (isset($resultTranslation['language'])) {
			echo $resultTranslation['language'];
		}
	} else {
//		echo $resultTranslation['trn_lng_id'];
?>
		<select class="inline" onchange="
this.form.portlet_action.value='VirgoSetLanguage';
	_pSF(form, 'trn_id_<?php echo $this->getId() ?>', '<?php echo $resultTranslation['trn_id'] ?>');
	_pSF(form, 'trn_Language_id_<?php echo $this->getId() ?>', this.options[this.selectedIndex].value);
<?php
					if (isset($ajax) && !$ajax) {
?>					
						form.submit();
...
?>
			<option value="<?php echo $value ?>" <?php echo $value == $resultTranslation['trn_lng_id'] ? " selected " : "" ?>><?php echo $label ?></option>
<?php			
		}
?>
		</select>
...
?>
		<span 
			class="<?php echo 'displayClass' ?>">
				<?php echo htmlentities($resultTranslation['trn_token'], ENT_QUOTES, "UTF-8") ?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
?>
...
PROFILE('text');
	if (P('show_table_text', "0") == "1") {
PROFILE('render_data_table_text');
?>
<?php
			if (P('form_only') != "4") {
?>
...
?>
		<span 
			class="<?php echo 'displayClass' ?>">
				<?php echo htmlentities($resultTranslation['trn_text'], ENT_QUOTES, "UTF-8") ?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
?>
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("TRANSLATION"), "\\'".rawurlencode($resultTranslation['trn_virgo_title'])."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'trn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultTranslation) ? (is_array($resultTranslation) ? $resultTranslation['trn_id'] : $resultTranslation->getId()) : '' ?>');
...
<?php
	...
	if (is_null($contextRowIdInTable)) {
		$forceContextOnFirstRow = P('force_context_on_first_row', "1");
		if ($forceContextOnFirstRow == "1") {
			virgoTranslation::setContextId($firstRowId, false);
			if (P('form_only') != "4") {
?>
<script type="text/javascript">
...
<?php
				...
				unset($resultTranslation);
				unset($resultsTranslation);
				if (isset($contextIdOwn) && trim($contextIdOwn) != "") {
					if ($contextIdConfirmed == false) {
						$tmpTranslation = new virgoTranslation();
						$tmpCount = $tmpTranslation->getAllRecordCount(' trn_id = ' . $contextIdOwn);
						if ($tmpCount == 0) {
							virgoTranslation::clearRemoteContextId($tabModeEditMenu);
						}
					}
				}
PROFILE('rows rendering');
...
?>
		$.getJSON('<?php echo $page->getUrl() ?>?portlet_action=SelectJson&trn_id_<?php echo $this->getId() ?>=' + virgoId + '&invoked_portlet_object_id=<?php echo $this->getId() ?>&virgo_action_mode_json=T&_virgo_ajax=1', function(data) {
<?php
			if (P('form_only') != "4") {
?>
...
?>
		...
		form.trn_id_<?php echo $this->getId() ?>.value=virgoId; 
		form.target = ''; 
<?php
...
<div class="button_wrapper button_wrapper_report inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								...
								for(var i = 0; i< children.length;i++) {
								  if (children[i].getAttribute('name') == 'trn_id_<?php echo $this->getId() ?>') {
									found = 1;
								  }
								}
								if (found == 0) {
								  ...
								  input.setAttribute('name', 'trn_id_<?php echo $this->getId() ?>');
								  input.setAttribute('value', '');
								  form.appendChild(input);
								} else {
	_pSF(form, 'trn_id_<?php echo $this->getId() ?>', '');
								}
								form.action = '';
...
<?php
					$sessionSeparator = virgoTranslation::getImportFieldSeparator();
...
<div class="button_wrapper button_wrapper_deleteselected inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								if (!nothingSelected(this.form, <?php echo $this->getId() ?>)) {
									if (!confirm(decodeURIComponent('<?php echo T('ARE_YOU_SURE_YOU_WANT_REMOVE', T('TRANSLATIONS'), "") ?>'))) return false;
								} else {
									var alertText = '<?php echo T('NOTHING_SELECTED') ?>';
									if (alertText.charAt(alertText.length-1)=='?') {
										if (!confirm(alertText)) return false;
...
/* MILESTONE 1.6 TableForm */
	} elseif ($translationDisplayMode == "TABLE_FORM") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
	...
	<form method="post" style="display: inline;" action="" id="virgo_form_translation" name="virgo_form_translation" enctype="multipart/form-data">
						<input type="text" name="trn_id_<?php echo $this->getId() ?>" id="trn_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
<?php			
				$resultsTranslation = $resultTranslation->getRecordsToEdit();
				$idsToCorrect = $resultTranslation->getInvalidRecords();
				$index = 0;
PROFILE('rows rendering');
				foreach ($resultsTranslation as $resultTranslation) {
					$index = $index + 1;
?>
			<tr id="virgo_tr_id_<?php echo $resultTranslation->trn_id ?>" class="<?php echo ($index % 2 == 0 ? "data_table_even" : "data_table_odd") ?>"
<?php
	if ($resultTranslation->trn_id == 0 && R('virgo_validate_new', "N") == "N") {
?>
		style="display: none;"
<?php
	}
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsTranslation) * 0;
?>
<?php
	if (!isset($resultTranslation)) {
		$resultTranslation = new portal\virgoTranslation();
...
?> 
						<input
							type="hidden"
							id="token_<?php echo $resultTranslation->trn_id ?>" 
							name="token_<?php echo $resultTranslation->trn_id ?>"
							value="<?php echo htmlentities($resultTranslation->trn_token, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsTranslation) * 1;
?>
<?php
	if (!isset($resultTranslation)) {
		$resultTranslation = new portal\virgoTranslation();
...
?> 
						<input
							type="hidden"
							id="text_<?php echo $resultTranslation->trn_id ?>" 
							name="text_<?php echo $resultTranslation->trn_id ?>"
							value="<?php echo htmlentities($resultTranslation->trn_text, ENT_QUOTES, "UTF-8") ?>"
						>
...
<?php
	$tabIndex = $index + sizeof($resultsTranslation) * 2;
?>
<?php
//		$limit_language = $componentParams->get('limit_to_language');
		$limit_language = null;
...
?>
				<td>
<?php
	if (isset($idsToCorrect[$resultTranslation->trn_id])) {
		$errorMessage = $idsToCorrect[$resultTranslation->trn_id];
?>
					<div class="error">
...
?>
		<div class="<?php echo $translationDisplayMode ?>">
<?php
?>
			<div class="buttons form-actions">
<?php
...
<div style="display: none; background-color:#FFFFFF; border:1px solid #000000; font-size:10px; margin:10px 0; padding:10px;"; id="extraFilesInfo_prt_translation" style="font-size: 12px; " onclick="document.getElementById('extraFilesInfo_prt_translation').style.display='none';">
...
<?php
	$infos = virgoTranslation::getExtraFilesInfo();
