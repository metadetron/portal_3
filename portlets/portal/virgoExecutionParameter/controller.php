...
//	require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoExecution'.DIRECTORY_SEPARATOR.'controller.php');
	...
	class virgoExecutionParameter {
		 ...
		 var   $epr_date_created = null;
		 var   $epr_usr_created_id = null;
		 var   $epr_date_modified = null;
		 var   $epr_usr_modified_id = null;
		 var   $epr_virgo_title = null;
		...
		static public function createGuiAware() {
			$ret = new virgoExecutionParameter();
		...
		public function __clone() {
        	$this->epr_id = null;
		    $this->epr_date_created = null;
		    $this->epr_usr_created_id = null;
		    $this->epr_date_modified = null;
		    $this->epr_usr_modified_id = null;
		    $this->epr_virgo_title = null;
    	}		
		...
		function getDateCreated() {
			return $this->epr_date_created;
		}
		function getUsrCreatedId() {
			return $this->epr_usr_created_id;
		}
		function getDateModified() {
			return $this->epr_date_modified;
		}
		function getUsrModifiedId() {
			return $this->epr_usr_modified_id;
		}
		...
		function loadRecordFromRequest($rowId) {
			$this->epr_id = $rowId;
	$tmpValue = null;
	$tmpValue = R('epr_name_' . $this->epr_id);
	if (!is_null($tmpValue)) {
		...
		if ($tmpValue == "") {
			...
			$this->epr_name = $tmpValue;
		}
	}
	$tmpValue = null;
	$tmpValue = R('epr_value_' . $this->epr_id);
	if (!is_null($tmpValue)) {
		if ($tmpValue == "") {
			$this->epr_value = null;
		} else {
			$this->epr_value = $tmpValue;
		}
	}
			$this->epr_exc_id = strval(R('epr_execution_' . $this->epr_id));
		}
		
		function loadFromRequest($rowId = null) {
			if (is_null($rowId)) {
				$rowId = R('epr_id_' . $_SESSION['current_portlet_object_id']);
			}
		...
		static function loadSearchFromRequest() {
			$criteriaExecutionParameter = array();	
			$criteriaFieldExecutionParameter = array();	
			$isNullExecutionParameter = R('virgo_search_name_is_null');
			
			$criteriaFieldExecutionParameter["is_null"] = 0;
			if ($isNullExecutionParameter == "not_null") {
				$criteriaFieldExecutionParameter["is_null"] = 1;
			} elseif ($isNullExecutionParameter == "null") {
				$criteriaFieldExecutionParameter["is_null"] = 2;
			}
...
//			if ($isSet) {
			$criteriaFieldExecutionParameter["value"] = $dataTypeCriteria;
//			}
			$criteriaExecutionParameter["name"] = $criteriaFieldExecutionParameter;
			$criteriaFieldExecutionParameter = array();	
			$isNullExecutionParameter = R('virgo_search_value_is_null');
...
//			if ($isSet) {
			$criteriaFieldExecutionParameter["value"] = $dataTypeCriteria;
//			}
			$criteriaExecutionParameter["value"] = $criteriaFieldExecutionParameter;
			...
			$criteriaExecutionParameter["execution"] = $criteriaParent;
			self::setCriteria($criteriaExecutionParameter);
		}
		...
		function showApplicableOnlyRecords(&$filterApplied, &$types, &$values) {
			$whereClauseExecutionParameter = ' 1 = 1 ';
			if (P('form_only') == "3") {
				...
				$eventColumn = "epr_" . P('event_column');
				$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND " . $eventColumn . " BETWEEN ? AND ? ";
				$types .= "ss";
			...
			foreach ($parentContextInfos as $parentContextInfo) {
				$whereClauseExecutionParameter = $whereClauseExecutionParameter . ' AND ' . $parentContextInfo['condition'];
			}
			$includeNulls = false;
			$includeIns = true;
			...
			if (isset($limit) && trim($limit) != '') {
				if ($includeIns) {
					eval("\$limit = \"$limit\";");
					$inCondition = " prt_execution_parameters.epr_exc_id IN (" . $limit . ") ";
				} else {
					$inCondition = " 1 ";
				}
				if ($includeNulls) {
					...
					$nullCondition = $nullCondition . " prt_execution_parameters.epr_exc_id IS NULL";
				} else {
					$nullCondition = " OR 0 "; 				}
				$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND ({$inCondition} {$nullCondition} )";
			}
			$filter = "";
			$criteriaExecutionParameter = self::getCriteria();
			if (isset($criteriaExecutionParameter["name"])) {
				$fieldCriteriaName = $criteriaExecutionParameter["name"];
				if ($fieldCriteriaName["is_null"] == 1) {
$filter = $filter . ' AND prt_execution_parameters.epr_name IS NOT NULL ';
				} elseif ($fieldCriteriaName["is_null"] == 2) {
$filter = $filter . ' AND prt_execution_parameters.epr_name IS NULL ';
				} else {
					$dataTypeCriteria = $fieldCriteriaName["value"];
			$condition = $dataTypeCriteria["default"];
			if (!is_null($condition) && trim($condition) != "") { 
				$filter = $filter . " AND prt_execution_parameters.epr_name like ? ";
				$types .= "s";
				$values[] = $condition;
			}
				}
			}
			if (isset($criteriaExecutionParameter["value"])) {
				$fieldCriteriaValue = $criteriaExecutionParameter["value"];
				if ($fieldCriteriaValue["is_null"] == 1) {
$filter = $filter . ' AND prt_execution_parameters.epr_value IS NOT NULL ';
				} elseif ($fieldCriteriaValue["is_null"] == 2) {
$filter = $filter . ' AND prt_execution_parameters.epr_value IS NULL ';
				} else {
					$dataTypeCriteria = $fieldCriteriaValue["value"];
			$condition = $dataTypeCriteria["default"];
			if (!is_null($condition) && trim($condition) != "") { 
				$filter = $filter . " AND prt_execution_parameters.epr_value like ? ";
				$types .= "s";
				$values[] = $condition;				
			}
				}
			}
			if (isset($criteriaExecutionParameter["execution"])) {
				$parentCriteria = $criteriaExecutionParameter["execution"];
				if ($parentCriteria["is_null"] == 1) {
					$filter = $filter . " AND epr_exc_id IS NULL ";
				} else {
					if (isset($parentCriteria["value"]) && trim($parentCriteria["value"]) != "") {
				  		$parent = $parentCriteria["value"];
						$filter = $filter . " AND prt_execution_parameters.epr_exc_id IN (SELECT exc_id FROM prt_executions WHERE exc_virgo_title LIKE ?) ";
						$types .= "s";
						$values[] = $parent;
					}
				}
			}
			$whereClauseExecutionParameter = $whereClauseExecutionParameter . " " . $filter . " ";
			...
			if (isset($customSQL) && trim($customSQL) != '') {
				...
				$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND " . $customSQL . " ";
			}
			...
			if (S($customParentQuery)) {
				...
				if (S($customParentId)) {
					...
					$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND " . $customParentQuery . " ";
				}
			}
			if (P("show_table_filter", '0') == 1) {
				$tableFilter = self::getLocalSessionValue('VirgoFilterName', null);
				if (S($tableFilter)) {
					$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND epr_name LIKE '%{$tableFilter}%' ";
				}
				$tableFilter = self::getLocalSessionValue('VirgoFilterValue', null);
				if (S($tableFilter)) {
					$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND epr_value LIKE '%{$tableFilter}%' ";
				}
				$parentFilter = self::getLocalSessionValue('VirgoFilterExecution', null);
				if (S($parentFilter)) {
					if ($parentFilter == "empty") {
						$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND epr_exc_id IS NULL ";
					} else {
						$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND epr_exc_id = $parentFilter ";
					}
				}
				$parentFilter = self::getLocalSessionValue('VirgoFilterTitleExecution', null);
				if (S($parentFilter)) {
					$whereClauseExecutionParameter = $whereClauseExecutionParameter . " AND prt_executions_parent.exc_virgo_title LIKE '%{$parentFilter}%' ";
				}				
			}
			return $whereClauseExecutionParameter;
		}
		...
		function getTableData(&$resultCount, &$filterApplied) {
			...
			$values = array();
			$filterApplied = false;
			$whereClauseExecutionParameter = $this->showApplicableOnlyRecords($filterApplied, $types, $values);
			$queryString = " SELECT prt_execution_parameters.epr_id, prt_execution_parameters.epr_virgo_title ";
			$defaultOrderColumn = P('default_sort_column');
			$orderColumnNotDisplayed = "";
			if (P('show_table_name', "1") != "0") {
				$queryString = $queryString . ", prt_execution_parameters.epr_name epr_name";
			} else {
				if ($defaultOrderColumn == "epr_name") {
					$orderColumnNotDisplayed = " prt_execution_parameters.epr_name ";
				}
			}
			if (P('show_table_value', "1") != "0") {
				$queryString = $queryString . ", prt_execution_parameters.epr_value epr_value";
			} else {
				if ($defaultOrderColumn == "epr_value") {
					$orderColumnNotDisplayed = " prt_execution_parameters.epr_value ";
				}
			}
			if (class_exists('portal\virgoExecution') && P('show_table_execution', "1") != "0") { // */ && !in_array("exc", $hideColumnFromContextInTable)) {
				$queryString = $queryString . ", prt_execution_parameters.epr_exc_id as epr_exc_id ";
			...
			$queryString = $queryString . " FROM prt_execution_parameters ";
			if (class_exists('portal\virgoExecution')) {
				$queryString = $queryString . " LEFT OUTER JOIN prt_executions AS prt_executions_parent ON (prt_execution_parameters.epr_exc_id = prt_executions_parent.exc_id) ";
			}
			...
			if (trim($extraCondition) != "") {
				$whereClauseExecutionParameter = $whereClauseExecutionParameter . " " . $extraCondition;
			}

			$resultCount = $this->getAllRecordCount($whereClauseExecutionParameter, $queryString, $types, $values);
		...
		function selectAll($where = '', $orderBy = '', $types = null, $values = null) {
			$query = "SELECT * "
			. "\n FROM prt_execution_parameters"
			;
			$componentParams = null;
			if (P('only_private_records', "0") == "1") {
				$privateCondition = " epr_usr_created_id = '" . virgoUser::getUserId() . "' ";
				if ($where == '') {
		...
		function select($showPage, $showRows, $orderColumn, $orderMode, $where = '', $queryString = '', $types = null, $values = null) {
			$componentParams = null;
			if (P('only_private_records', "0") == "1") {
				$privateCondition = " epr_usr_created_id = ? ";
				if ($where == '') {
					$where = $privateCondition;
				} else {
					$where = $where . " AND " . $privateCondition;
			...
			if ($queryString == '') {
				...
				if ($where != '') { 
					$query = "SELECT * FROM ( " . $queryString . (strpos(strtoupper($queryString), "WHERE") === false ? " WHERE " : " AND ") . $where . " ) as prt_execution_parameters ";
				} else {			
					$query = "SELECT * FROM ( " . $queryString . " ) as prt_execution_parameters ";
				}
			}
			if ($orderColumn != null) {
				$query = $query . "\n ORDER BY $orderColumn $orderMode, epr_id $orderMode";
			}
			if ($showRows != 'all') {
		...
		function getAllRecordCount($where = '', $queryString = '', $types = null, $values = null) {
			...
			if ($queryString == '') {
				$query = "SELECT COUNT(epr_id) cnt FROM execution_parameters";
				if ($where != '') {
					$query = $query . " WHERE " . $where . " ";
				}	
			} else {
				if ($where != '') {
					$query = "SELECT COUNT(*) as cnt FROM (" . $queryString . " WHERE " . $where . ") as execution_parameters ";
				} else {
					$query = "SELECT COUNT(*) as cnt FROM (" . $queryString . ") as execution_parameters ";
				}
			}
		...
		static function getById($id) {
			$instance = new virgoExecutionParameter();
			$instance->load($id);
			return $instance;
		}
		...
		function load($id, $fetchUsernames = false) {
			if (!is_null($id) && trim($id) != "") {
				$query = "SELECT * FROM prt_execution_parameters WHERE epr_id = {$id}";
				$results = QR($query);
				if (!isset($results)) {
					...
					if (count($results) > 0) {
						$row = $results[0];
						$this->epr_id = $row['epr_id'];
$this->epr_name = $row['epr_name'];
$this->epr_value = $row['epr_value'];
						$this->epr_exc_id = $row['epr_exc_id'];
						if ($fetchUsernames) {
							if ($row['epr_date_created']) {
								if ($row['epr_usr_created_id'] == 0) {
									$createdBy = "anonymous";
								} else {
									$createdByUser = new virgoUser((int)$row['epr_usr_created_id']);
									$createdBy = $createdByUser->getUsername();
								}
							} else {
								$createdBy = "";
							}
							if ($row['epr_date_modified']) {
								if ($row['epr_usr_modified_id'] == $row['epr_usr_created_id']) {
									$modifiedBy = $createdBy;
								} else {
									if ($row['epr_usr_modified_id'] == 0) {
										$modifiedBy = "anonymous";
									} else {
										$modifiedByUser = new virgoUser((int)$row['epr_usr_modified_id']);
						...
						$this->epr_date_created = $row['epr_date_created'];
						$this->epr_usr_created_id = $fetchUsernames ? $createdBy : $row['epr_usr_created_id'];
						$this->epr_date_modified = $row['epr_date_modified'];
						$this->epr_usr_modified_id = $fetchUsernames ? $modifiedBy : $row['epr_usr_modified_id'];
						$this->epr_virgo_title = $row['epr_virgo_title'];
					}
				}
			}
		}
		...
		function changeOwnershipAndStore($userId) {
			$query = " UPDATE prt_execution_parameters SET epr_usr_created_id = {$userId} WHERE epr_id = {$this->getId()} ";
			$result = Q($query);
			if (!$result) {
				L(mysqli_error(), $query, 'ERROR');
			}
			$this->epr_usr_created_id = $userId;
		}
		
		static function getRecordCreatedBy($userId) {
			$rets = virgoExecutionParameter::selectAllAsObjectsStatic('epr_usr_created_id = ' . $userId);
			foreach ($rets as $ret) {
				return $ret;
			}
			return null;
		...
		static function loadIdFromRequest($id = null) {
			return intval(is_null($id) ? R('epr_id_' . $_SESSION['current_portlet_object_id']) : $id);
		}
		
		static function lookup($lookup_id) {
			if ($lookup_id) {
				$tmp_epr = new virgoExecutionParameter();
				$tmp_epr->load((int)$lookup_id);
				return $tmp_epr->getVirgoTitle();
			}
		} 
		...
		function printVirgoListMatchedInternal($match, $fieldName) {
			if (A(virgoUser::getUserId())) {
				$match = QE($match);
				$extraAjaxFilter = PD('extra_ajax_filter', 'virgoExecutionParameter');
				if (S($extraAjaxFilter)) {
					$extraAjaxFilter = " AND ({$extraAjaxFilter}) ";
				} else {
					$extraAjaxFilter = "";
				}
				$resultsLabels = $this->getVirgoList(" epr_virgo_title LIKE '%{$match}%' {$extraAjaxFilter} ", false, true);
				$sizeOf = sizeof($resultsLabels);
				$maxListLabelSize = PD('ajax_max_label_list_size', 'virgoExecutionParameter', "10");
...
$this->printVirgoListMatchedInternal($match, $fieldName);
		...
		static function getVirgoList($where = '', $sizeOnly = false, $hash = false) {
			...
			if ($sizeOnly) {
				...
				$query = $query . " epr_id as id, epr_virgo_title as title ";
			}
			$query = $query . " FROM prt_execution_parameters ";
			$componentParams = null;
			$tmpQuery = " WHERE 1 = 1 ";
			if (P('only_private_records', "0", 'virgoExecutionParameter', 'portal') == "1") {
				$privateCondition = " epr_usr_created_id = '" . virgoUser::getUserId() . "' ";
			...
			if (!$sizeOnly) {
				$query = $query . " ORDER BY epr_virgo_title ";
			}
			if ($sizeOnly) {
	$result = QL($query);
				return $result[0];
	...
	$rows = QR($query);
				...
				foreach ($rows as $row) {
					$resExecutionParameter[$row['id']] = $row['title'];
				}
		...
		static function getVirgoListStatic($where = '', $sizeOnly = false, $hash = false) {
			$staticExecutionParameter = new virgoExecutionParameter();
			return $staticExecutionParameter->getVirgoList($where, $sizeOnly, $hash);
		}
		...
		function getExecution() {
			return virgoExecutionParameter::getExecutionStatic($this->epr_exc_id);
		}
...
(is_null($this->getValue()) || trim($this->getValue()) == '')
				...
				if (is_null($this->epr_exc_id) || trim($this->epr_exc_id) == "") {
					if (R('create_epr_execution_' . $this->epr_id) == "1") { 
		...
		function getCurrentRevision() {
			$query = "SELECT IFNULL(MAX(revision), 0) as revision FROM  prt_history_execution_parameters WHERE epr_id = " . $this->getId();
	$result = QL($query);
			return $result[0];
		}
		...
		function storeRecordChange($virgoOld, $user) {
			...
			if ($new_revision == 1 && !is_null($virgoOld)) {
				...
				$colNames = $colNames . ", epr_name";
				$values = $values . ", " . (is_null($objectToStore->getName()) ? "null" : "'" . QE($objectToStore->getName()) . "'");
				$colNames = $colNames . ", epr_value";
				$values = $values . ", " . (is_null($objectToStore->getValue()) ? "null" : "'" . QE($objectToStore->getValue()) . "'");
				$colNames = $colNames . ", epr_exc_id";
				$values = $values . ", " . (is_null($objectToStore->getExcId()) || $objectToStore->getExcId() == "" ? "null" : $objectToStore->getExcId());
				$query = "INSERT INTO prt_history_execution_parameters (revision, ip, username, user_id, timestamp, epr_id" . $colNames . ") VALUES ($new_revision, '$ip', '$username', $user_id, '" . date('Y-m-d H:i:s') . "', " . $this->getId() . $values . ") ";
				Q($query);
				$new_revision = 2;
			}
			$colNames = "";
		...
		function virgoTitleColumnExists() {
			$exists = false;
			$columns = Q("SHOW COLUMNS FROM prt_execution_parameters");
			if (isset($columns) && $columns !== false) {
				foreach ($columns as $c){
					if($c['Field'] == 'epr_virgo_title'){
						return true;
					}
				}
			}
		...
		function addVirgoTitleColumn() {
			$query = "ALTER TABLE prt_execution_parameters ADD COLUMN (epr_virgo_title VARCHAR(255));";
			Q($query);
		...
		function parentStore($updateNulls = false, $log = false) {
			$types = "";
			$values = array();
			if (isset($this->epr_id) && $this->epr_id != "") {
				$query = "UPDATE prt_execution_parameters SET ";
			if (isset($this->epr_name)) {
				$query .= " epr_name = ? ,";
				$types .= "s";
				$values[] = $this->epr_name;
			} else {
				$query .= " epr_name = NULL ,";				
			}
			if (isset($this->epr_value)) {
				$query .= " epr_value = ? ,";
				$types .= "s";
				$values[] = $this->epr_value;
			} else {
				$query .= " epr_value = NULL ,";				
			}
				if (isset($this->epr_exc_id) && trim($this->epr_exc_id) != "") {
					$query = $query . " epr_exc_id = ? , ";
					$types = $types . "i";
					$values[] = $this->epr_exc_id;
				} else {
					$query = $query . " epr_exc_id = NULL, ";
				}
				$query = $query . " epr_virgo_title = ? , ";
				$types = $types . "s";
				$values[] = $this->getVirgoTitle();

				$query = $query . " epr_date_modified = ? , ";
				$types = $types . "s";
				$values[] = $this->epr_date_modified;

				$query = $query . " epr_usr_modified_id = ? ";
				$types = $types . "i";
				$values[] = $this->epr_usr_modified_id;

				$query = $query . " WHERE epr_id = ? ";
				$types = $types . "i";
				$values[] = $this->epr_id;
			} else {
				$query = "INSERT INTO prt_execution_parameters ( ";
			$query = $query . " epr_name, ";
			$query = $query . " epr_value, ";
				$query = $query . " epr_exc_id, ";
				$query = $query . " epr_virgo_title, epr_date_created, epr_usr_created_id) VALUES ( ";
			if (isset($this->epr_name)) {
				$query .= " ? ,";
				$types .= "s";
				$values[] = $this->epr_name;
			...
			if (isset($this->epr_value)) {
				...
				$types = $types . "ssi";
				$values[] = $this->getVirgoTitle();
				$values[] = $this->epr_date_created;
				$values[] = $this->epr_usr_created_id;
			}
			...
			if (!$results) {
				...
				if ($log) {
					L("execution parameter stored successfully", "id = {$this->epr_id}", "TRACE");
				}
				return true;
			}
		}
		...
		function store($log = false) {
			$virgoOld = null;
			if ($this->epr_id) {
				$virgoOld = new virgoExecutionParameter($this->epr_id);
			}
			...
			if (!is_null($validationMessageText) && trim($validationMessageText) != "") {
				...
				if (!is_null($validationMessageText) && trim($validationMessageText) != "") {
					...
					if ($this->epr_id) {			
						$this->epr_date_modified = date("Y-m-d H:i:s");
						$this->epr_usr_modified_id = $userId;
					} else {
						$this->epr_date_created = date("Y-m-d H:i:s");
						$this->epr_usr_created_id = $userId;
					}
					$this->epr_virgo_title = $this->getVirgoTitle(); 
					if (!$this->parentStore(true, $log)) {
						...
						if(!$exists){
							...
							if (!$this->parentStore(true, $log)) {
								$error = $this->getError();
								$this->logFatal('Error storing "execution parameter" with id = ' . $this->getId() . ": " . $error);
		...
		function parentDelete() {
			$query = "DELETE FROM prt_execution_parameters WHERE epr_id = {$this->epr_id}";
			$results = Q($query);
			if (!$results) {
				L('Invalid query', $query, 'ERROR');
			}
		...
		static public function selectAllAsIdsStatic($where = '', $idsNotArrayOfIds = false) {
			$tmp = new virgoExecutionParameter();
			return $tmp->selectAllAsIds($where, $idsNotArrayOfIds);
		}
		
		public function selectAllAsIds($where = '', $idsNotArrayOfIds = false) {
			$query = "SELECT epr_id as id FROM prt_execution_parameters";
			if ($where != '') {
				$query = $query . " WHERE $where ";
			}
			$orderBy = "";
			if (property_exists($this, 'epr_order_column')) {
				$orderBy = " ORDER BY epr_order_column ASC ";
			} 
			if (property_exists($this, 'epr_kolejnosc_wyswietlania')) {
				$orderBy = " ORDER BY epr_kolejnosc_wyswietlania ASC ";
			} 
			$query = $query . $orderBy;
		...
		public function selectAllAsObjects($where = '') {
			...
			foreach ($results as $result) {
				$tmpObj = new virgoExecutionParameter($result['id']);
				$ret[] = $tmpObj;
			}
			return $ret;
		}
...
//				$id = $result->getId();
//				$query = " UPDATE prt_execution_parameters SET epr_virgo_title = '$title' WHERE epr_id = $id";
...
//			}
		...
		static function getByNameStatic($token) {
			$tmpStatic = new virgoExecutionParameter();
		...
		function getIdByName($token) {
			$res = $this->selectAll(" epr_name = ?", "", "s", array($token));
			foreach ($res as $r) {
				return $r['epr_id'];
			}
			return null;
		...
		function getIdByVirgoTitle($token) {
			$res = $this->selectAll(" epr_virgo_title = '" . $token . "'");
			foreach ($res as $r) {
				return $r['epr_id'];
			}
			return null;
		...
		private static function getSessionValue($namespace, $name, $default) {
			...
			virgoExecutionParameter::setSessionValue($namespace, $name, $default);
			return $default;
		}
		
		static function setRemoteSessionValue($name, $value, $menuItem) {
			virgoExecutionParameter::setSessionValue('Portal_ExecutionParameter-' . $menuItem, $name, $value);
		}
		
		static function getRemoteSessionValue($name, $default, $menuItem) {
			return virgoExecutionParameter::getSessionValue('Portal_ExecutionParameter-' . $menuItem, $name, $default);
		}
		
		static function setLocalSessionValue($name, $value) {
			virgoExecutionParameter::setRemoteSessionValue($name, $value, $_SESSION['current_portlet_object_id']);
		}
		
		static function getLocalSessionValue($name, $default) {
			return virgoExecutionParameter::getRemoteSessionValue($name, $default, $_SESSION['current_portlet_object_id']);
		}

		private static function setGlobalSessionValue($name, $value) {
			virgoExecutionParameter::setSessionValue('GLOBAL', $name, $value);
		}
		
		private static function getGlobalSessionValue($name, $default) {
			return virgoExecutionParameter::getSessionValue('GLOBAL', $name, $default);
		}
...
/*		static function isDebug() {
		...
		static function removeFromContext($pobId = null) {
			$context = self::getGlobalSessionValue('VIRGO_CONTEXT_usuniete', array());
			$context['epr_id'] = null;
			virgoExecutionParameter::setGlobalSessionValue('VIRGO_CONTEXT_usuniete', $context);
			if (is_null($pobId)) {
		...
		static function setRecordSet($criteria) {
			virgoExecutionParameter::setLocalSessionValue('VIRGO_RECORD_SET', $criteria);
		}
		
		static function getRecordSet() {
			return virgoExecutionParameter::getLocalSessionValue('VIRGO_RECORD_SET', array());
		}		
		
		static function setInvalidRecords($invalidRecords) {
			virgoExecutionParameter::setLocalSessionValue('VIRGO_INVALID_RECORDS', $invalidRecords);
		}
		
		static function getInvalidRecords() {
			return virgoExecutionParameter::getLocalSessionValue('VIRGO_INVALID_RECORDS', array());
		}
		
		static function setCriteria($criteria) {
			virgoExecutionParameter::setLocalSessionValue('VIRGO_CRITERIA', $criteria);
		}
		
		static function getCriteria() {
			if (P('filter_entity_pob_id', '') == '') {
				return virgoExecutionParameter::getLocalSessionValue('VIRGO_CRITERIA', null);
			} else {
				$filterPobId = P('filter_entity_pob_id', '');
				return virgoExecutionParameter::getRemoteSessionValue('VIRGO_CRITERIA', null, $filterPobId); 
			}
		}		

		static function setDisplayMode($mode) {
			virgoExecutionParameter::setLocalSessionValue('DisplayMode', $mode);
		}

		static function setRemoteDisplayMode($mode, $menuItem) { 
			virgoExecutionParameter::setRemoteSessionValue('DisplayMode', $mode, $menuItem);
		}
		...
		static function setContextId($id, $verifyToken = true, $pobId = null) {
			...
			virgoExecutionParameter::setRemoteContextId($id, (isset($pobId) ? $pobId : $_SESSION['current_portlet_object_id']));
		}
		
		static function getContextId() {
			return virgoExecutionParameter::getRemoteContextId($_SESSION['current_portlet_object_id']);
		}
		
		static function getRemoteContextId($menuItem) {
			return virgoExecutionParameter::getRemoteSessionValue('ContextId', null, $menuItem);
		}
		
		static function clearRemoteContextId($menuItem) {
			virgoExecutionParameter::setRemoteContextId(null, $menuItem);
		}
		
		static function setRemoteContextId($contextId, $menuItem) {
			virgoExecutionParameter::setRemoteSessionValue('ContextId', $contextId, $menuItem);
		}		
		...
		static function getDisplayMode() {
			...
			if (P('form_only', "0") == "1") {
				...
				return virgoExecutionParameter::getLocalSessionValue('DisplayMode', $defaultMode);
			}
		}
		
		static function setOrderColumn($column) {
			virgoExecutionParameter::setLocalSessionValue('OrderColumn', $column);
		}
		...
		static function getOrderColumn() {
			...
			if (is_null($defaultOrderColumn) || trim($defaultOrderColumn) == "") {
				$defaultOrderColumn = "epr_id";
			}
			return virgoExecutionParameter::getLocalSessionValue('OrderColumn', $defaultOrderColumn); 
		}		

		static function setOrderMode($mode) {
			virgoExecutionParameter::setLocalSessionValue('OrderMode', $mode);
		}
		...
		static function getOrderMode() {
			...
			return virgoExecutionParameter::getLocalSessionValue('OrderMode', $defaultOrderMode);
		}
		
		static function setShowRows($rows) {
			virgoExecutionParameter::setLocalSessionValue('ShowRows', $rows);
		}
...
/*			$systemParameter = new virgoSystemParameter();
			...
			return virgoExecutionParameter::getLocalSessionValue('ShowRows', $paramValue); 
		}
		
		static function setRemoteShowPage($param, $menuId) {
			virgoExecutionParameter::setRemoteSessionValue('ShowPage', $param, $menuId);
		}

		static function getRemoteShowPage($menuId) {
			return virgoExecutionParameter::getRemoteSessionValue('ShowPage', 1, $menuId); 
		}
		
		static function setShowPage($param) {
			virgoExecutionParameter::setLocalSessionValue('ShowPage', $param);
		}

		static function getShowPage() {
			return virgoExecutionParameter::getLocalSessionValue('ShowPage', 1); 
		}

		static function setRemoteImportFieldSeparator($param, $menuId) {
			virgoExecutionParameter::setRemoteSessionValue('ImportFieldSeparator', $param, $menuId);
		}

		static function getRemoteImportFieldSeparator($menuId) {
			return virgoExecutionParameter::getRemoteSessionValue('ImportFieldSeparator', null, $menuId); 
		}
		
		static function setImportFieldSeparator($param) {
			virgoExecutionParameter::setLocalSessionValue('ImportFieldSeparator', $param);
		}

		static function getImportFieldSeparator() {
			return virgoExecutionParameter::getLocalSessionValue('ImportFieldSeparator', null); 
		}
		...
		function internalActionStore($closeForm, $showOKMessage = true, $showError = true) {
			...
			if (isset($portletObject) && $portletObject->canExecute("$permissionToCheck")) {
				...
				if ($errorMessage == "") {
					if ($showOKMessage) {
						L(T('STORED_CORRECTLY', 'EXECUTION_PARAMETER'), '', 'INFO');
					}
					if ($closeForm) {
					...
					if ($creating && false) { //$componentParams->get('send_email') == "1") {
						...
						$fieldValues = '';						
						$fieldValues = $fieldValues . T($fieldValue, 'name', $this->epr_name);
						$fieldValues = $fieldValues . T($fieldValue, 'value', $this->epr_value);
						$parentExecution = new virgoExecution();
						$fieldValues = $fieldValues . T($fieldValue, 'execution', $parentExecution->lookup($this->epr_exc_id));
						$username = '';
						if ($this->epr_usr_created_id == 0) {
							$username = "anonymous";
						} else {
							$createdByUser =& JUser::getInstance((int)$this->epr_usr_created_id);
							$username = $createdByUser->username;
						}						
						$fieldValues = $fieldValues . T($fieldValue, 'User created', $username);
						$fieldValues = $fieldValues . T($fieldValue, 'Creation date', $this->epr_date_created);
		...
		static function portletActionStore($showOKMessage = true, &$id = null) {
			...
			if (is_null($oldId)) {
				if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoExecutionParameter'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
					$showOKMessage = false;
				}
			}
		...
		static function portletActionSelect($verifyToken = true, $pobId = null) {
			$tmpId = intval(R('epr_id_' . $_SESSION['current_portlet_object_id']));
		...
		static function portletActionSelectJson() {
			...
			return virgoExecutionParameter::getContextId();
		}
...
//			return $ret;
			...
			if (isset($portletObject) && $portletObject->canExecute("delete")) {
				...
				if ($res == "") {
					...
					if ($masterEntityPobId == "") { 						self::setDisplayMode("TABLE");
						virgoExecutionParameter::setRemoteContextId(null, $masterEntityPobId);
					}				
					L(T('DELETED_CORRECTLY', 'EXECUTION_PARAMETER'), '', 'INFO');
					return 0;
		...
		function getRecordsToEdit() {
			...
			foreach ($idsToEdit as $idToEdit) {
				...
				if (isset($idsToCorrect[$idToEdit])) {
					$resultExecutionParameter->loadRecordFromRequest($idToEdit);
				} else {
					$idToEditInt = (int)trim($idToEdit);
					if ($idToEditInt != 0) {
						$resultExecutionParameter->load($idToEditInt);
					} else {
						$resultExecutionParameter->epr_id = 0;
					}
				}
				$results[] = $resultExecutionParameter;
			}
			return $results;
		}
		...
		static function portletActionStoreSelected() {
			...
			foreach ($idsToStore as $idToStore) {
				$result = new virgoExecutionParameter();
				$result->loadFromRequest($idToStore);
				if ($result->epr_id != 0 || strtoupper($validateNew) == "Y") {
					if ($result->epr_id == 0) {
						$result->epr_id = null;
					}
					...
					if ($errorMessage != "") {
						$errors = $errors + 1;
						if (is_null($result->epr_id)) {
							$result->epr_id = 0;
						}
						$idsToCorrect[$result->epr_id] = $errorMessage;
					}
				}
			}
			if ($errors == 0) {
				if (sizeof($idsToStore) > 1 || strtoupper($validateNew) == "Y") {
					L(T('STORED_CORRECTLY', 'EXECUTION_PARAMETERS'), '', 'INFO');
				}
				self::setDisplayMode("TABLE");
				return 0;
			} else {
		...
		function multipleDelete($idsToDelete) {
			...
			foreach ($idsToDelete as $idToDelete) {
				$resultExecutionParameter->load((int)trim($idToDelete));
				$res = $resultExecutionParameter->delete();
			...
			if ($errorOcurred == 0) {
				L(T('DELETED_CORRECTLY', 'EXECUTION_PARAMETERS'), '', 'INFO');			
				self::setDisplayMode("TABLE");
				return 0;
			} else {
				return -1;
		...
		function getVirgoTitle() {
			$componentParams = null;
			$paramTitleSource = PD('title_value', 'virgoExecutionParameter');
		...
		function updateTitle() {
			...
			if (!is_null($val) && trim($val) != "") {
				$query = "UPDATE prt_execution_parameters SET epr_virgo_title = ? WHERE epr_id = " . $this->getId();
	...
	$rows = QR($query);
			foreach ($rows as $row) {
				$tmp = new virgoExecutionParameter($row['id']);
				$tmp->updateTitle();
			}
...
;				return true;
		...
		static function getParentsInContext() {
			if (!isset($_REQUEST['_virgo_parents_in_context_' . $_SESSION['current_portlet_object_id']])) {
				...
				foreach ($parentPobIds as $parentPobId) {
					...
					if (isset($parentInfo['value'])) {
						$parentInfo['condition'] = 'prt_execution_parameters.epr_' . $parentInfo['prefix'] . '_id = ' . $parentInfo['contextId'];
					} else {
						if ($whenNoParentSelected == "C") {
							$parentInfo['condition'] = 'prt_execution_parameters.epr_' . $parentInfo['prefix'] . '_id IS NULL';
							...
							foreach ($grandparentPobIds as $grandparentPobId) {
								...
								if (isset($class2prefix2[$grandparentInfo['className']])) {
									if (isset($grandparentInfo['value'])) {
										...
										$grandparentInfo['condition'] = 'prt_execution_parameters.epr_' . $parentInfo['prefix'] . '_id IN (SELECT ' . $parentInfo['prefix'] . '_id FROM prt_' . $tmp->getPlural() . ' WHERE ' . $parentInfo['prefix'] . '_' . $grandparentInfo['prefix'] . '_id = ' . $grandparentInfo['contextId'] . ') ';
									} else {
			...
			if (!isset($class2prefix[$className])) {
				L('Zle skonfigurowany komponent: jego rodzicem jest niby portlet o ID ' . $parentPobId . ' ale on pokazuje klase ' . $className . ', ktora NIE JEST rodzicem klasy virgoExecutionParameter!', '', 'ERROR');
				return array();
			}
		...
		static function portletActionReport() {
			...
			$user = virgoUser::getUser();
			$pdf->SetCreator(null);
			$pdf->SetTitle('Execution parameters report');
			$pdf->SetSubject(null);
			...
			if (is_null($reportTitle)) {
				$reportTitle = T('EXECUTION_PARAMETERS');
			}
		...
		$pdf->Ln();
			$criteriaExecutionParameter = $resultExecutionParameter->getCriteria();
			$fieldCriteriaName = $criteriaExecutionParameter["name"];
			if ($fieldCriteriaName["is_null"] == 1) {
				$pdf->SetFont($font, $fontBoldVariant, $fontSize + 2);
...
$filter = $filter . ' AND prt_execution_parameters.epr_value IS NULL ';
		...
		$resultsExecutionParameter = $resultExecutionParameter->select(
			'', 
			'all', 
			$resultExecutionParameter->getOrderColumn(), 
			$resultExecutionParameter->getOrderMode(), 
			$whereClauseExecutionParameter,
			$queryString);
		...
		foreach ($resultsExecutionParameter as $resultExecutionParameter) {
			...
			if (P('show_pdf_name', "1") != "0") {
			$tmpLen = $pdf->GetStringWidth(trim('' . $resultExecutionParameter['epr_name'])) + 6;
				if ($tmpLen > $minWidth['name']) {
					$minWidth['name'] = min($tmpLen, $maxWidth);
				}
			}
			...
			if (P('show_pdf_value', "0") != "0") {
			$tmpLen = $pdf->GetStringWidth(trim('' . $resultExecutionParameter['epr_value'])) + 6;
				if ($tmpLen > $minWidth['value']) {
					$minWidth['value'] = min($tmpLen, $maxWidth);
				}
			}
			if (P('show_pdf_execution', "1") == "1") {
			$parentValue = trim(virgoExecution::lookup($resultExecutionParameter['eprexc__id']));
		...
		$maxLn = 1;
//		$criteriaExecutionParameter = $resultExecutionParameter->getCriteria();
		if (is_null($criteriaExecutionParameter) || sizeof($criteriaExecutionParameter) == 0 || $countTmp == 0) {
		} else {
...
";
		...
		foreach ($resultsExecutionParameter as $resultExecutionParameter) {
			...
			if (P('show_pdf_execution', "1") == "1") {
			$parentValue = virgoExecution::lookup($resultExecutionParameter['epr_exc_id']);
			...
			if (P('show_pdf_name', "1") != "0") {
			$tmpLn = $pdf->MultiCell($minWidth['name'], $colHeight, '' . $resultExecutionParameter['epr_name'], 'T', 'L', 0, 0);
				if (P('show_pdf_name', "1") == "2") {
										if (!is_null($resultExecutionParameter['epr_name'])) {
				...
				if (P('show_pdf_name', "1") == "3") {
						...
						if (is_null($tmpSum)) {
							...
							$tmpSum = $tmpSum + $resultExecutionParameter['epr_name'];
						}
						$sums["name"] = $tmpSum;
					}
				}
			...
			if (P('show_pdf_value', "0") != "0") {
			$tmpLn = $pdf->MultiCell($minWidth['value'], $colHeight, '' . $resultExecutionParameter['epr_value'], 'T', 'L', 0, 0);
				if (P('show_pdf_value', "1") == "2") {
										if (!is_null($resultExecutionParameter['epr_value'])) {
				...
				if (P('show_pdf_value', "1") == "3") {
						...
						if (is_null($tmpSum)) {
							...
							$tmpSum = $tmpSum + $resultExecutionParameter['epr_value'];
						}
						$sums["value"] = $tmpSum;
					}
				}
...
//			$pdf->ln(50); //$maxLn * ($fontSize - 1) / 2);
		...
		static function portletActionExport() {
			...
			foreach ($resultsExecutionParameter as $resultExecutionParameter) {
				if (P('show_export_name', "1") != "0") {
			$data = $data . $stringDelimeter . $resultExecutionParameter['epr_name'] . $stringDelimeter . $separator;
				}
				if (P('show_export_value', "1") != "0") {
			$data = $data . $stringDelimeter . $resultExecutionParameter['epr_value'] . $stringDelimeter . $separator;
				}
				if (P('show_export_execution', "1") != "0") {
					$parentValue = virgoExecution::lookup($resultExecutionParameter['epr_exc_id']);
		...
		static function portletActionOffline() {
			...
			foreach ($resultsExecutionParameter as $resultExecutionParameter) {
				$index = $index + 1;
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $index, $resultExecutionParameter['epr_id']);
				$objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(0, $index)->getFont()->getColor()->applyFromArray(array("rgb" => 'BBBBBB'));
				$kolumna = 0;
				if (P('show_export_name', "1") == "1") {
					$kolumna = $kolumna + 1;
			$objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($kolumna, $index, $resultExecutionParameter['epr_name'], \PHPExcel_Cell_DataType::TYPE_STRING);
				...
				if (P('show_export_value', "1") == "1") {
					$kolumna = $kolumna + 1;
			$objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($kolumna, $index, $resultExecutionParameter['epr_value'], \PHPExcel_Cell_DataType::TYPE_STRING);
					$objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($kolumna, $index)->getProtection()->setLocked(\PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
				}
				if (P('show_export_execution', "1") == "1") {
					$kolumna = $kolumna + 1;
...
//				$user =& JFactory::getUser();
				if ( move_uploaded_file($tmp_src, $tmp_dest ) ) {
					...
					$propertyColumnHash['name'] = 'epr_name';
					$propertyColumnHash['name'] = 'epr_name';
					$propertyColumnHash['value'] = 'epr_value';
					$propertyColumnHash['value'] = 'epr_value';
					$propertyClassHash['execution'] = 'Execution';
					$propertyClassHash['execution'] = 'Execution';
					$propertyColumnHash['execution'] = 'epr_exc_id';
					$propertyColumnHash['execution'] = 'epr_exc_id';
					$importMode = P('import_mode', 'T');
					$recordsOK = 0;
					$recordsError = 0;
					while(!feof($fh)) {
						$importExecutionParameter = new virgoExecutionParameter();
						$line = fgets($fh);
							...
							foreach ($values as $value) {
								...
								if (isset($columns[$index]) && trim($columns[$index]) != "VIRGO_IGNORE") {
									...
									if (is_null($fieldName)) {
										L(T('PROPERTY_NOT_FOUND', T('EXECUTION_PARAMETER'), $columns[$index]), '', 'ERROR');
										return;
									} else {
										...
										$importExecutionParameter->$fieldName = $value;
									}
								}
								$index = $index + 1;
							}
...
if (isset($defaultValue) && trim($defaultValue) != "") {
	...
	$importExecutionParameter->setExcId($defaultValue);
}
							$errorMessage = $importExecutionParameter->store();
							if ($errorMessage != "") {
								if ($importMode == "T") {
									L($errorMessage, '', 'ERROR');
									fclose($fh);
		...
		static function portletActionVirgoSetExecution() {
			$this->loadFromDB();
			$parentId = R('epr_Execution_id_' . $_SESSION['current_portlet_object_id']);
		...
		static function portletActionStoreNewExecution() {
			...
			if (virgoExecution::portletActionStore(true, $id) == -1) {
				...
				$tmpId = self::loadIdFromRequest();
				$_POST['epr_execution_' . ($tmpId == 0 ? '' : $tmpId)] = $id;
		...
		static function createTable() {
			$query =  <<<SELECT
CREATE TABLE IF NOT EXISTS `prt_execution_parameters` (
  `epr_id` bigint(20) unsigned NOT NULL auto_increment,
  `epr_virgo_state` varchar(50) default NULL,
  `epr_virgo_title` varchar(255) default NULL,
	`epr_exc_id` int(11) default NULL,
  `epr_name` varchar(255), 
  `epr_value` longtext, 
  `epr_date_created` datetime NOT NULL,
  `epr_date_modified` datetime default NULL,
  `epr_usr_created_id` int(11) NOT NULL,
  `epr_usr_modified_id` int(11) default NULL,
  KEY `epr_exc_fk` (`epr_exc_id`),
  PRIMARY KEY  (`epr_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
/* 
	Example row to put in modules_project/initData/execution_parameter.sql 
INSERT INTO `prt_execution_parameters` (`epr_virgo_title`, `epr_name`, `epr_value`) 
VALUES (title, some_value, some_value);
*/
SELECT;
			if (!Q($query)) {
				L("Probably prt_execution_parameters table already exists.", '', 'FATAL');
		...
		static function getVirgoTableType() {
			...
			$rows = QPR($query, "ss", array($virgoConfig[VIRGO_PORTAL_INSTANCE]['database_name'], 'prt_execution_parameters'));
			foreach ($rows as $row) {
...
"2.0.0.0"  
		...
		static function checkCompatibility() {
			$virgoVersion = virgoExecutionParameter::getVirgoVersion();
