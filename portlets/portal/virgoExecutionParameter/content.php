<?php
	...
	use portal\virgoExecutionParameter;
...
//	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoExecution'.DIRECTORY_SEPARATOR.'controller.php')) require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoExecution'.DIRECTORY_SEPARATOR.'controller.php');
	$componentParams = null; //&JComponentHelper::getParams('com_prt_execution_parameter');
...
?>
<link rel="stylesheet" href="<?php echo $live_site ?>/components/com_prt_execution_parameter/portal.css" type="text/css" /> 
<?php
	}
?>
<?php
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoExecutionParameter'.DIRECTORY_SEPARATOR.'prt_epr.css')) {
?>
<link rel="stylesheet" href="<?php echo $_SESSION['portal_url'] ?>/portlets/portal/virgoExecutionParameter/prt_epr.css" type="text/css" /> 
<?php
	}
?>
<style>
...
<div class="virgo_container_portal virgo_container_entity_execution_parameter" style="border: none;">
...
//		$classNames["unique_visit"] = "virgoUniqueVisit";
		...
		$contextId = null;		
			$resultExecutionParameter = virgoExecutionParameter::createGuiAware();
			$contextId = $resultExecutionParameter->getContextId();
			if (isset($contextId)) {
				if (virgoExecutionParameter::getDisplayMode() != "CREATE" || R('portlet_action') == "Duplicate") {
					$resultExecutionParameter->load($contextId);
				}
			}
?>
<?php
...
<?php
	...
	if (isset($masterPobId)) {
		...
		if ($className == "virgoExecutionParameter") {
			$masterObject = new $className();
			$tmpId = $masterObject->getRemoteContextId($masterPobId);
			if (isset($tmpId)) {
				$resultExecutionParameter = new virgoExecutionParameter($tmpId);
				virgoExecutionParameter::setDisplayMode("FORM");
			} else {
				$resultExecutionParameter = new virgoExecutionParameter();
				virgoExecutionParameter::setDisplayMode("CREATE");
			}
		}
	} else {
		if (P('form_only', "0") == "5") {
			if (is_null($resultExecutionParameter->getId())) { 
				if (P('only_private_records', "0") == "1") {
					$allPrivateRecords = $resultExecutionParameter->selectAll();
					if (sizeof($allPrivateRecords) > 0) {
						$resultExecutionParameter = new virgoExecutionParameter($allPrivateRecords[0]['epr_id']);
						$resultExecutionParameter->putInContext(false);
					} else {
						$resultExecutionParameter = new virgoExecutionParameter();
					}
				} else {
					...
					if (isset($customSQL) && trim($customSQL) != '') {
						...
						$records = $resultExecutionParameter->selectAll($customSQL);
						if (sizeof($records) > 0) {
							$resultExecutionParameter = new virgoExecutionParameter($records[0]['epr_id']);
							$resultExecutionParameter->putInContext(false);
						} else {
							$resultExecutionParameter = new virgoExecutionParameter();
						}
			...
			$resultExecutionParameter = new virgoExecutionParameter(virgoUser::getUserId());
			$resultExecutionParameter->putInContext(false);
		}
	}
?>
...
?>
<?php
	$executionParameterDisplayMode = virgoExecutionParameter::getDisplayMode();
//	if ($executionParameterDisplayMode == "" || $executionParameterDisplayMode == "TABLE") {
//		$resultExecutionParameter = $resultExecutionParameter->portletActionForm();
//	}
?>
		<div class="form">
<?php
		$parentContextInfos = $resultExecutionParameter->getParentsInContext();
...
PROFILE('token');
	if (isset($resultExecutionParameter)) {
		$tmpId = is_array($resultExecutionParameter) ? $resultExecutionParameter['epr_id'] : $resultExecutionParameter->getId();
		if (isset($tmpId)) {
?>
...
?>
<?php
	if ($executionParameterDisplayMode != "TABLE") {
		...
		if (
			...
			$pob = $resultExecutionParameter->getMyPortletObject();
			...
			if (isset($invokedPortletId) && $invokedPortletId == $_SESSION['current_portlet_object_id'] && isset($reloadFromRequest) && $reloadFromRequest == "1") { 
				...
				$resultExecutionParameter->loadFromRequest();
			} else {
				if (P('form_only', "0") == "1" && isset($contextId)) {
					if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoExecutionParameter'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
						require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'portal'.DIRECTORY_SEPARATOR.'virgoExecutionParameter'.DIRECTORY_SEPARATOR.'create_store_message.php');
						$executionParameterDisplayMode = "-empty-";
					}
				}
			}
		}
	}
if (!$resultExecutionParameter->hideContentDueToNoParentSelected()) {
...
/* MILESTONE 1.1 Form */
	...
	if ($executionParameterDisplayMode == "FORM") {
?>
<script language="JavaScript">
  ...
  function confirmExit() {
	  ...
	  for (var i=0; i < checks.length; i++) {
		  ...
		  if (check.form.name == "virgo_form_execution_parameter") {
			  if (check.value == 'T') {
	...
	<div class="form_edit">
			<fieldset class="form">
				<legend>
<?php echo T('EXECUTION_PARAMETER') ?>:</legend>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label align="right" nowrap class="fieldlabel  obligatory " for="epr_execution_<?php echo isset($resultExecutionParameter->epr_id) ? $resultExecutionParameter->epr_id : '' ?>">
 *
...
//		$limit_execution = $componentParams->get('limit_to_execution');
		...
		$tmpId = portal\virgoExecutionParameter::getParentInContext("portal\\virgoExecution");
		$readOnly = "";
		if (isset($tmpId) || P('show_form_execution', "1") == "2") {
// to powinno bylo juz byc zrobione w createGuiAware()			
//			if (!is_null($tmpId)) {
//				$resultExecutionParameter->epr_exc__id = $tmpId;
//			}
			if (!is_null($resultExecutionParameter->getExcId())) {
				$parentId = $resultExecutionParameter->getExcId();
...
?>
						<input type="hidden" id="epr_execution_<?php echo $resultExecutionParameter->getId() ?>" name="epr_execution_<?php echo $resultExecutionParameter->getId() ?>" value="<?php echo $parentId ?>">
						<span class="inputbox readonly"><?php echo isset($parentValue) ? $parentValue : '' ?></span>
<?php
		} else {
?>
<?php
	$hint = TE('HINT_EXECUTION_PARAMETER_EXECUTION');
?>
<?php
	$whereList = "";
	...
	$showAjaxepr = P('show_form_execution', "1") == "3" || $parentCount > 100;
	if (!$showAjaxepr) {
?>
    						<select 
							class="inputbox  obligatory " 
							id="epr_execution_<?php echo !is_null($resultExecutionParameter->getId()) ? $resultExecutionParameter->getId() : '' ?>" 
							name="epr_execution_<?php echo !is_null($resultExecutionParameter->getId()) ? $resultExecutionParameter->getId() : '' ?>" 
<?php
	if (isset($hint)) {
?>
							title="<?php echo $hint ?>"
...
<?php 
				echo (!is_null($resultExecutionParameter->getExcId()) && $id == $resultExecutionParameter->getExcId() ? "selected='selected'" : "");
?>
							>
								<?php echo $label ?>
							</option>
...
?>
	...
	<input 
		...
		id="epr_execution_dropdown_<?php echo $resultExecutionParameter->getId() ?>" 
		autocomplete="off" 
...
$(function() {
        $( "#epr_execution_dropdown_<?php echo $resultExecutionParameter->getId() ?>" ).autocomplete({
            delay: 0,
            ...
            select: function( event, ui ) {
				if (ui.item.value != '') {
					$('#epr_execution_<?php echo $resultExecutionParameter->getId() ?>').val(ui.item.value);
				  	$('#epr_execution_dropdown_<?php echo $resultExecutionParameter->getId() ?>').val(ui.item.label);
				  	$('#epr_execution_dropdown_<?php echo $resultExecutionParameter->getId() ?>').addClass("locked");
				}
            ...
            open: function() {
				$('#epr_execution_<?php echo $resultExecutionParameter->getId() ?>').val('');
				$('#epr_execution_dropdown_<?php echo $resultExecutionParameter->getId() ?>').removeClass("locked");		
...
<div class="button_wrapper button_wrapper_addexecution inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'epr_id_<?php echo $this->getId() ?>', '<?php echo isset($resultExecutionParameter) ? (is_array($resultExecutionParameter) ? $resultExecutionParameter['epr_id'] : $resultExecutionParameter->getId()) : '' ?>');
...
?>						
<script type="text/javascript">
$('#epr_execution_dropdown_<?php echo $resultExecutionParameter->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["exc"])) {
		...
		$parentValue = $resultExecutionParameter->epr_exc_id;
	}
	
?>
				<input type="hidden" id="epr_execution_<?php echo $resultExecutionParameter->epr_id ?>" name="epr_execution_<?php echo $resultExecutionParameter->epr_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<?php
...
?>
					<label 
						...
						for="epr_name_<?php echo $resultExecutionParameter->getId() ?>"
					>* <?php echo T('NAME') ?>
</label>
<?php
	if (!$formsInTable) {
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultExecutionParameter->getName(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="name" name="epr_name_<?php echo $resultExecutionParameter->getId() ?>" value="<?php echo htmlentities($resultExecutionParameter->getName(), ENT_QUOTES, "UTF-8") ?>"> 
...
?>
					<label 
						...
						for="epr_value_<?php echo $resultExecutionParameter->getId() ?>"
...
<div 
	...
	id="value" 
><?php echo htmlentities($resultExecutionParameter->epr_value, ENT_QUOTES, "UTF-8") ?></div>
...
if (P('hide_audit', '0') == '0') {
	$showFieldset = 0;
	if ($resultExecutionParameter->getDateCreated()) {
		if ($resultExecutionParameter->getUsrCreatedId() == 0) {
			$username = "anonymous";
		} else {
			$createdByUser = new virgoUser((int)$resultExecutionParameter->getUsrCreatedId());
			$username = $createdByUser->getUsername();
		}
		$createdOn = $resultExecutionParameter->getDateCreated();
		$showFieldset = 1;
?>
		...
		<legend><?php echo T('AUDIT_DATA') ?>:</legend>
			<ul>
				<li>
					...
					<input type="hidden" id="usr_created_id" name="usr_created_id" value="<?php echo $resultExecutionParameter->getUsrCreatedId() ?>"	>
				</li>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('CREATION_DATE') ?></span>
					<span class="readonly"><i><?php echo $createdOn ?></i></span>
					<input type="hidden" id="date_created" name="date_created" value="<?php echo $resultExecutionParameter->getDateCreated() ?>"	>
				</li>
<?php
	}
?>
<?php
	if ($resultExecutionParameter->getDateModified()) {
		if ($resultExecutionParameter->getUsrModifiedId() == 0) {
			$username = "anonymous";
		} else {
			$modifiedByUser = new virgoUser((int)$resultExecutionParameter->getUsrModifiedId());
			$username = $modifiedByUser->getUsername();
		}
		$modifiedOn = $resultExecutionParameter->getDateModified();
?>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('MODIFIED_BY') ?></span>
					<span class="readonly"><b><?php echo $username ?></b></span>
					<input type="hidden" id="usr_modified_id" name="usr_modified_id" value="<?php echo $resultExecutionParameter->getUsrModifiedId() ?>"	>
				</li>
				<li>
					...
					<input type="hidden" id="date_modified" name="date_modified" value="<?php echo $resultExecutionParameter->getDateModified() ?>"	>
				</li>
<?php
	}
	if ($showFieldset == 1) {
...
?>
				...
				<div class="buttons form-actions">
						<input type="text" name="epr_id_<?php echo $this->getId() ?>" id="epr_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("EXECUTION_PARAMETER"), "\\'".rawurlencode($resultExecutionParameter->getVirgoTitle())."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
...
// document.forms['portlet_form_<?php echo $this->getId() ?>'].elements['epr_execution_<?php echo $resultExecutionParameter->getId() ?>'].focus();
</script>
	</div>
<?php
/* MILESTONE 1.2 Create */
	} elseif ($executionParameterDisplayMode == "CREATE") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
...
?>
<?php
		if (isset($resultExecutionParameter->epr_id)) {
			$resultExecutionParameter->epr_id = null;
		}
		$parentAjaxRendered = "0";
?>
<?php
...
if (isset($defaultValue) && trim($defaultValue) != "") {
	...
	$resultExecutionParameter->setExcId($defaultValue);
}
	}
?>
																										<?php
...
<?php
	...
	$parentCount = portal\virgoExecution::getVirgoListSize($whereList);
	$showAjaxepr = P('show_create_execution', "1") == "3" || $parentCount > 100;
	if (!$showAjaxepr) {
?>
    						<select 
							class="inputbox  obligatory " 
...
?>
					<label nowrap class="fieldlabel  obligatory " for="epr_name_<?php echo $resultExecutionParameter->getId() ?>">
* <?php echo T('NAME') ?>
					</label>
<?php
	if (!$formsInTable) {
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "name") {
				$resultExecutionParameter->setName($virgoSelectedDay);
			}
		}
	}
...
?>
					<label nowrap class="fieldlabel  <?php echo P('show_create_value_obligatory', "0") == "1" ? " obligatory " : "" ?> " for="epr_value_<?php echo $resultExecutionParameter->getId() ?>">
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "value") {
				$resultExecutionParameter->setValue($virgoSelectedDay);
			}
		}
	}
...
/* MILESTONE 1.3 Search */
	} elseif ($executionParameterDisplayMode == "SEARCH") {
?>
...
<?php
	if (P('show_search_execution', '1') == "1") {
		if (isset($criteriaExecutionParameter["execution"])) {
			$fieldCriteriaExecution = $criteriaExecutionParameter["execution"];
		}
?>
				<li
					<?php echo (!$formsInTable && $floatingFields) ? "style='display: inline-block; float: left;'" : '' ?>
...
?>
<?php
	unset($criteriaExecutionParameter);
?>
				</ul>
			</fieldset>
				<div class="buttons form-actions">
...
<?php
/* MILESTONE 1.4 View */
	} elseif ($executionParameterDisplayMode == "VIEW") {
?>
	<div class="form_view">
<?php
...
<?php
			...
			$readOnly = "";
			if ($resultExecutionParameter->getId() == 0) {
// przesuac do createforgui				$resultExecutionParameter->epr_exc__id = $tmpId;
			}
			$parentId = $resultExecutionParameter->getExecutionId();
			$parentValue = portal\virgoExecution::lookup($parentId);
				
?>
					<input
						type="hidden"
						id="epr_execution_<?php echo isset($resultExecutionParameter->epr_id) ? $resultExecutionParameter->epr_id : '' ?>" 
						name="epr_execution_<?php echo isset($resultExecutionParameter->epr_id) ? $resultExecutionParameter->epr_id : '' ?>" 						
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("EXECUTION_PARAMETER"), "\\'".$resultExecutionParameter->getVirgoTitle()."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'epr_id_<?php echo $this->getId() ?>', '<?php echo isset($resultExecutionParameter) ? (is_array($resultExecutionParameter) ? $resultExecutionParameter['epr_id'] : $resultExecutionParameter->getId()) : '' ?>');
...
<?php
/* MILESTONE 1.5 Table */
	} elseif ($executionParameterDisplayMode == "TABLE") {
PROFILE('TABLE');
		if (P('form_only') == "3") {
?>
<?php
	...
	$resultCount = -1;
	$filterApplied = false;
	$resultExecutionParameter->setShowPage(1); 
	$resultExecutionParameter->setShowRows('all'); 	
	$resultsExecutionParameter = $resultExecutionParameter->getTableData($resultCount, $filterApplied);
	$events = array();
	foreach ($resultsExecutionParameter as $resultExecutionParameter) {
		if (isset($resultExecutionParameter[$eventColumn]) && isset($events[substr($resultExecutionParameter[$eventColumn], 0, 10)])) {
			$eventsInDay = $events[substr($resultExecutionParameter[$eventColumn], 0, 10)];
		} else {
			$eventsInDay = array();
		}
		$eventObject = new virgoExecutionParameter($resultExecutionParameter['epr_id']);
		$eventsInDay[] = $eventObject;
		$events[substr($resultExecutionParameter[$eventColumn], 0, 10)] = $eventsInDay;
	}
?>
<style type="text/css">
...
?>			
			...
			<tr class='virgo_calendar_week'>
				...
				<input type='hidden' name='epr_id_<?php echo $this->getId() ?>' value=''/>
<?php
	$tmpWeekDay = 1;
	$safety = 0;
	if ($tmpDay["wday"] > 0) {
...
<?php
		...
		if (isset($events[$tmpStringDay])) {
			...
			foreach ($eventsInDay as $resultExecutionParameter) {
?>
<?php
PROFILE('token');
	if (isset($resultExecutionParameter)) {
...
<div class="button_wrapper button_wrapper_view inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'epr_id_<?php echo $this->getId() ?>', '<?php echo isset($resultExecutionParameter) ? (is_array($resultExecutionParameter) ? $resultExecutionParameter['epr_id'] : $resultExecutionParameter->getId()) : '' ?>');
 								...
 								form.target = '';
							" 
							value="<?php echo T($resultExecutionParameter->getVirgoTitle()) ?>"
						/><div class="button_right"></div></div><?php
	}
?>
<?php				
//				echo "<span class='virgo_calendar_event' onclick='var form=document.getElementById(\"portlet_form_".$this->getId()."\");form.portlet_action.value=\"View\";form.epr_id_".$this->getId().".value=\"".$eventInDay->getId()."\";form.submit();'>" . $eventInDay->getVirgoTitle() . "</span>";
			}
		}
		echo "</td>";
...
?>
		<script type="text/javascript">
			var executionParameterChildrenDivOpen = '';
			
			function childrenButtonClicked(clickedDivId) {
				var div = document.getElementById(clickedDivId);
				if (clickedDivId == executionParameterChildrenDivOpen) {
					div.style.display = 'none';
					executionParameterChildrenDivOpen = '';
				} else {
					if (executionParameterChildrenDivOpen != '') {
						document.getElementById(executionParameterChildrenDivOpen).style.display = 'none';
					}
					div.style.display = 'block';
					executionParameterChildrenDivOpen = clickedDivId;
				}
			}
...
PROFILE('table_01');
PROFILE('table_02');
			$showPage = $resultExecutionParameter->getShowPage(); 
			$showRows = $resultExecutionParameter->getShowRows(); 
?>
						<input type="text" name="epr_id_<?php echo $this->getId() ?>" id="epr_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>			
				<tr><td colspan="99" class="nav-header"><?php echo T('Execution parameters') ?></td></tr>
<?php
			}
?>			
<?php
PROFILE('table_02');
PROFILE('main select');
			$virgoOrderColumn = $resultExecutionParameter->getOrderColumn();
			$virgoOrderMode = $resultExecutionParameter->getOrderMode();
			$resultCount = -1;
			$filterApplied = false;
			$resultsExecutionParameter = $resultExecutionParameter->getTableData($resultCount, $filterApplied);
PROFILE('main select');
...
?>
<input name='epr_Execution_id_<?php echo $this->getId() ?>' id='epr_Execution_id_<?php echo $this->getId() ?>' type="hidden"/>
<?php		
	}
?>
				<th align="center" valign="middle" rowspan=" 2"><!-- bylo: nowrap -->
<?php
	$oc = $resultExecutionParameter->getOrderColumn(); 
	$om = $resultExecutionParameter->getOrderMode();
...
<?php
	if (P('show_table_filter', '0') == '1') {
		...
		if ($parentsCount < 51) {
			...
			$parentFilter = virgoExecutionParameter::getLocalSessionValue('VirgoFilterExecution', null);
?>
						<select 
...
<?php		
			...
			$parentFilter = virgoExecutionParameter::getLocalSessionValue('VirgoFilterTitleExecution', null);
?>
						<input
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'epr_name');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('NAME') ?>							<?php echo ($oc == "epr_name" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoExecutionParameter::getLocalSessionValue('VirgoFilterName', null);
?>
						<input
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'epr_value');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('VALUE') ?>							<?php echo ($oc == "epr_value" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoExecutionParameter::getLocalSessionValue('VirgoFilterValue', null);
?>
<?php		
	}
?>
...
<?php
			...
			if ($resultCount != 0) {
				if (((int)$showRows) * (((int)$showPage)-1) == $resultCount ) {
					...
					$resultExecutionParameter->setShowPage($showPage);
				}
				$index = 0;
PROFILE('table_04');
...
?>
<?php
$fileNameToInclude = PORTAL_PATH . "/portlets/portal/virgoExecutionParameter/modules/renderTableRow_{$_SESSION['current_portlet_object_id']}.php";
...
<?php
			if (P('form_only') != "4" || (P('chessboard_width') != 0 && $index % P('chessboard_width') == 1)) {
				if (is_null($firstRowId)) {
					$firstRowId = $resultExecutionParameter['epr_id'];
				}
				$displayClass = ' displayClass ';
				$tmpContextId = virgoExecutionParameter::getContextId();
				if (is_null($tmpContextId)) {
					$forceContextOnFirstRow = P('force_context_on_first_row', "1");
					if ($forceContextOnFirstRow == "1") {
						virgoExecutionParameter::setContextId($resultExecutionParameter['epr_id'], false);
						$tmpContextId = $resultExecutionParameter['epr_id'];
					}
				}
				if (isset($tmpContextId) && $resultExecutionParameter['epr_id'] == $tmpContextId) {
					if (P('form_only') != "4") {
...
?>
			<tr 
				id="<?php echo $this->getId() ?>_<?php echo isset($resultExecutionParameter['epr_id']) ? $resultExecutionParameter['epr_id'] : "" ?>" 
...
?>
				<input type="checkbox" class="checkbox" style="float: right;" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultExecutionParameter['epr_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
				<input type="checkbox" class="checkbox" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultExecutionParameter['epr_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
			<a  
				...
				onclick="
					...
					form.portlet_action.value='View'; 
					form.epr_id_<?php echo $this->getId() ?>.value='<?php echo isset($resultExecutionParameter['epr_id']) ? $resultExecutionParameter['epr_id'] : '' ?>'; 
					form.target = ''; 
<?php
					if (isset($ajax) && !$ajax) {
?>					
...
<?php 
	if (P('show_table_execution', "1") == "1") {
		if (isset($resultExecutionParameter['execution'])) {
			echo $resultExecutionParameter['execution'];
		}
	} else {
//		echo $resultExecutionParameter['epr_exc_id'];
?>
		<select class="inline" onchange="
this.form.portlet_action.value='VirgoSetExecution';
	_pSF(form, 'epr_id_<?php echo $this->getId() ?>', '<?php echo $resultExecutionParameter['epr_id'] ?>');
	_pSF(form, 'epr_Execution_id_<?php echo $this->getId() ?>', this.options[this.selectedIndex].value);
<?php
					if (isset($ajax) && !$ajax) {
?>					
						form.submit();
...
?>
			<option value="<?php echo $value ?>" <?php echo $value == $resultExecutionParameter['epr_exc_id'] ? " selected " : "" ?>><?php echo $label ?></option>
<?php			
		}
?>
		</select>
...
?>
		<span 
			class="<?php echo 'displayClass' ?>">
				<?php echo htmlentities($resultExecutionParameter['epr_name'], ENT_QUOTES, "UTF-8") ?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
?>
...
?>
		<?php
			$len = strlen($resultExecutionParameter['epr_value']);
			$shortTextSize = P('short_text_size_value', 30);
		?>
		<span 
			class="<?php echo 'displayClass' ?>" <?php print ($len > $shortTextSize ? 'title="' . htmlentities($resultExecutionParameter['epr_value'], ENT_QUOTES, "UTF-8") . '"' : '') ?>>
			<?php
				if ($shortTextSize > 0 && $len > $shortTextSize) {
					echo htmlentities(substr($resultExecutionParameter['epr_value'], 0, $shortTextSize), ENT_QUOTES, "UTF-8") . "...";
				} else {
					if ($shortTextSize == 0) {
						echo "<pre style='white-space: pre-line;'>";
					}
					echo htmlentities($resultExecutionParameter['epr_value'], ENT_QUOTES, "UTF-8");
					if ($shortTextSize == 0) {
						echo "</pre>";
					}
				}				 
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("EXECUTION_PARAMETER"), "\\'".rawurlencode($resultExecutionParameter['epr_virgo_title'])."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'epr_id_<?php echo $this->getId() ?>', '<?php echo isset($resultExecutionParameter) ? (is_array($resultExecutionParameter) ? $resultExecutionParameter['epr_id'] : $resultExecutionParameter->getId()) : '' ?>');
...
<?php
	...
	if (is_null($contextRowIdInTable)) {
		$forceContextOnFirstRow = P('force_context_on_first_row', "1");
		if ($forceContextOnFirstRow == "1") {
			virgoExecutionParameter::setContextId($firstRowId, false);
			if (P('form_only') != "4") {
?>
<script type="text/javascript">
...
<?php
				...
				unset($resultExecutionParameter);
				unset($resultsExecutionParameter);
				if (isset($contextIdOwn) && trim($contextIdOwn) != "") {
					if ($contextIdConfirmed == false) {
						$tmpExecutionParameter = new virgoExecutionParameter();
						$tmpCount = $tmpExecutionParameter->getAllRecordCount(' epr_id = ' . $contextIdOwn);
						if ($tmpCount == 0) {
							virgoExecutionParameter::clearRemoteContextId($tabModeEditMenu);
						}
					}
				}
PROFILE('rows rendering');
...
?>
		$.getJSON('<?php echo $page->getUrl() ?>?portlet_action=SelectJson&epr_id_<?php echo $this->getId() ?>=' + virgoId + '&invoked_portlet_object_id=<?php echo $this->getId() ?>&virgo_action_mode_json=T&_virgo_ajax=1', function(data) {
<?php
			if (P('form_only') != "4") {
?>
...
?>
		...
		form.epr_id_<?php echo $this->getId() ?>.value=virgoId; 
		form.target = ''; 
<?php
...
<div class="button_wrapper button_wrapper_report inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								...
								for(var i = 0; i< children.length;i++) {
								  if (children[i].getAttribute('name') == 'epr_id_<?php echo $this->getId() ?>') {
									found = 1;
								  }
								}
								if (found == 0) {
								  ...
								  input.setAttribute('name', 'epr_id_<?php echo $this->getId() ?>');
								  input.setAttribute('value', '');
								  form.appendChild(input);
								} else {
	_pSF(form, 'epr_id_<?php echo $this->getId() ?>', '');
								}
								form.action = '';
...
<?php
					$sessionSeparator = virgoExecutionParameter::getImportFieldSeparator();
...
<div class="button_wrapper button_wrapper_deleteselected inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								if (!nothingSelected(this.form, <?php echo $this->getId() ?>)) {
									if (!confirm(decodeURIComponent('<?php echo T('ARE_YOU_SURE_YOU_WANT_REMOVE', T('EXECUTION_PARAMETERS'), "") ?>'))) return false;
								} else {
									var alertText = '<?php echo T('NOTHING_SELECTED') ?>';
									if (alertText.charAt(alertText.length-1)=='?') {
										if (!confirm(alertText)) return false;
...
/* MILESTONE 1.6 TableForm */
	} elseif ($executionParameterDisplayMode == "TABLE_FORM") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
	...
	<form method="post" style="display: inline;" action="" id="virgo_form_execution_parameter" name="virgo_form_execution_parameter" enctype="multipart/form-data">
						<input type="text" name="epr_id_<?php echo $this->getId() ?>" id="epr_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
<?php			
				$resultsExecutionParameter = $resultExecutionParameter->getRecordsToEdit();
				$idsToCorrect = $resultExecutionParameter->getInvalidRecords();
				$index = 0;
PROFILE('rows rendering');
				foreach ($resultsExecutionParameter as $resultExecutionParameter) {
					$index = $index + 1;
...
<?php
	if ($resultExecutionParameter->epr_id == 0 && R('virgo_validate_new', "N") == "N") {
?>
		style="display: none;"
<?php
	}
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsExecutionParameter) * 0;
?>
<?php
	if (!isset($resultExecutionParameter)) {
		$resultExecutionParameter = new portal\virgoExecutionParameter();
...
?> 
						<input
							type="hidden"
							id="name_<?php echo $resultExecutionParameter->epr_id ?>" 
							name="name_<?php echo $resultExecutionParameter->epr_id ?>"
							value="<?php echo htmlentities($resultExecutionParameter->epr_name, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsExecutionParameter) * 1;
?>
<?php
	if (!isset($resultExecutionParameter)) {
		$resultExecutionParameter = new portal\virgoExecutionParameter();
...
?> 
						<input
							type="hidden"
							id="value_<?php echo $resultExecutionParameter->epr_id ?>" 
							name="value_<?php echo $resultExecutionParameter->epr_id ?>"
							value="<?php echo htmlentities($resultExecutionParameter->epr_value, ENT_QUOTES, "UTF-8") ?>"
						>
...
<?php
	$tabIndex = $index + sizeof($resultsExecutionParameter) * 2;
?>
<?php
//		$limit_execution = $componentParams->get('limit_to_execution');
		$limit_execution = null;
...
?>
				<td>
<?php
	if (isset($idsToCorrect[$resultExecutionParameter->epr_id])) {
		$errorMessage = $idsToCorrect[$resultExecutionParameter->epr_id];
?>
					<div class="error">
...
<?php
	} elseif ($executionParameterDisplayMode == "ADD_NEW_PARENT_EXECUTION") {
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "begin") {
				$resultExecutionParameter->setBegin($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultExecutionParameter->getBegin(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="begin" name="exc_begin_<?php echo $resultExecution->getId() ?>" value="<?php echo htmlentities($resultExecutionParameter->getBegin(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "progress") {
				$resultExecutionParameter->setProgress($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultExecutionParameter->getProgress(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="progress" name="exc_progress_<?php echo $resultExecution->getId() ?>" value="<?php echo htmlentities($resultExecutionParameter->getProgress(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "end") {
				$resultExecutionParameter->setEnd($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultExecutionParameter->getEnd(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="end" name="exc_end_<?php echo $resultExecution->getId() ?>" value="<?php echo htmlentities($resultExecutionParameter->getEnd(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "result") {
				$resultExecutionParameter->setResult($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultExecutionParameter->getResult(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="result" name="exc_result_<?php echo $resultExecution->getId() ?>" value="<?php echo htmlentities($resultExecutionParameter->getResult(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "statistics") {
				$resultExecutionParameter->setStatistics($virgoSelectedDay);
			}
		}
	}
...
>
	<?php echo $resultExecutionParameter->epr_statistics ?>
</div>
...
//		$limit_process = $componentParams->get('limit_to_process');
		$limit_process = null;
		$tmpId = portal\virgoExecutionParameter::getParentInContext("portal\\virgoProcess");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#epr_process_dropdown_<?php echo $resultExecutionParameter->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["prc"])) {
		...
		$parentValue = $resultExecutionParameter->epr_prc_id;
	}
	
?>
				<input type="hidden" id="epr_process_<?php echo $resultExecutionParameter->epr_id ?>" name="epr_process_<?php echo $resultExecutionParameter->epr_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
...
?>
 <div class="button_wrapper button_wrapper_backfromparent inlineBlock"><input 							name="submit_button" 
	...
	$childId = R('epr_id_' . $pobId);
?>
<input 
	type="hidden" 
	name="epr_id_<?php echo $pobId ?>" 
	value="<?php echo $childId ?>"
/>
<input 
	type="hidden" 
	name="epr_name_<?php echo $childId ?>" 
	value="<?php echo R('epr_name_' . $childId) ?>"
/>
<input 
	type="hidden" 
	name="epr_value_<?php echo $childId ?>" 
	value="<?php echo R('epr_value_' . $childId) ?>"
/>
<input 
	type="hidden" 
	name="epr_execution_<?php echo $childId ?>" 
	value="<?php echo R('epr_execution_' . $childId) ?>"
/>
<?php		
	} else {
		$virgoShowReturn = true;
?>
		<div class="<?php echo $executionParameterDisplayMode ?>">
<?php
?>
			<div class="buttons form-actions">
<?php
...
<div style="display: none; background-color:#FFFFFF; border:1px solid #000000; font-size:10px; margin:10px 0; padding:10px;"; id="extraFilesInfo_prt_execution_parameter" style="font-size: 12px; " onclick="document.getElementById('extraFilesInfo_prt_execution_parameter').style.display='none';">
...
<?php
	$infos = virgoExecutionParameter::getExtraFilesInfo();
