<?php
	...
	use sealock\virgoStatusZamowieniaWorkflow;
...
//	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoStatusZamowienia'.DIRECTORY_SEPARATOR.'controller.php')) require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoStatusZamowienia'.DIRECTORY_SEPARATOR.'controller.php');
	$componentParams = null; //&JComponentHelper::getParams('com_slk_status_zamowienia_workflow');
...
?>
<link rel="stylesheet" href="<?php echo $live_site ?>/components/com_slk_status_zamowienia_workflow/sealock.css" type="text/css" /> 
<?php
	}
?>
<?php
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoStatusZamowieniaWorkflow'.DIRECTORY_SEPARATOR.'slk_szw.css')) {
?>
<link rel="stylesheet" href="<?php echo $_SESSION['portal_url'] ?>/portlets/sealock/virgoStatusZamowieniaWorkflow/slk_szw.css" type="text/css" /> 
<?php
	}
?>
<style>
...
<div class="virgo_container_sealock virgo_container_entity_status_zamowienia_workflow" style="border: none;">
...
//		$classNames["pozycja_dokumentu"] = "virgoPozycjaDokumentu";
		...
		$ancestors = array();
		$contextId = null;		
			$resultStatusZamowieniaWorkflow = virgoStatusZamowieniaWorkflow::createGuiAware();
			$contextId = $resultStatusZamowieniaWorkflow->getContextId();
			if (isset($contextId)) {
				if (virgoStatusZamowieniaWorkflow::getDisplayMode() != "CREATE" || R('portlet_action') == "Duplicate") {
					$resultStatusZamowieniaWorkflow->load($contextId);
				}
			}
?>
<?php
...
<?php
	...
	if (isset($masterPobId)) {
		...
		if ($className == "virgoStatusZamowieniaWorkflow") {
			$masterObject = new $className();
			$tmpId = $masterObject->getRemoteContextId($masterPobId);
			if (isset($tmpId)) {
				$resultStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow($tmpId);
				virgoStatusZamowieniaWorkflow::setDisplayMode("FORM");
			} else {
				$resultStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow();
				virgoStatusZamowieniaWorkflow::setDisplayMode("CREATE");
			}
		}
	} else {
		if (P('form_only', "0") == "5") {
			if (is_null($resultStatusZamowieniaWorkflow->getId())) { 
				if (P('only_private_records', "0") == "1") {
					$allPrivateRecords = $resultStatusZamowieniaWorkflow->selectAll();
					if (sizeof($allPrivateRecords) > 0) {
						$resultStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow($allPrivateRecords[0]['szw_id']);
						$resultStatusZamowieniaWorkflow->putInContext(false);
					} else {
						$resultStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow();
					}
				} else {
					...
					if (isset($customSQL) && trim($customSQL) != '') {
						...
						$records = $resultStatusZamowieniaWorkflow->selectAll($customSQL);
						if (sizeof($records) > 0) {
							$resultStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow($records[0]['szw_id']);
							$resultStatusZamowieniaWorkflow->putInContext(false);
						} else {
							$resultStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow();
						}
			...
			$resultStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow(virgoUser::getUserId());
			$resultStatusZamowieniaWorkflow->putInContext(false);
		}
	}
?>
...
?>
<?php
	$statusZamowieniaWorkflowDisplayMode = virgoStatusZamowieniaWorkflow::getDisplayMode();
//	if ($statusZamowieniaWorkflowDisplayMode == "" || $statusZamowieniaWorkflowDisplayMode == "TABLE") {
//		$resultStatusZamowieniaWorkflow = $resultStatusZamowieniaWorkflow->portletActionForm();
//	}
?>
		<div class="form">
<?php
		$parentContextInfos = $resultStatusZamowieniaWorkflow->getParentsInContext();
...
?>
			<tr>
				<td class="db_criteria_label" nowrap="nowrap">
					<?php echo T('Status zamówienia') ?> <?php echo T('Next') ?>				</td>
				<td class="db_criteria_value">
					<?php echo T('EMPTY_VALUE') ?>				</td>
				<td class="db_criteria_value">
					<button class="button btn btn-mini" onclick="
						this.form.virgo_filter_column.value='status_zamowienianext';
...
?>
			<tr>
				<td class="db_criteria_label" nowrap="nowrap">
					<?php echo T('Status zamówienia') ?> <?php echo T('Prev') ?>				</td>
				<td class="db_criteria_value">
					<?php echo T('EMPTY_VALUE') ?>				</td>
				<td class="db_criteria_value">
					<button class="button btn btn-mini" onclick="
						this.form.virgo_filter_column.value='status_zamowieniaprev';
						this.form.portlet_action.value='RemoveCriterium';
						<?php echo JSFS() ?>
					">
<?php echo T('REMOVE_CRITERIA') ?>					</button>
...
<?php
PROFILE('token');
	if (isset($resultStatusZamowieniaWorkflow)) {
		$tmpId = is_array($resultStatusZamowieniaWorkflow) ? $resultStatusZamowieniaWorkflow['szw_id'] : $resultStatusZamowieniaWorkflow->getId();
		if (isset($tmpId)) {
?>
...
?>
<?php
	if ($statusZamowieniaWorkflowDisplayMode != "TABLE") {
		...
		if (
			...
			$pob = $resultStatusZamowieniaWorkflow->getMyPortletObject();
			...
			if (isset($invokedPortletId) && $invokedPortletId == $_SESSION['current_portlet_object_id'] && isset($reloadFromRequest) && $reloadFromRequest == "1") { 
				...
				$resultStatusZamowieniaWorkflow->loadFromRequest();
			} else {
				if (P('form_only', "0") == "1" && isset($contextId)) {
					if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoStatusZamowieniaWorkflow'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
						require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoStatusZamowieniaWorkflow'.DIRECTORY_SEPARATOR.'create_store_message.php');
						$statusZamowieniaWorkflowDisplayMode = "-empty-";
					}
				}
			}
		}
	}
if (!$resultStatusZamowieniaWorkflow->hideContentDueToNoParentSelected()) {
...
/* MILESTONE 1.1 Form */
	...
	if ($statusZamowieniaWorkflowDisplayMode == "FORM") {
?>
<script language="JavaScript">
  ...
  function confirmExit() {
	  ...
	  for (var i=0; i < checks.length; i++) {
		  ...
		  if (check.form.name == "virgo_form_status_zamowienia_workflow") {
			  if (check.value == 'T') {
	...
	<div class="form_edit">
			<fieldset class="form">
				<legend>
<?php echo T('STATUS_ZAMOWIENIA_WORKFLOW') ?>:</legend>
<?php
	if (!$formsInTable) {
?>
...
?>
																																			<?php
	if (class_exists('sealock\virgoStatusZamowienia') && ((P('show_form_status_zamowienianext', "1") == "1" || P('show_form_status_zamowienianext', "1") == "2" || P('show_form_status_zamowienianext', "1") == "3"))) {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label align="right" nowrap class="fieldlabel  obligatory " for="szw_statusZamowieniaNext_<?php echo isset($resultStatusZamowieniaWorkflow->szw_id) ? $resultStatusZamowieniaWorkflow->szw_id : '' ?>">
 *
<?php $customLabel = TE('STATUS_ZAMOWIENIA_NEXT'); echo !is_null($customLabel) ? $customLabel : T('STATUS_ZAMOWIENIA') . ' ' . T('NEXT') ?> 
					</label>
<?php
	if (!$formsInTable) {
?>
...
?>
<?php
//		$limit_status_zamowienianext = $componentParams->get('limit_to_status_zamowienianext');
		$limit_status_zamowienianext = null;
		$readOnly = "";
		if (isset($tmpId) || P('show_form_status_zamowienianext', "1") == "2") {
// to powinno bylo juz byc zrobione w createGuiAware()			
//			if (!is_null($tmpId)) {
//				$resultStatusZamowieniaWorkflow->szw_szm_next_id = $tmpId;
//			}
			if (!is_null($resultStatusZamowieniaWorkflow->getSzmNextId())) {
				$parentId = $resultStatusZamowieniaWorkflow->getSzmNextId();
...
?>
						<input type="hidden" id="szw_statusZamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" name="szw_statusZamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" value="<?php echo $parentId ?>">
						<span class="inputbox readonly"><?php echo isset($parentValue) ? $parentValue : '' ?></span>
<?php
		} else {
?>
<?php
	$hint = TE('HINT_STATUS_ZAMOWIENIA_WORKFLOW_NEXTSTATUS_ZAMOWIENIA');
?>
<?php
	$whereList = "";
	if (!is_null($limit_status_zamowienianext) && trim($limit_status_zamowienianext) != "") {
		$whereList = $whereList . " szm_id ";
		if (trim($limit_status_zamowienianext) == "page_title") {
			$limit_status_zamowienianext = "SELECT szm_id FROM slk_statusy_zamowien WHERE szm_" . $limit_status_zamowienianext . " = '" . $mainframe->getPageTitle() . "'";
		}
		eval("\$limit_status_zamowienianext = \"$limit_status_zamowienianext\";");
		$whereList = $whereList . " IN (" . $limit_status_zamowienianext . ") ";
	}						
	$parentCount = sealock\virgoStatusZamowienia::getVirgoListSize($whereList);
	$showAjaxszw = P('show_form_status_zamowienianext', "1") == "3" || $parentCount > 100;
	if (!$showAjaxszw) {
?>
    						<select 
							class="inputbox  obligatory " 
							id="szw_statusZamowieniaNext_<?php echo !is_null($resultStatusZamowieniaWorkflow->getId()) ? $resultStatusZamowieniaWorkflow->getId() : '' ?>" 
							name="szw_statusZamowieniaNext_<?php echo !is_null($resultStatusZamowieniaWorkflow->getId()) ? $resultStatusZamowieniaWorkflow->getId() : '' ?>" 
<?php
	if (isset($hint)) {
?>
							title="<?php echo $hint ?>"
...
<?php 
				echo (!is_null($resultStatusZamowieniaWorkflow->getSzmNextId()) && $id == $resultStatusZamowieniaWorkflow->getSzmNextId() ? "selected='selected'" : "");
?>
							>
								<?php echo $label ?>
							</option>
...
?>
	<input type="hidden" id="szw_status_zamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" name="szw_statusZamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" value="<?php echo $parentId ?>"/> 
	<input 
		type="text"
		class="inputbox dropdown <?php echo !is_null($parentId) && trim($parentId) != '' && $parentId != '0' ? 'locked' : '' ?>  obligatory " 
		id="szw_status_zamowieniaNext_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" 
		autocomplete="off" 
...
$(function() {
        $( "#szw_status_zamowieniaNext_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" ).autocomplete({
            delay: 0,
            ...
            select: function( event, ui ) {
				if (ui.item.value != '') {
					$('#szw_status_zamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').val(ui.item.value);
				  	$('#szw_status_zamowieniaNext_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').val(ui.item.label);
				  	$('#szw_status_zamowieniaNext_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').addClass("locked");
				}
            ...
            open: function() {
				$('#szw_status_zamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').val('');
				$('#szw_status_zamowieniaNext_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').removeClass("locked");		
...
?>						
<script type="text/javascript">
$('#szw_status_zamowienia_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["szm"])) {
		...
		$parentValue = $resultStatusZamowieniaWorkflow->szw_szm_next_id;
	}
	
?>
				<input type="hidden" id="szw_statusZamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->szw_id ?>" name="szw_statusZamowieniaNext_<?php echo $resultStatusZamowieniaWorkflow->szw_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<?php
	if (class_exists('sealock\virgoStatusZamowienia') && ((P('show_form_status_zamowieniaprev', "1") == "1" || P('show_form_status_zamowieniaprev', "1") == "2" || P('show_form_status_zamowieniaprev', "1") == "3"))) {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label align="right" nowrap class="fieldlabel  obligatory " for="szw_statusZamowieniaPrev_<?php echo isset($resultStatusZamowieniaWorkflow->szw_id) ? $resultStatusZamowieniaWorkflow->szw_id : '' ?>">
 *
<?php $customLabel = TE('STATUS_ZAMOWIENIA_PREV'); echo !is_null($customLabel) ? $customLabel : T('STATUS_ZAMOWIENIA') . ' ' . T('PREV') ?> 
					</label>
<?php
	if (!$formsInTable) {
?>
...
?>
<?php
//		$limit_status_zamowieniaprev = $componentParams->get('limit_to_status_zamowieniaprev');
		$limit_status_zamowieniaprev = null;
		$readOnly = "";
		if (isset($tmpId) || P('show_form_status_zamowieniaprev', "1") == "2") {
// to powinno bylo juz byc zrobione w createGuiAware()			
//			if (!is_null($tmpId)) {
//				$resultStatusZamowieniaWorkflow->szw_szm_prev_id = $tmpId;
//			}
			if (!is_null($resultStatusZamowieniaWorkflow->getSzmPrevId())) {
				$parentId = $resultStatusZamowieniaWorkflow->getSzmPrevId();
				$parentValue = sealock\virgoStatusZamowienia::lookup($parentId);
			} else {
				$parentId = null;
				$parentValue = null;
...
?>
						<input type="hidden" id="szw_statusZamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" name="szw_statusZamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" value="<?php echo $parentId ?>">
						<span class="inputbox readonly"><?php echo isset($parentValue) ? $parentValue : '' ?></span>
<?php
		} else {
?>
<?php
	$hint = TE('HINT_STATUS_ZAMOWIENIA_WORKFLOW_PREVSTATUS_ZAMOWIENIA');
?>
<?php
	$whereList = "";
	if (!is_null($limit_status_zamowieniaprev) && trim($limit_status_zamowieniaprev) != "") {
		$whereList = $whereList . " szm_id ";
		if (trim($limit_status_zamowieniaprev) == "page_title") {
			$limit_status_zamowieniaprev = "SELECT szm_id FROM slk_statusy_zamowien WHERE szm_" . $limit_status_zamowieniaprev . " = '" . $mainframe->getPageTitle() . "'";
		}
		eval("\$limit_status_zamowieniaprev = \"$limit_status_zamowieniaprev\";");
		$whereList = $whereList . " IN (" . $limit_status_zamowieniaprev . ") ";
	}						
	$parentCount = sealock\virgoStatusZamowienia::getVirgoListSize($whereList);
	$showAjaxszw = P('show_form_status_zamowieniaprev', "1") == "3" || $parentCount > 100;
	if (!$showAjaxszw) {
?>
    						<select 
							class="inputbox  obligatory " 
							id="szw_statusZamowieniaPrev_<?php echo !is_null($resultStatusZamowieniaWorkflow->getId()) ? $resultStatusZamowieniaWorkflow->getId() : '' ?>" 
							name="szw_statusZamowieniaPrev_<?php echo !is_null($resultStatusZamowieniaWorkflow->getId()) ? $resultStatusZamowieniaWorkflow->getId() : '' ?>" 
<?php
	if (isset($hint)) {
?>
							title="<?php echo $hint ?>"
...
?>	
							<option value="<?php echo $id ?>" 
<?php 
				echo (!is_null($resultStatusZamowieniaWorkflow->getSzmPrevId()) && $id == $resultStatusZamowieniaWorkflow->getSzmPrevId() ? "selected='selected'" : "");
?>
							>
								<?php echo $label ?>
							</option>
...
?>
	<input type="hidden" id="szw_status_zamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" name="szw_statusZamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" value="<?php echo $parentId ?>"/> 
	<input 
		type="text"
		class="inputbox dropdown <?php echo !is_null($parentId) && trim($parentId) != '' && $parentId != '0' ? 'locked' : '' ?>  obligatory " 
		id="szw_status_zamowieniaPrev_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" 
		autocomplete="off" 
		value="<?php echo $parentValue ?>" 
<?php
	if (isset($hint)) {
...
<script type="text/javascript">
$(function() {
        $( "#szw_status_zamowieniaPrev_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>" ).autocomplete({
            delay: 0,
            source: function( request, response ) {
                $.ajax({
                    url: "?",
            ...
            select: function( event, ui ) {
				if (ui.item.value != '') {
					$('#szw_status_zamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').val(ui.item.value);
				  	$('#szw_status_zamowieniaPrev_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').val(ui.item.label);
				  	$('#szw_status_zamowieniaPrev_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').addClass("locked");
				}
				event.stopImmediatePropagation();
				event.preventDefault();
            },
            ...
            open: function() {
				$('#szw_status_zamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').val('');
				$('#szw_status_zamowieniaPrev_dropdown_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>').removeClass("locked");		
                $( this ).removeClass( "ui-corner-all" ).addClass( "ui-corner-top" );
            },
            close: function() {
                $( this ).removeClass( "ui-corner-top" ).addClass( "ui-corner-all" );
...
<?php
	if (isset($context["szm"])) {
		...
		$parentValue = $resultStatusZamowieniaWorkflow->szw_szm_prev_id;
	}
	
?>
				<input type="hidden" id="szw_statusZamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->szw_id ?>" name="szw_statusZamowieniaPrev_<?php echo $resultStatusZamowieniaWorkflow->szw_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
...
if (P('hide_audit', '0') == '0') {
	$showFieldset = 0;
	if ($resultStatusZamowieniaWorkflow->getDateCreated()) {
		if ($resultStatusZamowieniaWorkflow->getUsrCreatedId() == 0) {
			$username = "anonymous";
		} else {
			$createdByUser = new virgoUser((int)$resultStatusZamowieniaWorkflow->getUsrCreatedId());
			$username = $createdByUser->getUsername();
		}
		$createdOn = $resultStatusZamowieniaWorkflow->getDateCreated();
		$showFieldset = 1;
?>
		...
		<legend><?php echo T('AUDIT_DATA') ?>:</legend>
			<ul>
				<li>
					...
					<input type="hidden" id="usr_created_id" name="usr_created_id" value="<?php echo $resultStatusZamowieniaWorkflow->getUsrCreatedId() ?>"	>
				</li>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('CREATION_DATE') ?></span>
					<span class="readonly"><i><?php echo $createdOn ?></i></span>
					<input type="hidden" id="date_created" name="date_created" value="<?php echo $resultStatusZamowieniaWorkflow->getDateCreated() ?>"	>
				</li>
<?php
	}
?>
<?php
	if ($resultStatusZamowieniaWorkflow->getDateModified()) {
		if ($resultStatusZamowieniaWorkflow->getUsrModifiedId() == 0) {
			$username = "anonymous";
		} else {
			$modifiedByUser = new virgoUser((int)$resultStatusZamowieniaWorkflow->getUsrModifiedId());
			$username = $modifiedByUser->getUsername();
		}
		$modifiedOn = $resultStatusZamowieniaWorkflow->getDateModified();
?>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('MODIFIED_BY') ?></span>
					<span class="readonly"><b><?php echo $username ?></b></span>
					<input type="hidden" id="usr_modified_id" name="usr_modified_id" value="<?php echo $resultStatusZamowieniaWorkflow->getUsrModifiedId() ?>"	>
				</li>
				<li>
					...
					<input type="hidden" id="date_modified" name="date_modified" value="<?php echo $resultStatusZamowieniaWorkflow->getDateModified() ?>"	>
				</li>
<?php
	}
	if ($showFieldset == 1) {
...
?>
				...
				<div class="buttons form-actions">
						<input type="text" name="szw_id_<?php echo $this->getId() ?>" id="szw_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
 ...
 <div class="button_wrapper button_wrapper_store inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'szw_id_<?php echo $this->getId() ?>', '<?php echo isset($resultStatusZamowieniaWorkflow) ? (is_array($resultStatusZamowieniaWorkflow) ? $resultStatusZamowieniaWorkflow['szw_id'] : $resultStatusZamowieniaWorkflow->getId()) : '' ?>');
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("STATUS_ZAMOWIENIA_WORKFLOW"), "\\'".rawurlencode($resultStatusZamowieniaWorkflow->getVirgoTitle())."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
...
// document.forms['portlet_form_<?php echo $this->getId() ?>'].elements['szw_status_zamowienia_<?php echo $resultStatusZamowieniaWorkflow->getId() ?>'].focus();
</script>
	</div>
<?php
/* MILESTONE 1.2 Create */
	} elseif ($statusZamowieniaWorkflowDisplayMode == "CREATE") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
...
?>
<?php
		if (isset($resultStatusZamowieniaWorkflow->szw_id)) {
			$resultStatusZamowieniaWorkflow->szw_id = null;
		}
		$parentAjaxRendered = "0";
?>
<?php
...
$defaultValue = P('create_default_value_status_zamowienianext');
if (isset($defaultValue) && trim($defaultValue) != "") {
	...
	$resultStatusZamowieniaWorkflow->setSzmNextId($defaultValue);
}
$defaultValue = P('create_default_value_status_zamowieniaprev');
if (isset($defaultValue) && trim($defaultValue) != "") {
	if ($defaultValue == -1) {
		$loggedInUserId = virgoUser::getUserId();
		$parentCreatedByLoggedInUser = virgoStatusZamowienia::getRecordCreatedBy($loggedInUserId);
	...
	$resultStatusZamowieniaWorkflow->setSzmPrevId($defaultValue);
}
	}
?>
																																			<?php
	if (class_exists('sealock\virgoStatusZamowienia') && ((P('show_create_status_zamowienianext', "1") == "1" || P('show_create_status_zamowienianext', "1") == "2" || P('show_create_status_zamowienianext', "1") == "3"))) {
?>
<?php
	if (!$formsInTable) {
?>
...
//		$limit_status_zamowienianext = $componentParams->get('limit_to_status_zamowienianext');
		$limit_status_zamowienianext = null;
		$readOnly = "";
		if (isset($tmpId) || P('show_create_status_zamowienianext', "1") == "2") {
// to powinno bylo juz byc zrobione w createGuiAware()			
//			if (!is_null($tmpId)) {
//				$resultStatusZamowieniaWorkflow->szw_szm_next_id = $tmpId;
//			}
...
<?php
	...
	$parentCount = sealock\virgoStatusZamowienia::getVirgoListSize($whereList);
	$showAjaxszw = P('show_create_status_zamowienianext', "1") == "3" || $parentCount > 100;
	if (!$showAjaxszw) {
?>
    						<select 
							class="inputbox  obligatory " 
...
?>
<?php
	if (class_exists('sealock\virgoStatusZamowienia') && ((P('show_create_status_zamowieniaprev', "1") == "1" || P('show_create_status_zamowieniaprev', "1") == "2" || P('show_create_status_zamowieniaprev', "1") == "3"))) {
?>
<?php
	if (!$formsInTable) {
?>
...
//		$limit_status_zamowieniaprev = $componentParams->get('limit_to_status_zamowieniaprev');
		$limit_status_zamowieniaprev = null;
		$readOnly = "";
		if (isset($tmpId) || P('show_create_status_zamowieniaprev', "1") == "2") {
// to powinno bylo juz byc zrobione w createGuiAware()			
//			if (!is_null($tmpId)) {
//				$resultStatusZamowieniaWorkflow->szw_szm_prev_id = $tmpId;
//			}
...
<?php
	...
	$parentCount = sealock\virgoStatusZamowienia::getVirgoListSize($whereList);
	$showAjaxszw = P('show_create_status_zamowieniaprev', "1") == "3" || $parentCount > 100;
	if (!$showAjaxszw) {
?>
    						<select 
							class="inputbox  obligatory " 
...
/* MILESTONE 1.3 Search */
	} elseif ($statusZamowieniaWorkflowDisplayMode == "SEARCH") {
?>
...
?>	
<?php
	if (P('show_search_status_zamowienianext', '1') == "1") {
		if (isset($criteriaStatusZamowieniaWorkflow["status_zamowienianext"])) {
			$fieldCriteriaStatusZamowieniaNext = $criteriaStatusZamowieniaWorkflow["status_zamowienianext"];
		}
?>
				<li
				...
				><?php echo T('STATUS_ZAMOWIENIA') ?> <?php echo T('NEXT') ?></label>
<?php
	$ids = (isset($fieldCriteriaStatusZamowieniaNext["ids"]) ? $fieldCriteriaStatusZamowieniaNext["ids"] : array());
	$resultsStatusZamowienia = sealock\virgoStatusZamowienia::getVirgoList();
	$maxListboxSize = 10;
?>
    <select class="inputbox " id="virgo_search_statusZamowieniaNext[]" name="virgo_search_statusZamowieniaNext[]" multiple
<?php
...
?>
			...
			>
				<option value=""
<?php
		if (isset($fieldCriteriaStatusZamowieniaNext) && $fieldCriteriaStatusZamowieniaNext["is_null"] == 0) {
?>
				selected="selected"
<?php
		}
...
<?php
		if (isset($fieldCriteriaStatusZamowieniaNext) && $fieldCriteriaStatusZamowieniaNext["is_null"] == 1) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="null"
<?php
		if (isset($fieldCriteriaStatusZamowieniaNext) && $fieldCriteriaStatusZamowieniaNext["is_null"] == 2) {
?>
				selected="selected"
<?php
		}
...
?>
<?php
	if (P('show_search_status_zamowieniaprev', '1') == "1") {
		if (isset($criteriaStatusZamowieniaWorkflow["status_zamowieniaprev"])) {
			$fieldCriteriaStatusZamowieniaPrev = $criteriaStatusZamowieniaWorkflow["status_zamowieniaprev"];
		}
?>
				<li
					<?php echo (!$formsInTable && $floatingFields) ? "style='display: inline-block; float: left;'" : '' ?>
				...
				<label align="right" nowrap class="fieldlabel"
					<?php echo (!$formsInTable && $floatingFields) ? "style='width: inherit;'" : '' ?>				
				><?php echo T('STATUS_ZAMOWIENIA') ?> <?php echo T('PREV') ?></label>
<?php
	$ids = (isset($fieldCriteriaStatusZamowieniaPrev["ids"]) ? $fieldCriteriaStatusZamowieniaPrev["ids"] : array());
	$resultsStatusZamowienia = sealock\virgoStatusZamowienia::getVirgoList();
	$maxListboxSize = 10;
?>
    <select class="inputbox " id="virgo_search_statusZamowieniaPrev[]" name="virgo_search_statusZamowieniaPrev[]" multiple
<?php
	if (sizeof($resultsStatusZamowienia) > $maxListboxSize) {
		echo "size=" . $maxListboxSize;
	}
...
?>
			...
			>
				<option value=""
<?php
		if (isset($fieldCriteriaStatusZamowieniaPrev) && $fieldCriteriaStatusZamowieniaPrev["is_null"] == 0) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="not_null"
<?php
		if (isset($fieldCriteriaStatusZamowieniaPrev) && $fieldCriteriaStatusZamowieniaPrev["is_null"] == 1) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="null"
<?php
		if (isset($fieldCriteriaStatusZamowieniaPrev) && $fieldCriteriaStatusZamowieniaPrev["is_null"] == 2) {
?>
				selected="selected"
<?php
		}
...
?>
<?php
	unset($criteriaStatusZamowieniaWorkflow);
?>
				</ul>
			</fieldset>
				<div class="buttons form-actions">
...
<?php
/* MILESTONE 1.4 View */
	} elseif ($statusZamowieniaWorkflowDisplayMode == "VIEW") {
?>
	<div class="form_view">
<?php
...
?>
																																			<?php
	if (class_exists('sealock\virgoStatusZamowienia') && P('show_view_status_zamowienianext', '1') == "1") { // && (isset($masterComponentName) && $masterComponentName != "status_zamowienia" || is_null($contextId))) {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<span align="right" nowrap class="fieldlabel">Status zamówienia Next</span>
<?php
	if (!$formsInTable) {
?>
<?php
...
<?php
			...
			$readOnly = "";
			if ($resultStatusZamowieniaWorkflow->getId() == 0) {
// przesuac do createforgui				$resultStatusZamowieniaWorkflow->szw_szm_next_id = $tmpId;
			}
			$parentId = $resultStatusZamowieniaWorkflow->getStatusZamowieniaNextId();
			$parentValue = sealock\virgoStatusZamowienia::lookup($parentId);
				
?>
					<input
						type="hidden"
						id="szw_statusZamowieniaNext_<?php echo isset($resultStatusZamowieniaWorkflow->szw_id) ? $resultStatusZamowieniaWorkflow->szw_id : '' ?>" 
						name="szw_statusZamowieniaNext_<?php echo isset($resultStatusZamowieniaWorkflow->szw_id) ? $resultStatusZamowieniaWorkflow->szw_id : '' ?>" 						
...
?>
<?php
	if (class_exists('sealock\virgoStatusZamowienia') && P('show_view_status_zamowieniaprev', '1') == "1") { // && (isset($masterComponentName) && $masterComponentName != "status_zamowienia" || is_null($contextId))) {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<span align="right" nowrap class="fieldlabel">Status zamówienia Prev</span>
<?php
	if (!$formsInTable) {
?>
<?php
...
<?php
			...
			$readOnly = "";
			if ($resultStatusZamowieniaWorkflow->getId() == 0) {
// przesuac do createforgui				$resultStatusZamowieniaWorkflow->szw_szm_prev_id = $tmpId;
			}
			$parentId = $resultStatusZamowieniaWorkflow->getStatusZamowieniaPrevId();
			$parentValue = sealock\virgoStatusZamowienia::lookup($parentId);
				
?>
					<input
						type="hidden"
						id="szw_statusZamowieniaPrev_<?php echo isset($resultStatusZamowieniaWorkflow->szw_id) ? $resultStatusZamowieniaWorkflow->szw_id : '' ?>" 
						name="szw_statusZamowieniaPrev_<?php echo isset($resultStatusZamowieniaWorkflow->szw_id) ? $resultStatusZamowieniaWorkflow->szw_id : '' ?>" 						
						value="<?php echo $parentId ?>"
					/>
					<span class="inputbox readonly"><?php echo $parentValue ?></span>
<?php
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("STATUS_ZAMOWIENIA_WORKFLOW"), "\\'".$resultStatusZamowieniaWorkflow->getVirgoTitle()."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'szw_id_<?php echo $this->getId() ?>', '<?php echo isset($resultStatusZamowieniaWorkflow) ? (is_array($resultStatusZamowieniaWorkflow) ? $resultStatusZamowieniaWorkflow['szw_id'] : $resultStatusZamowieniaWorkflow->getId()) : '' ?>');
...
<?php
/* MILESTONE 1.5 Table */
	} elseif ($statusZamowieniaWorkflowDisplayMode == "TABLE") {
PROFILE('TABLE');
		if (P('form_only') == "3") {
?>
<?php
	...
	$resultCount = -1;
	$filterApplied = false;
	$resultStatusZamowieniaWorkflow->setShowPage(1); 
	$resultStatusZamowieniaWorkflow->setShowRows('all'); 	
	$resultsStatusZamowieniaWorkflow = $resultStatusZamowieniaWorkflow->getTableData($resultCount, $filterApplied);
	$events = array();
	foreach ($resultsStatusZamowieniaWorkflow as $resultStatusZamowieniaWorkflow) {
		if (isset($resultStatusZamowieniaWorkflow[$eventColumn]) && isset($events[substr($resultStatusZamowieniaWorkflow[$eventColumn], 0, 10)])) {
			$eventsInDay = $events[substr($resultStatusZamowieniaWorkflow[$eventColumn], 0, 10)];
		} else {
			$eventsInDay = array();
		}
		$eventObject = new virgoStatusZamowieniaWorkflow($resultStatusZamowieniaWorkflow['szw_id']);
		$eventsInDay[] = $eventObject;
		$events[substr($resultStatusZamowieniaWorkflow[$eventColumn], 0, 10)] = $eventsInDay;
	}
?>
<style type="text/css">
...
?>			
			...
			<tr class='virgo_calendar_week'>
				...
				<input type='hidden' name='szw_id_<?php echo $this->getId() ?>' value=''/>
<?php
	$tmpWeekDay = 1;
	$safety = 0;
	if ($tmpDay["wday"] > 0) {
...
<?php
		...
		if (isset($events[$tmpStringDay])) {
			...
			foreach ($eventsInDay as $resultStatusZamowieniaWorkflow) {
?>
<?php
PROFILE('token');
	if (isset($resultStatusZamowieniaWorkflow)) {
...
<div class="button_wrapper button_wrapper_view inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'szw_id_<?php echo $this->getId() ?>', '<?php echo isset($resultStatusZamowieniaWorkflow) ? (is_array($resultStatusZamowieniaWorkflow) ? $resultStatusZamowieniaWorkflow['szw_id'] : $resultStatusZamowieniaWorkflow->getId()) : '' ?>');
 								...
 								form.target = '';
							" 
							value="<?php echo T($resultStatusZamowieniaWorkflow->getVirgoTitle()) ?>"
						/><div class="button_right"></div></div><?php
	}
?>
<?php				
//				echo "<span class='virgo_calendar_event' onclick='var form=document.getElementById(\"portlet_form_".$this->getId()."\");form.portlet_action.value=\"View\";form.szw_id_".$this->getId().".value=\"".$eventInDay->getId()."\";form.submit();'>" . $eventInDay->getVirgoTitle() . "</span>";
			}
		}
		echo "</td>";
...
?>
		<script type="text/javascript">
			var statusZamowieniaWorkflowChildrenDivOpen = '';
			
			function childrenButtonClicked(clickedDivId) {
				var div = document.getElementById(clickedDivId);
				if (clickedDivId == statusZamowieniaWorkflowChildrenDivOpen) {
					div.style.display = 'none';
					statusZamowieniaWorkflowChildrenDivOpen = '';
				} else {
					if (statusZamowieniaWorkflowChildrenDivOpen != '') {
						document.getElementById(statusZamowieniaWorkflowChildrenDivOpen).style.display = 'none';
					}
					div.style.display = 'block';
					statusZamowieniaWorkflowChildrenDivOpen = clickedDivId;
				}
			}
...
PROFILE('table_01');
PROFILE('table_02');
			$showPage = $resultStatusZamowieniaWorkflow->getShowPage(); 
			$showRows = $resultStatusZamowieniaWorkflow->getShowRows(); 
?>
						<input type="text" name="szw_id_<?php echo $this->getId() ?>" id="szw_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>			
				<tr><td colspan="99" class="nav-header"><?php echo T('Status zamówienia workflows') ?></td></tr>
<?php
			}
?>			
<?php
PROFILE('table_02');
PROFILE('main select');
			$virgoOrderColumn = $resultStatusZamowieniaWorkflow->getOrderColumn();
			$virgoOrderMode = $resultStatusZamowieniaWorkflow->getOrderMode();
			$resultCount = -1;
			$filterApplied = false;
			$resultsStatusZamowieniaWorkflow = $resultStatusZamowieniaWorkflow->getTableData($resultCount, $filterApplied);
PROFILE('main select');
...
?>
<?php
if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoStatusZamowienia'.DIRECTORY_SEPARATOR.'controller.php') && P('show_table_status_zamowienianext', "1") != "0"  ) {
	if (P('show_table_status_zamowienianext', "1") == "2") {
		$tmpLookupStatusZamowieniaNext = virgoStatusZamowienia::getVirgoListStatic();
?>
<input name='szw_StatusZamowieniaNext_id_<?php echo $this->getId() ?>' id='szw_StatusZamowieniaNext_id_<?php echo $this->getId() ?>' type="hidden"/>
<?php		
	}
?>
				<th align="center" valign="middle" rowspan=" 2"><!-- bylo: nowrap -->
<?php
	$oc = $resultStatusZamowieniaWorkflow->getOrderColumn(); 
	$om = $resultStatusZamowieniaWorkflow->getOrderMode();
...
?>
	...
	_pSF(form, 'virgo_order_column', 'status_zamowienia_next');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php
	$customLabel = TE('STATUS_ZAMOWIENIA_NEXT'); 
	echo !is_null($customLabel) ? $customLabel : T('STATUS_ZAMOWIENIA') . '&nbsp;' . T('NEXT')
?>
							<?php echo ($oc == "status_zamowienia_next" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		...
		if ($parentsCount < 51) {
			...
			$parentFilter = virgoStatusZamowieniaWorkflow::getLocalSessionValue('VirgoFilterStatusZamowieniaNext', null);
?>
						<select 
							name="virgo_filter_status_zamowienianext"
							class="virgo_filter"
...
<?php		
			...
			$parentFilter = virgoStatusZamowieniaWorkflow::getLocalSessionValue('VirgoFilterTitleStatusZamowieniaNext', null);
?>
						<input
							name="virgo_filter_title_status_zamowienianext"
							class="virgo_filter"
							onChange="<?php echo JSFS(null, "Submit", true, array(), false, "SetVirgoTableFilter") ?>"
...
?>
<?php
if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoStatusZamowienia'.DIRECTORY_SEPARATOR.'controller.php') && P('show_table_status_zamowieniaprev', "1") != "0"  ) {
	if (P('show_table_status_zamowieniaprev', "1") == "2") {
		$tmpLookupStatusZamowieniaPrev = virgoStatusZamowienia::getVirgoListStatic();
?>
<input name='szw_StatusZamowieniaPrev_id_<?php echo $this->getId() ?>' id='szw_StatusZamowieniaPrev_id_<?php echo $this->getId() ?>' type="hidden"/>
<?php		
	}
?>
				<th align="center" valign="middle" rowspan=" 2"><!-- bylo: nowrap -->
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder';
	_pSF(form, 'virgo_order_column', 'status_zamowienia_prev');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php
	$customLabel = TE('STATUS_ZAMOWIENIA_PREV'); 
	echo !is_null($customLabel) ? $customLabel : T('STATUS_ZAMOWIENIA') . '&nbsp;' . T('PREV')
?>
							<?php echo ($oc == "status_zamowienia_prev" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$parentsCount = virgoStatusZamowienia::getVirgoListStatic('', true);
		if ($parentsCount < 51) {
			$parents = virgoStatusZamowienia::getVirgoListStatic();
			$parentFilter = virgoStatusZamowieniaWorkflow::getLocalSessionValue('VirgoFilterStatusZamowieniaPrev', null);
?>
						<select 
							name="virgo_filter_status_zamowieniaprev"
							class="virgo_filter"
							onChange="<?php echo JSFS(null, "Submit", true, array(), false, "SetVirgoTableFilter") ?>"
						>
							<option value=""></option>
...
<?php		
			...
			$parentFilter = virgoStatusZamowieniaWorkflow::getLocalSessionValue('VirgoFilterTitleStatusZamowieniaPrev', null);
?>
						<input
							name="virgo_filter_title_status_zamowieniaprev"
							class="virgo_filter"
							onChange="<?php echo JSFS(null, "Submit", true, array(), false, "SetVirgoTableFilter") ?>"
							value="<?php echo $parentFilter ?>"
						/>
...
<?php
			...
			if ($resultCount != 0) {
				if (((int)$showRows) * (((int)$showPage)-1) == $resultCount ) {
					...
					$resultStatusZamowieniaWorkflow->setShowPage($showPage);
				}
				$index = 0;
PROFILE('table_04');
...
?>
<?php
$fileNameToInclude = PORTAL_PATH . "/portlets/sealock/virgoStatusZamowieniaWorkflow/modules/renderTableRow_{$_SESSION['current_portlet_object_id']}.php";
...
<?php
			if (P('form_only') != "4" || (P('chessboard_width') != 0 && $index % P('chessboard_width') == 1)) {
				if (is_null($firstRowId)) {
					$firstRowId = $resultStatusZamowieniaWorkflow['szw_id'];
				}
				$displayClass = ' displayClass ';
				$tmpContextId = virgoStatusZamowieniaWorkflow::getContextId();
				if (is_null($tmpContextId)) {
					$forceContextOnFirstRow = P('force_context_on_first_row', "1");
					if ($forceContextOnFirstRow == "1") {
						virgoStatusZamowieniaWorkflow::setContextId($resultStatusZamowieniaWorkflow['szw_id'], false);
						$tmpContextId = $resultStatusZamowieniaWorkflow['szw_id'];
					}
				}
				if (isset($tmpContextId) && $resultStatusZamowieniaWorkflow['szw_id'] == $tmpContextId) {
					if (P('form_only') != "4") {
...
?>
			<tr 
				id="<?php echo $this->getId() ?>_<?php echo isset($resultStatusZamowieniaWorkflow['szw_id']) ? $resultStatusZamowieniaWorkflow['szw_id'] : "" ?>" 
				class="<?php echo (P('form_only') == "4" ? "data_table_chessboard" : ($index % 2 == 0 ? "data_table_even" : "data_table_odd")) ?> <?php echo $contextClass ?>
 <? echo $displayClass ?> 
<?php
				if (class_exists('sealock\virgoStatusZamowienia') && P('show_view_status_zamowienianext', "1") == "1") {
?>
 status_zamowienia_<?php echo isset($resultStatusZamowieniaWorkflow['szw_szm_next_id']) ? $resultStatusZamowieniaWorkflow['szw_szm_next_id'] : "" ?>
<?php
				}
?>
<?php
				if (class_exists('sealock\virgoStatusZamowienia') && P('show_view_status_zamowieniaprev', "1") == "1") {
?>
 status_zamowienia_<?php echo isset($resultStatusZamowieniaWorkflow['szw_szm_prev_id']) ? $resultStatusZamowieniaWorkflow['szw_szm_prev_id'] : "" ?>
<?php
				}
?>
			" 
...
?>
				<input type="checkbox" class="checkbox" style="float: right;" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultStatusZamowieniaWorkflow['szw_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
				<input type="checkbox" class="checkbox" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultStatusZamowieniaWorkflow['szw_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
<?php
	if (class_exists('sealock\virgoStatusZamowienia') && P('show_table_status_zamowienianext', "1") != "0"  ) {
?>
<?php
			if (P('form_only') != "4") {
?>
				<td 
					align="left" 
					class="<?php echo ($index % 2 == 0 ? 'data_table_even' : 'data_table_odd') ?> <?php echo P('show_table_status_zamowienianext', "1") == "1" ? " selectable " : " " ?> "
				>    
<?php
			} else {
?>
				<li class="status_zamowienia_next">
<?php
			} 
?>
    <span 
...
?>
			<a  
				...
				onclick="
					...
					form.portlet_action.value='View'; 
					form.szw_id_<?php echo $this->getId() ?>.value='<?php echo isset($resultStatusZamowieniaWorkflow['szw_id']) ? $resultStatusZamowieniaWorkflow['szw_id'] : '' ?>'; 
					form.target = ''; 
<?php
					if (isset($ajax) && !$ajax) {
?>					
...
?>
<?php 
	if (P('show_table_status_zamowienianext', "1") == "1") {
		if (isset($resultStatusZamowieniaWorkflow['status_zamowienia_next'])) {
			echo $resultStatusZamowieniaWorkflow['status_zamowienia_next'];
		}
	} else {
//		echo $resultStatusZamowieniaWorkflow['szw_szm_id'];
?>
		<select class="inline" onchange="
this.form.portlet_action.value='VirgoSetStatusZamowieniaNext';
	_pSF(form, 'szw_id_<?php echo $this->getId() ?>', '<?php echo $resultStatusZamowieniaWorkflow['szw_id'] ?>');
	_pSF(form, 'szw_StatusZamowieniaNext_id_<?php echo $this->getId() ?>', this.options[this.selectedIndex].value);
<?php
					if (isset($ajax) && !$ajax) {
?>					
						form.submit();
...
?>					
		">
<?php
		foreach ($tmpLookupStatusZamowieniaNext as $value => $label) {
?>
			<option value="<?php echo $value ?>" <?php echo $value == $resultStatusZamowieniaWorkflow['szw_szm_id'] ? " selected " : "" ?>><?php echo $label ?></option>
<?php			
		}
?>
		</select>
...
?>
<?php
	if (class_exists('sealock\virgoStatusZamowienia') && P('show_table_status_zamowieniaprev', "1") != "0"  ) {
?>
<?php
			if (P('form_only') != "4") {
?>
				<td 
					align="left" 
					class="<?php echo ($index % 2 == 0 ? 'data_table_even' : 'data_table_odd') ?> <?php echo P('show_table_status_zamowieniaprev', "1") == "1" ? " selectable " : " " ?> "
				>    
<?php
			} else {
?>
				<li class="status_zamowienia_prev">
<?php
			} 
?>
    <span 
...
?>
<?php 
	if (P('show_table_status_zamowieniaprev', "1") == "1") {
		if (isset($resultStatusZamowieniaWorkflow['status_zamowienia_prev'])) {
			echo $resultStatusZamowieniaWorkflow['status_zamowienia_prev'];
		}
	} else {
//		echo $resultStatusZamowieniaWorkflow['szw_szm_id'];
?>
		<select class="inline" onchange="
this.form.portlet_action.value='VirgoSetStatusZamowieniaPrev';
	_pSF(form, 'szw_id_<?php echo $this->getId() ?>', '<?php echo $resultStatusZamowieniaWorkflow['szw_id'] ?>');
	_pSF(form, 'szw_StatusZamowieniaPrev_id_<?php echo $this->getId() ?>', this.options[this.selectedIndex].value);
<?php
					if (isset($ajax) && !$ajax) {
?>					
						form.submit();
...
?>					
		">
<?php
		foreach ($tmpLookupStatusZamowieniaPrev as $value => $label) {
?>
			<option value="<?php echo $value ?>" <?php echo $value == $resultStatusZamowieniaWorkflow['szw_szm_id'] ? " selected " : "" ?>><?php echo $label ?></option>
<?php			
		}
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("STATUS_ZAMOWIENIA_WORKFLOW"), "\\'".rawurlencode($resultStatusZamowieniaWorkflow['szw_virgo_title'])."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'szw_id_<?php echo $this->getId() ?>', '<?php echo isset($resultStatusZamowieniaWorkflow) ? (is_array($resultStatusZamowieniaWorkflow) ? $resultStatusZamowieniaWorkflow['szw_id'] : $resultStatusZamowieniaWorkflow->getId()) : '' ?>');
...
<?php
	...
	if (is_null($contextRowIdInTable)) {
		$forceContextOnFirstRow = P('force_context_on_first_row', "1");
		if ($forceContextOnFirstRow == "1") {
			virgoStatusZamowieniaWorkflow::setContextId($firstRowId, false);
			if (P('form_only') != "4") {
?>
<script type="text/javascript">
...
<?php
				...
				unset($resultStatusZamowieniaWorkflow);
				unset($resultsStatusZamowieniaWorkflow);
				if (isset($contextIdOwn) && trim($contextIdOwn) != "") {
					if ($contextIdConfirmed == false) {
						$tmpStatusZamowieniaWorkflow = new virgoStatusZamowieniaWorkflow();
						$tmpCount = $tmpStatusZamowieniaWorkflow->getAllRecordCount(' szw_id = ' . $contextIdOwn);
						if ($tmpCount == 0) {
							virgoStatusZamowieniaWorkflow::clearRemoteContextId($tabModeEditMenu);
						}
					}
				}
PROFILE('rows rendering');
...
?>
		$.getJSON('<?php echo $page->getUrl() ?>?portlet_action=SelectJson&szw_id_<?php echo $this->getId() ?>=' + virgoId + '&invoked_portlet_object_id=<?php echo $this->getId() ?>&virgo_action_mode_json=T&_virgo_ajax=1', function(data) {
<?php
			if (P('form_only') != "4") {
?>
...
?>
		...
		form.szw_id_<?php echo $this->getId() ?>.value=virgoId; 
		form.target = ''; 
<?php
...
<div class="button_wrapper button_wrapper_report inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								...
								for(var i = 0; i< children.length;i++) {
								  if (children[i].getAttribute('name') == 'szw_id_<?php echo $this->getId() ?>') {
									found = 1;
								  }
								}
								if (found == 0) {
								  ...
								  input.setAttribute('name', 'szw_id_<?php echo $this->getId() ?>');
								  input.setAttribute('value', '');
								  form.appendChild(input);
								} else {
	_pSF(form, 'szw_id_<?php echo $this->getId() ?>', '');
								}
								form.action = '';
...
<?php
					$sessionSeparator = virgoStatusZamowieniaWorkflow::getImportFieldSeparator();
...
<div class="button_wrapper button_wrapper_deleteselected inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								if (!nothingSelected(this.form, <?php echo $this->getId() ?>)) {
									if (!confirm(decodeURIComponent('<?php echo T('ARE_YOU_SURE_YOU_WANT_REMOVE', T('STATUS_ZAMOWIENIA_WORKFLOWS'), "") ?>'))) return false;
								} else {
									var alertText = '<?php echo T('NOTHING_SELECTED') ?>';
									if (alertText.charAt(alertText.length-1)=='?') {
										if (!confirm(alertText)) return false;
...
/* MILESTONE 1.6 TableForm */
	} elseif ($statusZamowieniaWorkflowDisplayMode == "TABLE_FORM") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
	...
	<form method="post" style="display: inline;" action="" id="virgo_form_status_zamowienia_workflow" name="virgo_form_status_zamowienia_workflow" enctype="multipart/form-data">
						<input type="text" name="szw_id_<?php echo $this->getId() ?>" id="szw_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>
<?php
	if (P('show_table_status_zamowienianext', "1") == "1" /* && ($masterComponentName != "status_zamowienia" || is_null($contextId)) */) {
?>
				<td align="center" nowrap>Status zamówienia Next</td>
<?php
	}
?>
<?php
	if (P('show_table_status_zamowieniaprev', "1") == "1" /* && ($masterComponentName != "status_zamowienia" || is_null($contextId)) */) {
?>
				<td align="center" nowrap>Status zamówienia Prev</td>
<?php
	}
?>
				<td></td>
...
<?php			
				$resultsStatusZamowieniaWorkflow = $resultStatusZamowieniaWorkflow->getRecordsToEdit();
				$idsToCorrect = $resultStatusZamowieniaWorkflow->getInvalidRecords();
				$index = 0;
PROFILE('rows rendering');
				foreach ($resultsStatusZamowieniaWorkflow as $resultStatusZamowieniaWorkflow) {
					$index = $index + 1;
?>
			<tr id="virgo_tr_id_<?php echo $resultStatusZamowieniaWorkflow->szw_id ?>" class="<?php echo ($index % 2 == 0 ? "data_table_even" : "data_table_odd") ?>"
<?php
	if ($resultStatusZamowieniaWorkflow->szw_id == 0 && R('virgo_validate_new', "N") == "N") {
?>
		style="display: none;"
<?php
	}
...
<?php
	$tabIndex = $index + sizeof($resultsStatusZamowieniaWorkflow) * 0;
?>
<?php
//		$limit_status_zamowienianext = $componentParams->get('limit_to_status_zamowienianext');
		$limit_status_zamowienianext = null;
...
?>
				<td nowrap align="left" class="<?php echo ($index % 2 == 0 ? "data_table_even" : "data_table_odd") ?>">    
<?php
	$tabIndex = $index + sizeof($resultsStatusZamowieniaWorkflow) * 1;
?>
<?php
//		$limit_status_zamowieniaprev = $componentParams->get('limit_to_status_zamowieniaprev');
		$limit_status_zamowieniaprev = null;
...
?>
				<td>
<?php
	if (isset($idsToCorrect[$resultStatusZamowieniaWorkflow->szw_id])) {
		$errorMessage = $idsToCorrect[$resultStatusZamowieniaWorkflow->szw_id];
?>
					<div class="error">
...
?>
		<div class="<?php echo $statusZamowieniaWorkflowDisplayMode ?>">
<?php
?>
			<div class="buttons form-actions">
<?php
...
<div style="display: none; background-color:#FFFFFF; border:1px solid #000000; font-size:10px; margin:10px 0; padding:10px;"; id="extraFilesInfo_slk_status_zamowienia_workflow" style="font-size: 12px; " onclick="document.getElementById('extraFilesInfo_slk_status_zamowienia_workflow').style.display='none';">
...
<?php
	$infos = virgoStatusZamowieniaWorkflow::getExtraFilesInfo();
