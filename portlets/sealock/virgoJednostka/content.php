<?php
	...
	use sealock\virgoJednostka;

//	setlocale(LC_ALL, '$messages.LOCALE');
	$componentParams = null; //&JComponentHelper::getParams('com_slk_jednostka');
...
?>
<link rel="stylesheet" href="<?php echo $live_site ?>/components/com_slk_jednostka/sealock.css" type="text/css" /> 
<?php
	}
?>
<?php
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoJednostka'.DIRECTORY_SEPARATOR.'slk_jdn.css')) {
?>
<link rel="stylesheet" href="<?php echo $_SESSION['portal_url'] ?>/portlets/sealock/virgoJednostka/slk_jdn.css" type="text/css" /> 
<?php
	}
?>
<style>
...
<div class="virgo_container_sealock virgo_container_entity_jednostka" style="border: none;">
...
//		$classNames["pozycja_dokumentu"] = "virgoPozycjaDokumentu";
		...
		$ancestors = array();
		$contextId = null;		
			$resultJednostka = virgoJednostka::createGuiAware();
			$contextId = $resultJednostka->getContextId();
			if (isset($contextId)) {
				if (virgoJednostka::getDisplayMode() != "CREATE" || R('portlet_action') == "Duplicate") {
					$resultJednostka->load($contextId);
				}
			}
?>
<?php
...
<?php
	...
	if (isset($masterPobId)) {
		...
		if ($className == "virgoJednostka") {
			$masterObject = new $className();
			$tmpId = $masterObject->getRemoteContextId($masterPobId);
			if (isset($tmpId)) {
				$resultJednostka = new virgoJednostka($tmpId);
				virgoJednostka::setDisplayMode("FORM");
			} else {
				$resultJednostka = new virgoJednostka();
				virgoJednostka::setDisplayMode("CREATE");
			}
		}
	} else {
		if (P('form_only', "0") == "5") {
			if (is_null($resultJednostka->getId())) { 
				if (P('only_private_records', "0") == "1") {
					$allPrivateRecords = $resultJednostka->selectAll();
					if (sizeof($allPrivateRecords) > 0) {
						$resultJednostka = new virgoJednostka($allPrivateRecords[0]['jdn_id']);
						$resultJednostka->putInContext(false);
					} else {
						$resultJednostka = new virgoJednostka();
					}
				} else {
					...
					if (isset($customSQL) && trim($customSQL) != '') {
						...
						$records = $resultJednostka->selectAll($customSQL);
						if (sizeof($records) > 0) {
							$resultJednostka = new virgoJednostka($records[0]['jdn_id']);
							$resultJednostka->putInContext(false);
						} else {
							$resultJednostka = new virgoJednostka();
						}
			...
			$resultJednostka = new virgoJednostka(virgoUser::getUserId());
			$resultJednostka->putInContext(false);
		}
	}
?>
...
?>
<?php
	$jednostkaDisplayMode = virgoJednostka::getDisplayMode();
//	if ($jednostkaDisplayMode == "" || $jednostkaDisplayMode == "TABLE") {
//		$resultJednostka = $resultJednostka->portletActionForm();
//	}
?>
		<div class="form">
<?php
		$parentContextInfos = $resultJednostka->getParentsInContext();
...
<?php
PROFILE('token');
	if (isset($resultJednostka)) {
		$tmpId = is_array($resultJednostka) ? $resultJednostka['jdn_id'] : $resultJednostka->getId();
		if (isset($tmpId)) {
?>
...
<?php
	if ($jednostkaDisplayMode != "TABLE") {
		...
		if (
			...
			$pob = $resultJednostka->getMyPortletObject();
			...
			if (isset($invokedPortletId) && $invokedPortletId == $_SESSION['current_portlet_object_id'] && isset($reloadFromRequest) && $reloadFromRequest == "1") { 
				...
				$resultJednostka->loadFromRequest();
			} else {
				if (P('form_only', "0") == "1" && isset($contextId)) {
					if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoJednostka'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
						require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoJednostka'.DIRECTORY_SEPARATOR.'create_store_message.php');
						$jednostkaDisplayMode = "-empty-";
					}
				}
			}
		}
	}
if (!$resultJednostka->hideContentDueToNoParentSelected()) {
...
<script language="JavaScript">
  ...
  function confirmExit() {
	  ...
	  for (var i=0; i < checks.length; i++) {
		  ...
		  if (check.form.name == "virgo_form_jednostka") {
			  if (check.value == 'T') {
...
?>
					<label 
						...
						for="jdn_nazwa_<?php echo $resultJednostka->getId() ?>"
					>* <?php echo T('NAZWA') ?>
</label>
<?php
	if (!$formsInTable) {
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultJednostka->getNazwa(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="nazwa" name="jdn_nazwa_<?php echo $resultJednostka->getId() ?>" value="<?php echo htmlentities($resultJednostka->getNazwa(), ENT_QUOTES, "UTF-8") ?>"> 
...
<?php
	if (!isset($resultJednostka)) {
		$resultJednostka = new sealock\virgoJednostka();
	}
?>
						<input 
							class="inputbox  obligatory   medium  " 
							type="text"
							id="jdn_nazwa_<?php echo $resultJednostka->getId() ?>" 
							name="jdn_nazwa_<?php echo $resultJednostka->getId() ?>"
							maxlength="255"
							size="30" 
							value="<?php echo htmlentities($resultJednostka->getNazwa(), ENT_QUOTES, "UTF-8") ?>" 
...
$('#jdn_nazwa_<?php echo $resultJednostka->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
?>
					<label 
						...
						for="jdn_opis_<?php echo $resultJednostka->getId() ?>"
...
<div 
	...
	id="opis" 
><?php echo htmlentities($resultJednostka->getOpis(), ENT_QUOTES, "UTF-8") ?></div>
...
<textarea 
	class="inputbox opis" 
	id="jdn_opis_<?php echo $resultJednostka->getId() ?>" 
	name="jdn_opis_<?php echo $resultJednostka->getId() ?>"
	rows="5"
	cols="48"
	onchange="this.form.virgo_changed.value='T'"	
<?php
...
?>
><?php echo htmlentities($resultJednostka->getOpis(), ENT_QUOTES, "UTF-8") ?></textarea>
<?php
	if (isset($hint)) {
?>						
<script type="text/javascript">
$('#jdn_opis_<?php echo $resultJednostka->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php			
				...
				$resultsTowar = $tmpTowar->selectAll(' twr_jdn_id = ' . $resultJednostka->jdn_id);
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsJednostka) * 0;
?>
<?php
...
?> 
						<input
							type="hidden"
							id="kod_<?php echo $resultJednostka->jdn_id ?>" 
							name="kod_<?php echo $resultJednostka->jdn_id ?>"
							value="<?php echo htmlentities($resultJednostka->jdn_kod, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsJednostka) * 1;
?>
<?php
	if (!isset($resultTowar)) {
		$resultTowar = new sealock\virgoTowar();
...
?> 
						<input
							type="hidden"
							id="nazwa_<?php echo $resultJednostka->jdn_id ?>" 
							name="nazwa_<?php echo $resultJednostka->jdn_id ?>"
							value="<?php echo htmlentities($resultJednostka->jdn_nazwa, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsJednostka) * 2;
?>
<?php
	if (!isset($resultTowar)) {
		$resultTowar = new sealock\virgoTowar();
...
?> 
						<input
							type="hidden"
							id="stanAktualny_<?php echo $resultJednostka->jdn_id ?>" 
							name="stanAktualny_<?php echo $resultJednostka->jdn_id ?>"
							value="<?php echo htmlentities($resultJednostka->jdn_stan_aktualny, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsJednostka) * 3;
?>
<?php
	if (!isset($resultTowar)) {
		$resultTowar = new sealock\virgoTowar();
...
?> 
						<input
							type="hidden"
							id="stanMinimalny_<?php echo $resultJednostka->jdn_id ?>" 
							name="stanMinimalny_<?php echo $resultJednostka->jdn_id ?>"
							value="<?php echo htmlentities($resultJednostka->jdn_stan_minimalny, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsJednostka) * 4;
?>
<?php
	if (!isset($resultTowar)) {
		$resultTowar = new sealock\virgoTowar();
...
?> 
						<input
							type="hidden"
							id="produkt_<?php echo $resultJednostka->jdn_id ?>" 
							name="produkt_<?php echo $resultJednostka->jdn_id ?>"
							value="<?php echo htmlentities($resultJednostka->jdn_produkt, ENT_QUOTES, "UTF-8") ?>"
						>
...
//		$limit_grupa_towaru = $componentParams->get('limit_to_grupa_towaru');
		...
		$tmpId = sealock\virgoJednostka::getParentInContext("sealock\\virgoGrupaTowaru");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#jdn_grupa_towaru_dropdown_<?php echo $resultJednostka->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
if (P('hide_audit', '0') == '0') {
	$showFieldset = 0;
	if ($resultJednostka->getDateCreated()) {
		if ($resultJednostka->getUsrCreatedId() == 0) {
			$username = "anonymous";
		} else {
			$createdByUser = new virgoUser((int)$resultJednostka->getUsrCreatedId());
			$username = $createdByUser->getUsername();
		}
		$createdOn = $resultJednostka->getDateCreated();
		$showFieldset = 1;
?>
		...
		<legend><?php echo T('AUDIT_DATA') ?>:</legend>
			<ul>
				<li>
					...
					<input type="hidden" id="usr_created_id" name="usr_created_id" value="<?php echo $resultJednostka->getUsrCreatedId() ?>"	>
				</li>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('CREATION_DATE') ?></span>
					<span class="readonly"><i><?php echo $createdOn ?></i></span>
					<input type="hidden" id="date_created" name="date_created" value="<?php echo $resultJednostka->getDateCreated() ?>"	>
				</li>
<?php
	}
?>
<?php
	if ($resultJednostka->getDateModified()) {
		if ($resultJednostka->getUsrModifiedId() == 0) {
			$username = "anonymous";
		} else {
			$modifiedByUser = new virgoUser((int)$resultJednostka->getUsrModifiedId());
			$username = $modifiedByUser->getUsername();
		}
		$modifiedOn = $resultJednostka->getDateModified();
?>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('MODIFIED_BY') ?></span>
					<span class="readonly"><b><?php echo $username ?></b></span>
					<input type="hidden" id="usr_modified_id" name="usr_modified_id" value="<?php echo $resultJednostka->getUsrModifiedId() ?>"	>
				</li>
				<li>
					...
					<input type="hidden" id="date_modified" name="date_modified" value="<?php echo $resultJednostka->getDateModified() ?>"	>
				</li>
<?php
	}
	if ($showFieldset == 1) {
...
?>
				...
				<div class="buttons form-actions">
						<input type="text" name="jdn_id_<?php echo $this->getId() ?>" id="jdn_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
 ...
 <div class="button_wrapper button_wrapper_store inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'jdn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultJednostka) ? (is_array($resultJednostka) ? $resultJednostka['jdn_id'] : $resultJednostka->getId()) : '' ?>');
...
<?php
	...
	if ($resultJednostka->isDeletedVirgo()) {
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("JEDNOSTKA"), "\\'".rawurlencode($resultJednostka->getVirgoTitle())."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
...
// document.forms['portlet_form_<?php echo $this->getId() ?>'].elements['jdn_nazwa_<?php echo $resultJednostka->getId() ?>'].focus();
</script>
	</div>
<?php
/* MILESTONE 1.2 Create */
	} elseif ($jednostkaDisplayMode == "CREATE") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
...
?>
<?php
		if (isset($resultJednostka->jdn_id)) {
			$resultJednostka->jdn_id = null;
		}
		$parentAjaxRendered = "0";
?>
<?php
...
?>
					<label nowrap class="fieldlabel  obligatory " for="jdn_nazwa_<?php echo $resultJednostka->getId() ?>">
* <?php echo T('NAZWA') ?>
					</label>
<?php
	if (!$formsInTable) {
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "nazwa") {
				$resultJednostka->setNazwa($virgoSelectedDay);
			}
		}
	}
...
?>
					<label nowrap class="fieldlabel  <?php echo P('show_create_opis_obligatory', "0") == "1" ? " obligatory " : "" ?> " for="jdn_opis_<?php echo $resultJednostka->getId() ?>">
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "opis") {
				$resultJednostka->setOpis($virgoSelectedDay);
			}
		}
	}
...
/* MILESTONE 1.3 Search */
	} elseif ($jednostkaDisplayMode == "SEARCH") {
?>
...
?>
<?php
	$record = new sealock\virgoJednostka();
	$recordId = is_array($resultJednostka) ? $resultJednostka['jdn_id'] : $resultJednostka->jdn_id;
...
<?php
/* MILESTONE 1.4 View */
	} elseif ($jednostkaDisplayMode == "VIEW") {
?>
	<div class="form_view">
<?php
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("JEDNOSTKA"), "\\'".$resultJednostka->getVirgoTitle()."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'jdn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultJednostka) ? (is_array($resultJednostka) ? $resultJednostka['jdn_id'] : $resultJednostka->getId()) : '' ?>');
...
<?php
/* MILESTONE 1.5 Table */
	} elseif ($jednostkaDisplayMode == "TABLE") {
PROFILE('TABLE');
		if (P('form_only') == "3") {
?>
<?php
	...
	$resultCount = -1;
	$filterApplied = false;
	$resultJednostka->setShowPage(1); 
	$resultJednostka->setShowRows('all'); 	
	$resultsJednostka = $resultJednostka->getTableData($resultCount, $filterApplied);
	$events = array();
	foreach ($resultsJednostka as $resultJednostka) {
		if (isset($resultJednostka[$eventColumn]) && isset($events[substr($resultJednostka[$eventColumn], 0, 10)])) {
			$eventsInDay = $events[substr($resultJednostka[$eventColumn], 0, 10)];
		} else {
			$eventsInDay = array();
		}
		$eventObject = new virgoJednostka($resultJednostka['jdn_id']);
		$eventsInDay[] = $eventObject;
		$events[substr($resultJednostka[$eventColumn], 0, 10)] = $eventsInDay;
	}
?>
<style type="text/css">
...
?>			
			...
			<tr class='virgo_calendar_week'>
				...
				<input type='hidden' name='jdn_id_<?php echo $this->getId() ?>' value=''/>
<?php
	$tmpWeekDay = 1;
	$safety = 0;
	if ($tmpDay["wday"] > 0) {
...
<?php
		...
		if (isset($events[$tmpStringDay])) {
			...
			foreach ($eventsInDay as $resultJednostka) {
?>
<?php
PROFILE('token');
	if (isset($resultJednostka)) {
...
<div class="button_wrapper button_wrapper_view inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'jdn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultJednostka) ? (is_array($resultJednostka) ? $resultJednostka['jdn_id'] : $resultJednostka->getId()) : '' ?>');
 								...
 								form.target = '';
							" 
							value="<?php echo T($resultJednostka->getVirgoTitle()) ?>"
						/><div class="button_right"></div></div><?php
	}
?>
<?php				
//				echo "<span class='virgo_calendar_event' onclick='var form=document.getElementById(\"portlet_form_".$this->getId()."\");form.portlet_action.value=\"View\";form.jdn_id_".$this->getId().".value=\"".$eventInDay->getId()."\";form.submit();'>" . $eventInDay->getVirgoTitle() . "</span>";
			}
		}
		echo "</td>";
...
?>
		<script type="text/javascript">
			var jednostkaChildrenDivOpen = '';
			
			function childrenButtonClicked(clickedDivId) {
				var div = document.getElementById(clickedDivId);
				if (clickedDivId == jednostkaChildrenDivOpen) {
					div.style.display = 'none';
					jednostkaChildrenDivOpen = '';
				} else {
					if (jednostkaChildrenDivOpen != '') {
						document.getElementById(jednostkaChildrenDivOpen).style.display = 'none';
					}
					div.style.display = 'block';
					jednostkaChildrenDivOpen = clickedDivId;
				}
			}
...
PROFILE('table_02');
	...
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoJednostka'.DIRECTORY_SEPARATOR.'controller.php')) require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoJednostka'.DIRECTORY_SEPARATOR.'controller.php');
			$showPage = $resultJednostka->getShowPage(); 
			$showRows = $resultJednostka->getShowRows(); 
?>
						<input type="text" name="jdn_id_<?php echo $this->getId() ?>" id="jdn_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
<?php
			if (P('master_mode', "0") == "1") {
?>			
				<tr><td colspan="99" class="nav-header"><?php echo T('Jednostki') ?></td></tr>
<?php
			}
?>			
<?php
PROFILE('table_02');
PROFILE('main select');
			$virgoOrderColumn = $resultJednostka->getOrderColumn();
			$virgoOrderMode = $resultJednostka->getOrderMode();
			$resultCount = -1;
			$filterApplied = false;
			$resultsJednostka = $resultJednostka->getTableData($resultCount, $filterApplied);
PROFILE('main select');
...
<?php
	$oc = $resultJednostka->getOrderColumn(); 
	$om = $resultJednostka->getOrderMode();
...
?>
	...
	_pSF(form, 'virgo_order_column', 'jdn_nazwa');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('NAZWA') ?>							<?php echo ($oc == "jdn_nazwa" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoJednostka::getLocalSessionValue('VirgoFilterNazwa', null);
?>
						<input
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'jdn_opis');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('OPIS') ?>							<?php echo ($oc == "jdn_opis" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoJednostka::getLocalSessionValue('VirgoFilterOpis', null);
?>
<?php		
	}
?>
...
<?php
			...
			if ($resultCount != 0) {
				if (((int)$showRows) * (((int)$showPage)-1) == $resultCount ) {
					...
					$resultJednostka->setShowPage($showPage);
				}
				$index = 0;
PROFILE('table_04');
...
?>
<?php
$fileNameToInclude = PORTAL_PATH . "/portlets/sealock/virgoJednostka/modules/renderTableRow_{$_SESSION['current_portlet_object_id']}.php";
...
<?php
			if (P('form_only') != "4" || (P('chessboard_width') != 0 && $index % P('chessboard_width') == 1)) {
				if (is_null($firstRowId)) {
					$firstRowId = $resultJednostka['jdn_id'];
				}
				$displayClass = ' displayClass ';
				$tmpContextId = virgoJednostka::getContextId();
				if (is_null($tmpContextId)) {
					$forceContextOnFirstRow = P('force_context_on_first_row', "1");
					if ($forceContextOnFirstRow == "1") {
						virgoJednostka::setContextId($resultJednostka['jdn_id'], false);
						$tmpContextId = $resultJednostka['jdn_id'];
					}
				}
				if (isset($tmpContextId) && $resultJednostka['jdn_id'] == $tmpContextId) {
					if (P('form_only') != "4") {
...
?>
			<tr 
				id="<?php echo $this->getId() ?>_<?php echo isset($resultJednostka['jdn_id']) ? $resultJednostka['jdn_id'] : "" ?>" 
				class="<?php echo (P('form_only') == "4" ? "data_table_chessboard" : ($index % 2 == 0 ? "data_table_even" : "data_table_odd")) ?> <?php echo $contextClass ?>
 <? echo $displayClass ?> 
jednostka_<?php echo isset($resultJednostka['jdn_id']) ? $resultJednostka['jdn_id'] : "" ?>
			" 
			>
<?php
			} 
...
<?php
	...
	if (P('master_mode', "0") != "1") {
			echo  $resultJednostka['jdn_id'];
	}
			if (P('master_mode', "0") != "1" && !$resultJednostka['jdn_virgo_deleted']) {
?>
				<input type="checkbox" class="checkbox" style="float: right;" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultJednostka['jdn_id'] ?>">
<?php
			}
			if (P('form_only') != "4") {
?>
...
<?php
			...
			if (P('master_mode', "0") != "1" && !$resultJednostka['jdn_virgo_deleted']) {
?>
				<input type="checkbox" class="checkbox" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultJednostka['jdn_id'] ?>">
<?php
			}
			if (P('form_only') != "4") {
?>
...
?>
			<a  
				...
				onclick="
					...
					form.portlet_action.value='View'; 
					form.jdn_id_<?php echo $this->getId() ?>.value='<?php echo isset($resultJednostka['jdn_id']) ? $resultJednostka['jdn_id'] : '' ?>'; 
					form.target = ''; 
<?php
					if (isset($ajax) && !$ajax) {
?>					
...
?>
		<span 
	style="<?php echo $resultJednostka['jdn_virgo_deleted'] ? "text-decoration: line-through;" : "" ?>"
			class="<?php echo 'displayClass' ?>">
				<?php echo htmlentities($resultJednostka['jdn_nazwa'], ENT_QUOTES, "UTF-8") ?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
?>
...
?>
		<?php
			$len = strlen($resultJednostka['jdn_opis']);
			$shortTextSize = P('short_text_size_opis', 30);
		?>
		<span 
	style="<?php echo $resultJednostka['jdn_virgo_deleted'] ? "text-decoration: line-through;" : "" ?>"
			class="<?php echo 'displayClass' ?>" <?php print ($len > $shortTextSize ? 'title="' . htmlentities($resultJednostka['jdn_opis'], ENT_QUOTES, "UTF-8") . '"' : '') ?>>
			<?php
				if ($shortTextSize > 0 && $len > $shortTextSize) {
					echo htmlentities(substr($resultJednostka['jdn_opis'], 0, $shortTextSize), ENT_QUOTES, "UTF-8") . "...";
				} else {
					if ($shortTextSize == 0) {
						echo "<pre style='white-space: pre-line;'>";
					}
					echo htmlentities($resultJednostka['jdn_opis'], ENT_QUOTES, "UTF-8");
					if ($shortTextSize == 0) {
						echo "</pre>";
					}
				}				 
...
<?php
			if ($this->canExecute("form")) {
		if ($resultJednostka['jdn_virgo_deleted']) {
?> <div class="button_wrapper inlineBlock"><input type="submit" disabled class="button button_disabled btn btn-mini" value="<?php echo T('EDIT') ?>"><div class="button_right button_disabled"></div></div><?php
		} else {
?> 
<?php
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("JEDNOSTKA"), "\\'".rawurlencode($resultJednostka['jdn_virgo_title'])."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'jdn_id_<?php echo $this->getId() ?>', '<?php echo isset($resultJednostka) ? (is_array($resultJednostka) ? $resultJednostka['jdn_id'] : $resultJednostka->getId()) : '' ?>');
...
<?php
	...
	if (is_null($contextRowIdInTable)) {
		$forceContextOnFirstRow = P('force_context_on_first_row', "1");
		if ($forceContextOnFirstRow == "1") {
			virgoJednostka::setContextId($firstRowId, false);
			if (P('form_only') != "4") {
?>
<script type="text/javascript">
...
<?php
				...
				unset($resultJednostka);
				unset($resultsJednostka);
				if (isset($contextIdOwn) && trim($contextIdOwn) != "") {
					if ($contextIdConfirmed == false) {
						$tmpJednostka = new virgoJednostka();
						$tmpCount = $tmpJednostka->getAllRecordCount(' jdn_id = ' . $contextIdOwn);
						if ($tmpCount == 0) {
							virgoJednostka::clearRemoteContextId($tabModeEditMenu);
						}
					}
				}
PROFILE('rows rendering');
...
?>
		$.getJSON('<?php echo $page->getUrl() ?>?portlet_action=SelectJson&jdn_id_<?php echo $this->getId() ?>=' + virgoId + '&invoked_portlet_object_id=<?php echo $this->getId() ?>&virgo_action_mode_json=T&_virgo_ajax=1', function(data) {
<?php
			if (P('form_only') != "4") {
?>
...
?>
		...
		form.jdn_id_<?php echo $this->getId() ?>.value=virgoId; 
		form.target = ''; 
<?php
...
<div class="button_wrapper button_wrapper_report inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								...
								for(var i = 0; i< children.length;i++) {
								  if (children[i].getAttribute('name') == 'jdn_id_<?php echo $this->getId() ?>') {
									found = 1;
								  }
								}
								if (found == 0) {
								  ...
								  input.setAttribute('name', 'jdn_id_<?php echo $this->getId() ?>');
								  input.setAttribute('value', '');
								  form.appendChild(input);
								} else {
	_pSF(form, 'jdn_id_<?php echo $this->getId() ?>', '');
								}
								form.action = '';
...
<?php
					$sessionSeparator = virgoJednostka::getImportFieldSeparator();
...
<div class="button_wrapper button_wrapper_deleteselected inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								if (!nothingSelected(this.form, <?php echo $this->getId() ?>)) {
									if (!confirm(decodeURIComponent('<?php echo T('ARE_YOU_SURE_YOU_WANT_REMOVE', T('JEDNOSTKI'), "") ?>'))) return false;
								} else {
									var alertText = '<?php echo T('NOTHING_SELECTED') ?>';
									if (alertText.charAt(alertText.length-1)=='?') {
										if (!confirm(alertText)) return false;
...
/* MILESTONE 1.6 TableForm */
	} elseif ($jednostkaDisplayMode == "TABLE_FORM") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
	...
	<form method="post" style="display: inline;" action="" id="virgo_form_jednostka" name="virgo_form_jednostka" enctype="multipart/form-data">
						<input type="text" name="jdn_id_<?php echo $this->getId() ?>" id="jdn_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
<?php			
				$resultsJednostka = $resultJednostka->getRecordsToEdit();
				$idsToCorrect = $resultJednostka->getInvalidRecords();
				$index = 0;
PROFILE('rows rendering');
				foreach ($resultsJednostka as $resultJednostka) {
					$index = $index + 1;
?>
			<tr id="virgo_tr_id_<?php echo $resultJednostka->jdn_id ?>" class="<?php echo ($index % 2 == 0 ? "data_table_even" : "data_table_odd") ?>"
<?php
	if ($resultJednostka->jdn_id == 0 && R('virgo_validate_new', "N") == "N") {
?>
		style="display: none;"
<?php
	}
...
?> 
						<input
							type="hidden"
							id="opis_<?php echo $resultJednostka->jdn_id ?>" 
							name="opis_<?php echo $resultJednostka->jdn_id ?>"
							value="<?php echo htmlentities($resultJednostka->jdn_opis, ENT_QUOTES, "UTF-8") ?>"
						>
...
?>
				<td>
<?php
	if (isset($idsToCorrect[$resultJednostka->jdn_id])) {
		$errorMessage = $idsToCorrect[$resultJednostka->jdn_id];
?>
					<div class="error">
						<?php echo $errorMessage ?>
					</div>
...
?>
		<div class="<?php echo $jednostkaDisplayMode ?>">
<?php
?>
			<div class="buttons form-actions">
<?php
...
<div style="display: none; background-color:#FFFFFF; border:1px solid #000000; font-size:10px; margin:10px 0; padding:10px;"; id="extraFilesInfo_slk_jednostka" style="font-size: 12px; " onclick="document.getElementById('extraFilesInfo_slk_jednostka').style.display='none';">
...
<?php
	$infos = virgoJednostka::getExtraFilesInfo();
