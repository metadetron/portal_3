<?php
	...
	use sealock\virgoOdbiorca;

//	setlocale(LC_ALL, '$messages.LOCALE');
	$componentParams = null; //&JComponentHelper::getParams('com_slk_odbiorca');
...
?>
<link rel="stylesheet" href="<?php echo $live_site ?>/components/com_slk_odbiorca/sealock.css" type="text/css" /> 
<?php
	}
?>
<?php
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoOdbiorca'.DIRECTORY_SEPARATOR.'slk_odb.css')) {
?>
<link rel="stylesheet" href="<?php echo $_SESSION['portal_url'] ?>/portlets/sealock/virgoOdbiorca/slk_odb.css" type="text/css" /> 
<?php
	}
?>
<style>
...
<div class="virgo_container_sealock virgo_container_entity_odbiorca" style="border: none;">
...
//		$classNames["pozycja_dokumentu"] = "virgoPozycjaDokumentu";
		...
		$ancestors = array();
		$contextId = null;		
			$resultOdbiorca = virgoOdbiorca::createGuiAware();
			$contextId = $resultOdbiorca->getContextId();
			if (isset($contextId)) {
				if (virgoOdbiorca::getDisplayMode() != "CREATE" || R('portlet_action') == "Duplicate") {
					$resultOdbiorca->load($contextId);
				}
			}
?>
<?php
...
<?php
	...
	if (isset($masterPobId)) {
		...
		if ($className == "virgoOdbiorca") {
			$masterObject = new $className();
			$tmpId = $masterObject->getRemoteContextId($masterPobId);
			if (isset($tmpId)) {
				$resultOdbiorca = new virgoOdbiorca($tmpId);
				virgoOdbiorca::setDisplayMode("FORM");
			} else {
				$resultOdbiorca = new virgoOdbiorca();
				virgoOdbiorca::setDisplayMode("CREATE");
			}
		}
	} else {
		if (P('form_only', "0") == "5") {
			if (is_null($resultOdbiorca->getId())) { 
				if (P('only_private_records', "0") == "1") {
					$allPrivateRecords = $resultOdbiorca->selectAll();
					if (sizeof($allPrivateRecords) > 0) {
						$resultOdbiorca = new virgoOdbiorca($allPrivateRecords[0]['odb_id']);
						$resultOdbiorca->putInContext(false);
					} else {
						$resultOdbiorca = new virgoOdbiorca();
					}
				} else {
					...
					if (isset($customSQL) && trim($customSQL) != '') {
						...
						$records = $resultOdbiorca->selectAll($customSQL);
						if (sizeof($records) > 0) {
							$resultOdbiorca = new virgoOdbiorca($records[0]['odb_id']);
							$resultOdbiorca->putInContext(false);
						} else {
							$resultOdbiorca = new virgoOdbiorca();
						}
			...
			$resultOdbiorca = new virgoOdbiorca(virgoUser::getUserId());
			$resultOdbiorca->putInContext(false);
		}
	}
?>
...
?>
<?php
	$odbiorcaDisplayMode = virgoOdbiorca::getDisplayMode();
//	if ($odbiorcaDisplayMode == "" || $odbiorcaDisplayMode == "TABLE") {
//		$resultOdbiorca = $resultOdbiorca->portletActionForm();
//	}
?>
		<div class="form">
<?php
		$parentContextInfos = $resultOdbiorca->getParentsInContext();
...
?>
				...
				<td class="db_criteria_value">
					<button class="button btn btn-mini" onclick="
						this.form.virgo_filter_column.value='nip';
						this.form.portlet_action.value='RemoveCriterium';
						<?php echo JSFS() ?>
					">
<?php echo T('REMOVE_CRITERIA') ?>					</button>
...
PROFILE('token');
	if (isset($resultOdbiorca)) {
		$tmpId = is_array($resultOdbiorca) ? $resultOdbiorca['odb_id'] : $resultOdbiorca->getId();
		if (isset($tmpId)) {
?>
...
<?php
	if ($odbiorcaDisplayMode != "TABLE") {
		...
		if (
			...
			$pob = $resultOdbiorca->getMyPortletObject();
			...
			if (isset($invokedPortletId) && $invokedPortletId == $_SESSION['current_portlet_object_id'] && isset($reloadFromRequest) && $reloadFromRequest == "1") { 
				...
				if (P('form_only', "0") == "1" && isset($contextId)) {
					if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoOdbiorca'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
						require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoOdbiorca'.DIRECTORY_SEPARATOR.'create_store_message.php');
						$odbiorcaDisplayMode = "-empty-";
					}
				}
			}
		}
	}
if (!$resultOdbiorca->hideContentDueToNoParentSelected()) {
...
<script language="JavaScript">
  ...
  function confirmExit() {
	  ...
	  for (var i=0; i < checks.length; i++) {
		  ...
		  if (check.form.name == "virgo_form_odbiorca") {
			  if (check.value == 'T') {
...
?>
					<label 
						...
						for="odb_nazwa_<?php echo $resultOdbiorca->getId() ?>"
					>* <?php echo T('NAZWA') ?>
</label>
<?php
	if (!$formsInTable) {
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultOdbiorca->getNazwa(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="nazwa" name="odb_nazwa_<?php echo $resultOdbiorca->getId() ?>" value="<?php echo htmlentities($resultOdbiorca->getNazwa(), ENT_QUOTES, "UTF-8") ?>"> 
...
<?php
	if (P('show_form_nip', "1") == "1" || P('show_form_nip', "1") == "2") {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label 
						nowrap="nowrap"
						class="fieldlabel  <?php echo P('show_form_nip_obligatory', "0") == "1" ? " obligatory " : "" ?>   nip varchar" 
						for="odb_nip_<?php echo $resultOdbiorca->getId() ?>"
					> <?php echo P('show_form_nip_obligatory', "0") == "1" ? "* " : "" ?><?php echo T('NIP') ?>
</label>
<?php
	if (!$formsInTable) {
?>
...
?>
					<span align="left" nowrap>
<?php
	if (P('show_form_nip', "1") == "2") {
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultOdbiorca->getNip(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="nip" name="odb_nip_<?php echo $resultOdbiorca->getId() ?>" value="<?php echo htmlentities($resultOdbiorca->getNip(), ENT_QUOTES, "UTF-8") ?>"> 
...
?>
<?php
	if (false) { //$componentParamsZamowienie->get('show_table_status_zamowienia') == "1" && ($masterComponentName != "status_zamowienia" || is_null($contextId))) {
?>
...
<?php			
				...
				$resultsZamowienie = $tmpZamowienie->selectAll(' zmw_odb_id = ' . $resultOdbiorca->odb_id);
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsOdbiorca) * 0;
?>
<?php
...
?> 
						<input
							type="hidden"
							id="numer_<?php echo $resultOdbiorca->odb_id ?>" 
							name="numer_<?php echo $resultOdbiorca->odb_id ?>"
							value="<?php echo htmlentities($resultOdbiorca->odb_numer, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsOdbiorca) * 1;
?>
<?php
	if (!isset($resultZamowienie)) {
		$resultZamowienie = new sealock\virgoZamowienie();
...
?> 
						<input
							type="hidden"
							id="dataZlozenia_<?php echo $resultOdbiorca->odb_id ?>" 
							name="dataZlozenia_<?php echo $resultOdbiorca->odb_id ?>"
							value="<?php echo htmlentities($resultOdbiorca->odb_data_zlozenia, ENT_QUOTES, "UTF-8") ?>"
						>
...
//		$limit_status_zamowienia = $componentParams->get('limit_to_status_zamowienia');
		...
		$tmpId = sealock\virgoOdbiorca::getParentInContext("sealock\\virgoStatusZamowienia");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#odb_status_zamowienia_dropdown_<?php echo $resultOdbiorca->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
if (P('hide_audit', '0') == '0') {
	$showFieldset = 0;
	if ($resultOdbiorca->getDateCreated()) {
		if ($resultOdbiorca->getUsrCreatedId() == 0) {
			$username = "anonymous";
		} else {
			$createdByUser = new virgoUser((int)$resultOdbiorca->getUsrCreatedId());
			$username = $createdByUser->getUsername();
		}
		$createdOn = $resultOdbiorca->getDateCreated();
		$showFieldset = 1;
?>
		...
		<legend><?php echo T('AUDIT_DATA') ?>:</legend>
			<ul>
				<li>
					...
					<input type="hidden" id="usr_created_id" name="usr_created_id" value="<?php echo $resultOdbiorca->getUsrCreatedId() ?>"	>
				</li>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('CREATION_DATE') ?></span>
					<span class="readonly"><i><?php echo $createdOn ?></i></span>
					<input type="hidden" id="date_created" name="date_created" value="<?php echo $resultOdbiorca->getDateCreated() ?>"	>
				</li>
<?php
	}
?>
<?php
	if ($resultOdbiorca->getDateModified()) {
		if ($resultOdbiorca->getUsrModifiedId() == 0) {
			$username = "anonymous";
		} else {
			$modifiedByUser = new virgoUser((int)$resultOdbiorca->getUsrModifiedId());
			$username = $modifiedByUser->getUsername();
		}
		$modifiedOn = $resultOdbiorca->getDateModified();
?>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('MODIFIED_BY') ?></span>
					<span class="readonly"><b><?php echo $username ?></b></span>
					<input type="hidden" id="usr_modified_id" name="usr_modified_id" value="<?php echo $resultOdbiorca->getUsrModifiedId() ?>"	>
				</li>
				<li>
					...
					<input type="hidden" id="date_modified" name="date_modified" value="<?php echo $resultOdbiorca->getDateModified() ?>"	>
				</li>
<?php
	}
	if ($showFieldset == 1) {
...
?>
				...
				<div class="buttons form-actions">
						<input type="text" name="odb_id_<?php echo $this->getId() ?>" id="odb_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
 ...
 <div class="button_wrapper button_wrapper_store inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'odb_id_<?php echo $this->getId() ?>', '<?php echo isset($resultOdbiorca) ? (is_array($resultOdbiorca) ? $resultOdbiorca['odb_id'] : $resultOdbiorca->getId()) : '' ?>');
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("ODBIORCA"), "\\'".rawurlencode($resultOdbiorca->getVirgoTitle())."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
...
// document.forms['portlet_form_<?php echo $this->getId() ?>'].elements['odb_nazwa_<?php echo $resultOdbiorca->getId() ?>'].focus();
</script>
	</div>
<?php
/* MILESTONE 1.2 Create */
	} elseif ($odbiorcaDisplayMode == "CREATE") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
...
?>
<?php
		if (isset($resultOdbiorca->odb_id)) {
			$resultOdbiorca->odb_id = null;
		}
		$parentAjaxRendered = "0";
?>
<?php
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "nazwa") {
				$resultOdbiorca->setNazwa($virgoSelectedDay);
			}
		}
	}
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "nip") {
				$resultOdbiorca->setNip($virgoSelectedDay);
			}
		}
	}
...
/* MILESTONE 1.3 Search */
	} elseif ($odbiorcaDisplayMode == "SEARCH") {
?>
...
?>
		...
		<span class="virgo_search_empty_values" align="left" nowrap style="font-size: 0.7em;">
			<?php echo T('VALUE') ?>			<select 
				id="virgo_search_nip_is_null" 
				name="virgo_search_nip_is_null"
			>
				<option value=""
<?php
		if (isset($fieldCriteriaNip) && $fieldCriteriaNip["is_null"] == 0) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="not_null"
<?php
		if (isset($fieldCriteriaNip) && $fieldCriteriaNip["is_null"] == 1) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="null"
<?php
		if (isset($fieldCriteriaNip) && $fieldCriteriaNip["is_null"] == 2) {
?>
				selected="selected"
<?php
		}
...
?>
<?php
	$record = new sealock\virgoOdbiorca();
	$recordId = is_array($resultOdbiorca) ? $resultOdbiorca['odb_id'] : $resultOdbiorca->odb_id;
...
<?php
/* MILESTONE 1.4 View */
	} elseif ($odbiorcaDisplayMode == "VIEW") {
?>
	<div class="form_view">
<?php
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("ODBIORCA"), "\\'".$resultOdbiorca->getVirgoTitle()."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'odb_id_<?php echo $this->getId() ?>', '<?php echo isset($resultOdbiorca) ? (is_array($resultOdbiorca) ? $resultOdbiorca['odb_id'] : $resultOdbiorca->getId()) : '' ?>');
...
<?php
/* MILESTONE 1.5 Table */
	} elseif ($odbiorcaDisplayMode == "TABLE") {
PROFILE('TABLE');
		if (P('form_only') == "3") {
?>
<?php
	...
	$resultCount = -1;
	$filterApplied = false;
	$resultOdbiorca->setShowPage(1); 
	$resultOdbiorca->setShowRows('all'); 	
	$resultsOdbiorca = $resultOdbiorca->getTableData($resultCount, $filterApplied);
	$events = array();
	foreach ($resultsOdbiorca as $resultOdbiorca) {
		if (isset($resultOdbiorca[$eventColumn]) && isset($events[substr($resultOdbiorca[$eventColumn], 0, 10)])) {
			$eventsInDay = $events[substr($resultOdbiorca[$eventColumn], 0, 10)];
		} else {
			$eventsInDay = array();
		}
		$eventObject = new virgoOdbiorca($resultOdbiorca['odb_id']);
		$eventsInDay[] = $eventObject;
		$events[substr($resultOdbiorca[$eventColumn], 0, 10)] = $eventsInDay;
	}
?>
<style type="text/css">
...
?>			
			...
			<tr class='virgo_calendar_week'>
				...
				<input type='hidden' name='odb_id_<?php echo $this->getId() ?>' value=''/>
<?php
	$tmpWeekDay = 1;
	$safety = 0;
	if ($tmpDay["wday"] > 0) {
...
<?php
		...
		if (isset($events[$tmpStringDay])) {
			...
			foreach ($eventsInDay as $resultOdbiorca) {
?>
<?php
PROFILE('token');
	if (isset($resultOdbiorca)) {
...
<div class="button_wrapper button_wrapper_view inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'odb_id_<?php echo $this->getId() ?>', '<?php echo isset($resultOdbiorca) ? (is_array($resultOdbiorca) ? $resultOdbiorca['odb_id'] : $resultOdbiorca->getId()) : '' ?>');
 								...
 								form.target = '';
							" 
							value="<?php echo T($resultOdbiorca->getVirgoTitle()) ?>"
						/><div class="button_right"></div></div><?php
	}
?>
<?php				
//				echo "<span class='virgo_calendar_event' onclick='var form=document.getElementById(\"portlet_form_".$this->getId()."\");form.portlet_action.value=\"View\";form.odb_id_".$this->getId().".value=\"".$eventInDay->getId()."\";form.submit();'>" . $eventInDay->getVirgoTitle() . "</span>";
			}
		}
		echo "</td>";
...
?>
		<script type="text/javascript">
			var odbiorcaChildrenDivOpen = '';
			
			function childrenButtonClicked(clickedDivId) {
				var div = document.getElementById(clickedDivId);
				if (clickedDivId == odbiorcaChildrenDivOpen) {
					div.style.display = 'none';
					odbiorcaChildrenDivOpen = '';
				} else {
					if (odbiorcaChildrenDivOpen != '') {
						document.getElementById(odbiorcaChildrenDivOpen).style.display = 'none';
					}
					div.style.display = 'block';
					odbiorcaChildrenDivOpen = clickedDivId;
				}
			}
...
PROFILE('table_02');
	...
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoOdbiorca'.DIRECTORY_SEPARATOR.'controller.php')) require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoOdbiorca'.DIRECTORY_SEPARATOR.'controller.php');
			$showPage = $resultOdbiorca->getShowPage(); 
			$showRows = $resultOdbiorca->getShowRows(); 
?>
						<input type="text" name="odb_id_<?php echo $this->getId() ?>" id="odb_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>			
				<tr><td colspan="99" class="nav-header"><?php echo T('Odbiorcy') ?></td></tr>
<?php
			}
?>			
<?php
PROFILE('table_02');
PROFILE('main select');
			$virgoOrderColumn = $resultOdbiorca->getOrderColumn();
			$virgoOrderMode = $resultOdbiorca->getOrderMode();
			$resultCount = -1;
			$filterApplied = false;
			$resultsOdbiorca = $resultOdbiorca->getTableData($resultCount, $filterApplied);
PROFILE('main select');
...
<?php
	$oc = $resultOdbiorca->getOrderColumn(); 
	$om = $resultOdbiorca->getOrderMode();
...
?>
	...
	_pSF(form, 'virgo_order_column', 'odb_nazwa');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('NAZWA') ?>							<?php echo ($oc == "odb_nazwa" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoOdbiorca::getLocalSessionValue('VirgoFilterNazwa', null);
?>
						<input
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'odb_nip');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('NIP') ?>							<?php echo ($oc == "odb_nip" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoOdbiorca::getLocalSessionValue('VirgoFilterNip', null);
?>
						<input
							name="virgo_filter_nip"
							class="virgo_filter"
							onChange="<?php echo JSFS(null, "Submit", true, array(), false, "SetVirgoTableFilter") ?>"
							value="<?php echo $tableFilter ?>"
						/>
...
<?php
			...
			if ($resultCount != 0) {
				if (((int)$showRows) * (((int)$showPage)-1) == $resultCount ) {
					...
					$resultOdbiorca->setShowPage($showPage);
				}
				$index = 0;
PROFILE('table_04');
...
?>
<?php
$fileNameToInclude = PORTAL_PATH . "/portlets/sealock/virgoOdbiorca/modules/renderTableRow_{$_SESSION['current_portlet_object_id']}.php";
...
<?php
			if (P('form_only') != "4" || (P('chessboard_width') != 0 && $index % P('chessboard_width') == 1)) {
				if (is_null($firstRowId)) {
					$firstRowId = $resultOdbiorca['odb_id'];
				}
				$displayClass = ' displayClass ';
				$tmpContextId = virgoOdbiorca::getContextId();
				if (is_null($tmpContextId)) {
					$forceContextOnFirstRow = P('force_context_on_first_row', "1");
					if ($forceContextOnFirstRow == "1") {
						virgoOdbiorca::setContextId($resultOdbiorca['odb_id'], false);
						$tmpContextId = $resultOdbiorca['odb_id'];
					}
				}
				if (isset($tmpContextId) && $resultOdbiorca['odb_id'] == $tmpContextId) {
					if (P('form_only') != "4") {
...
?>
			<tr 
				id="<?php echo $this->getId() ?>_<?php echo isset($resultOdbiorca['odb_id']) ? $resultOdbiorca['odb_id'] : "" ?>" 
...
?>
				<input type="checkbox" class="checkbox" style="float: right;" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultOdbiorca['odb_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
				<input type="checkbox" class="checkbox" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultOdbiorca['odb_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
			<a  
				...
				onclick="
					...
					form.portlet_action.value='View'; 
					form.odb_id_<?php echo $this->getId() ?>.value='<?php echo isset($resultOdbiorca['odb_id']) ? $resultOdbiorca['odb_id'] : '' ?>'; 
					form.target = ''; 
<?php
					if (isset($ajax) && !$ajax) {
?>					
...
?>
		<span 
			class="<?php echo 'displayClass' ?>">
				<?php echo htmlentities($resultOdbiorca['odb_nazwa'], ENT_QUOTES, "UTF-8") ?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
?>
...
?>
		<span 
			class="<?php echo 'displayClass' ?>">
				<?php echo htmlentities($resultOdbiorca['odb_nip'], ENT_QUOTES, "UTF-8") ?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
?>
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("ODBIORCA"), "\\'".rawurlencode($resultOdbiorca['odb_virgo_title'])."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'odb_id_<?php echo $this->getId() ?>', '<?php echo isset($resultOdbiorca) ? (is_array($resultOdbiorca) ? $resultOdbiorca['odb_id'] : $resultOdbiorca->getId()) : '' ?>');
...
<?php
	...
	if (is_null($contextRowIdInTable)) {
		$forceContextOnFirstRow = P('force_context_on_first_row', "1");
		if ($forceContextOnFirstRow == "1") {
			virgoOdbiorca::setContextId($firstRowId, false);
			if (P('form_only') != "4") {
?>
<script type="text/javascript">
...
<?php
				...
				unset($resultOdbiorca);
				unset($resultsOdbiorca);
				if (isset($contextIdOwn) && trim($contextIdOwn) != "") {
					if ($contextIdConfirmed == false) {
						$tmpOdbiorca = new virgoOdbiorca();
						$tmpCount = $tmpOdbiorca->getAllRecordCount(' odb_id = ' . $contextIdOwn);
						if ($tmpCount == 0) {
							virgoOdbiorca::clearRemoteContextId($tabModeEditMenu);
						}
					}
				}
PROFILE('rows rendering');
...
?>
		$.getJSON('<?php echo $page->getUrl() ?>?portlet_action=SelectJson&odb_id_<?php echo $this->getId() ?>=' + virgoId + '&invoked_portlet_object_id=<?php echo $this->getId() ?>&virgo_action_mode_json=T&_virgo_ajax=1', function(data) {
<?php
			if (P('form_only') != "4") {
?>
...
?>
		...
		form.odb_id_<?php echo $this->getId() ?>.value=virgoId; 
		form.target = ''; 
<?php
...
<div class="button_wrapper button_wrapper_report inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								...
								for(var i = 0; i< children.length;i++) {
								  if (children[i].getAttribute('name') == 'odb_id_<?php echo $this->getId() ?>') {
									found = 1;
								  }
								}
								if (found == 0) {
								  ...
								  input.setAttribute('name', 'odb_id_<?php echo $this->getId() ?>');
								  input.setAttribute('value', '');
								  form.appendChild(input);
								} else {
	_pSF(form, 'odb_id_<?php echo $this->getId() ?>', '');
								}
								form.action = '';
...
<?php
					$sessionSeparator = virgoOdbiorca::getImportFieldSeparator();
...
<div class="button_wrapper button_wrapper_deleteselected inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								if (!nothingSelected(this.form, <?php echo $this->getId() ?>)) {
									if (!confirm(decodeURIComponent('<?php echo T('ARE_YOU_SURE_YOU_WANT_REMOVE', T('ODBIORCY'), "") ?>'))) return false;
								} else {
									var alertText = '<?php echo T('NOTHING_SELECTED') ?>';
									if (alertText.charAt(alertText.length-1)=='?') {
										if (!confirm(alertText)) return false;
...
/* MILESTONE 1.6 TableForm */
	} elseif ($odbiorcaDisplayMode == "TABLE_FORM") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
	...
	<form method="post" style="display: inline;" action="" id="virgo_form_odbiorca" name="virgo_form_odbiorca" enctype="multipart/form-data">
						<input type="text" name="odb_id_<?php echo $this->getId() ?>" id="odb_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
<?php			
				$resultsOdbiorca = $resultOdbiorca->getRecordsToEdit();
				$idsToCorrect = $resultOdbiorca->getInvalidRecords();
				$index = 0;
PROFILE('rows rendering');
				foreach ($resultsOdbiorca as $resultOdbiorca) {
					$index = $index + 1;
?>
			<tr id="virgo_tr_id_<?php echo $resultOdbiorca->odb_id ?>" class="<?php echo ($index % 2 == 0 ? "data_table_even" : "data_table_odd") ?>"
<?php
	if ($resultOdbiorca->odb_id == 0 && R('virgo_validate_new', "N") == "N") {
?>
		style="display: none;"
<?php
	}
...
?> 
						<input
							type="hidden"
							id="nazwa_<?php echo $resultOdbiorca->odb_id ?>" 
							name="nazwa_<?php echo $resultOdbiorca->odb_id ?>"
							value="<?php echo htmlentities($resultOdbiorca->odb_nazwa, ENT_QUOTES, "UTF-8") ?>"
						>
...
?> 
						<input
							type="hidden"
							id="nip_<?php echo $resultOdbiorca->odb_id ?>" 
							name="nip_<?php echo $resultOdbiorca->odb_id ?>"
							value="<?php echo htmlentities($resultOdbiorca->odb_nip, ENT_QUOTES, "UTF-8") ?>"
						>
...
?>
				<td>
<?php
	if (isset($idsToCorrect[$resultOdbiorca->odb_id])) {
		$errorMessage = $idsToCorrect[$resultOdbiorca->odb_id];
?>
					<div class="error">
						<?php echo $errorMessage ?>
					</div>
...
?>
		<div class="<?php echo $odbiorcaDisplayMode ?>">
<?php
?>
			<div class="buttons form-actions">
<?php
...
<div style="display: none; background-color:#FFFFFF; border:1px solid #000000; font-size:10px; margin:10px 0; padding:10px;"; id="extraFilesInfo_slk_odbiorca" style="font-size: 12px; " onclick="document.getElementById('extraFilesInfo_slk_odbiorca').style.display='none';">
...
<?php
	$infos = virgoOdbiorca::getExtraFilesInfo();
