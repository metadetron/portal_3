<?php
	...
	use sealock\virgoPozycjaBilansu;
...
//	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoBilansOtwarcia'.DIRECTORY_SEPARATOR.'controller.php')) require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoBilansOtwarcia'.DIRECTORY_SEPARATOR.'controller.php');
	$componentParams = null; //&JComponentHelper::getParams('com_slk_pozycja_bilansu');
...
?>
<link rel="stylesheet" href="<?php echo $live_site ?>/components/com_slk_pozycja_bilansu/sealock.css" type="text/css" /> 
<?php
	}
?>
<?php
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoPozycjaBilansu'.DIRECTORY_SEPARATOR.'slk_pbl.css')) {
?>
<link rel="stylesheet" href="<?php echo $_SESSION['portal_url'] ?>/portlets/sealock/virgoPozycjaBilansu/slk_pbl.css" type="text/css" /> 
<?php
	}
?>
<style>
...
<div class="virgo_container_sealock virgo_container_entity_pozycja_bilansu" style="border: none;">
...
//		$classNames["pozycja_dokumentu"] = "virgoPozycjaDokumentu";
		...
		$ancestors = array();
		$ancestors["bilans_otwarcia"] = "TRUE";
		$ancestors["magazyn"] = "TRUE";
		$ancestors["towar"] = "TRUE";
		$contextId = null;		
			$resultPozycjaBilansu = virgoPozycjaBilansu::createGuiAware();
			$contextId = $resultPozycjaBilansu->getContextId();
			if (isset($contextId)) {
				if (virgoPozycjaBilansu::getDisplayMode() != "CREATE" || R('portlet_action') == "Duplicate") {
					$resultPozycjaBilansu->load($contextId);
				}
			}
?>
<?php
...
<?php
	...
	if (isset($masterPobId)) {
		...
		if ($className == "virgoPozycjaBilansu") {
			$masterObject = new $className();
			$tmpId = $masterObject->getRemoteContextId($masterPobId);
			if (isset($tmpId)) {
				$resultPozycjaBilansu = new virgoPozycjaBilansu($tmpId);
				virgoPozycjaBilansu::setDisplayMode("FORM");
			} else {
				$resultPozycjaBilansu = new virgoPozycjaBilansu();
				virgoPozycjaBilansu::setDisplayMode("CREATE");
			}
		}
	} else {
		if (P('form_only', "0") == "5") {
			if (is_null($resultPozycjaBilansu->getId())) { 
				if (P('only_private_records', "0") == "1") {
					$allPrivateRecords = $resultPozycjaBilansu->selectAll();
					if (sizeof($allPrivateRecords) > 0) {
						$resultPozycjaBilansu = new virgoPozycjaBilansu($allPrivateRecords[0]['pbl_id']);
						$resultPozycjaBilansu->putInContext(false);
					} else {
						$resultPozycjaBilansu = new virgoPozycjaBilansu();
					}
				} else {
					...
					if (isset($customSQL) && trim($customSQL) != '') {
						...
						$records = $resultPozycjaBilansu->selectAll($customSQL);
						if (sizeof($records) > 0) {
							$resultPozycjaBilansu = new virgoPozycjaBilansu($records[0]['pbl_id']);
							$resultPozycjaBilansu->putInContext(false);
						} else {
							$resultPozycjaBilansu = new virgoPozycjaBilansu();
						}
			...
			$resultPozycjaBilansu = new virgoPozycjaBilansu(virgoUser::getUserId());
			$resultPozycjaBilansu->putInContext(false);
		}
	}
?>
...
?>
<?php
	$pozycjaBilansuDisplayMode = virgoPozycjaBilansu::getDisplayMode();
//	if ($pozycjaBilansuDisplayMode == "" || $pozycjaBilansuDisplayMode == "TABLE") {
//		$resultPozycjaBilansu = $resultPozycjaBilansu->portletActionForm();
//	}
?>
		<div class="form">
<?php
		$parentContextInfos = $resultPozycjaBilansu->getParentsInContext();
...
?>			
			<tr>
				<td class="db_criteria_label" nowrap="nowrap">
					<?php echo T('Ilośc!') ?> <?php echo T(' ') ?>				</td>
...
?>
			<tr>
				<td class="db_criteria_label" nowrap="nowrap">
					<?php echo T('Bilans otwarcia') ?> <?php echo T('') ?>				</td>
				<td class="db_criteria_value">
					<?php echo T('EMPTY_VALUE') ?>				</td>
				<td class="db_criteria_value">
					<button class="button btn btn-mini" onclick="
						this.form.virgo_filter_column.value='bilans_otwarcia';
						this.form.portlet_action.value='RemoveCriterium';
						<?php echo JSFS() ?>
					">
<?php echo T('REMOVE_CRITERIA') ?>					</button>
...
?>
			<tr>
				<td class="db_criteria_label" nowrap="nowrap">
					<?php echo T('bilans_otwarcia') ?> <?php echo T('') ?>				</td>
				<td class="db_criteria_value">
					<?php echo $renderCriteria ?>
				</td>
				<td class="db_criteria_value">
...
PROFILE('token');
	if (isset($resultPozycjaBilansu)) {
		$tmpId = is_array($resultPozycjaBilansu) ? $resultPozycjaBilansu['pbl_id'] : $resultPozycjaBilansu->getId();
		if (isset($tmpId)) {
?>
...
?>
<?php
	if ($pozycjaBilansuDisplayMode != "TABLE") {
		...
		if (
			...
			|| $tmpAction == "StoreNewBilansOtwarcia"
		) {
			...
			$pob = $resultPozycjaBilansu->getMyPortletObject();
			...
			if (isset($invokedPortletId) && $invokedPortletId == $_SESSION['current_portlet_object_id'] && isset($reloadFromRequest) && $reloadFromRequest == "1") { 
				...
				$resultPozycjaBilansu->loadFromRequest();
			} else {
				if (P('form_only', "0") == "1" && isset($contextId)) {
					if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoPozycjaBilansu'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
						require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoPozycjaBilansu'.DIRECTORY_SEPARATOR.'create_store_message.php');
						$pozycjaBilansuDisplayMode = "-empty-";
					}
				}
			}
		}
	}
if (!$resultPozycjaBilansu->hideContentDueToNoParentSelected()) {
...
/* MILESTONE 1.1 Form */
	...
	if ($pozycjaBilansuDisplayMode == "FORM") {
?>
<script language="JavaScript">
  ...
  function confirmExit() {
	  ...
	  for (var i=0; i < checks.length; i++) {
		  ...
		  if (check.form.name == "virgo_form_pozycja_bilansu") {
			  if (check.value == 'T') {
	...
	<div class="form_edit">
			<fieldset class="form">
				<legend>
<?php echo T('POZYCJA_BILANSU') ?>:</legend>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label align="right" nowrap class="fieldlabel  obligatory " for="pbl_towar_<?php echo isset($resultPozycjaBilansu->pbl_id) ? $resultPozycjaBilansu->pbl_id : '' ?>">
 *
...
//		$limit_towar = $componentParams->get('limit_to_towar');
		$limit_towar = null;
		$tmpId = sealock\virgoPozycjaBilansu::getParentInContext("sealock\\virgoTowar");
		$readOnly = "";
...
<div class="button_wrapper button_wrapper_addtowar inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '<?php echo isset($resultPozycjaBilansu) ? (is_array($resultPozycjaBilansu) ? $resultPozycjaBilansu['pbl_id'] : $resultPozycjaBilansu->getId()) : '' ?>');
...
?>						
<script type="text/javascript">
$('#pbl_towar_dropdown_<?php echo $resultPozycjaBilansu->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["twr"])) {
		...
		$parentValue = $resultPozycjaBilansu->pbl_twr_id;
	}
	
?>
				<input type="hidden" id="pbl_towar_<?php echo $resultPozycjaBilansu->pbl_id ?>" name="pbl_towar_<?php echo $resultPozycjaBilansu->pbl_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<?php
	if (class_exists('sealock\virgoBilansOtwarcia') && ((P('show_form_bilans_otwarcia', "1") == "1" || P('show_form_bilans_otwarcia', "1") == "2" || P('show_form_bilans_otwarcia', "1") == "3") && !isset($context["bot"]))) {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<label align="right" nowrap class="fieldlabel  obligatory " for="pbl_bilansOtwarcia_<?php echo isset($resultPozycjaBilansu->pbl_id) ? $resultPozycjaBilansu->pbl_id : '' ?>">
 *
<?php $customLabel = TE('BILANS_OTWARCIA_'); echo !is_null($customLabel) ? $customLabel : T('BILANS_OTWARCIA') . ' ' . T('') ?> 
					</label>
<?php
	if (!$formsInTable) {
?>
...
//		$limit_bilans_otwarcia = $componentParams->get('limit_to_bilans_otwarcia');
		...
		$tmpId = sealock\virgoPozycjaBilansu::getParentInContext("sealock\\virgoBilansOtwarcia");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#pbl_bilans_otwarcia_dropdown_<?php echo $resultPozycjaBilansu->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
?>
<?php
	if (isset($context["bot"])) {
		$parentValue = $context["bot"];
	} else {
		$parentValue = $resultPozycjaBilansu->pbl_bot_id;
	}
	
?>
				<input type="hidden" id="pbl_bilansOtwarcia_<?php echo $resultPozycjaBilansu->pbl_id ?>" name="pbl_bilansOtwarcia_<?php echo $resultPozycjaBilansu->pbl_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<?php
...
?>
					<label 
						...
						for="pbl_ilosc_<?php echo $resultPozycjaBilansu->getId() ?>"
					>* <?php echo T('ILOSC') ?>
</label>
<?php
	if (!$formsInTable) {
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo number_format($resultPozycjaBilansu->getIlosc(), 2, ',', ' ') ?>
						&nbsp;</span>
						<input type="hidden" id="ilosc" name="pbl_ilosc_<?php echo $resultPozycjaBilansu->getId() ?>" value="<?php echo htmlentities($resultPozycjaBilansu->getIlosc(), ENT_QUOTES, "UTF-8") ?>"> 
...
if (P('hide_audit', '0') == '0') {
	$showFieldset = 0;
	if ($resultPozycjaBilansu->getDateCreated()) {
		if ($resultPozycjaBilansu->getUsrCreatedId() == 0) {
			$username = "anonymous";
		} else {
			$createdByUser = new virgoUser((int)$resultPozycjaBilansu->getUsrCreatedId());
			$username = $createdByUser->getUsername();
		}
		$createdOn = $resultPozycjaBilansu->getDateCreated();
		$showFieldset = 1;
?>
		...
		<legend><?php echo T('AUDIT_DATA') ?>:</legend>
			<ul>
				<li>
					...
					<input type="hidden" id="usr_created_id" name="usr_created_id" value="<?php echo $resultPozycjaBilansu->getUsrCreatedId() ?>"	>
				</li>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('CREATION_DATE') ?></span>
					<span class="readonly"><i><?php echo $createdOn ?></i></span>
					<input type="hidden" id="date_created" name="date_created" value="<?php echo $resultPozycjaBilansu->getDateCreated() ?>"	>
				</li>
<?php
	}
?>
<?php
	if ($resultPozycjaBilansu->getDateModified()) {
		if ($resultPozycjaBilansu->getUsrModifiedId() == 0) {
			$username = "anonymous";
		} else {
			$modifiedByUser = new virgoUser((int)$resultPozycjaBilansu->getUsrModifiedId());
			$username = $modifiedByUser->getUsername();
		}
		$modifiedOn = $resultPozycjaBilansu->getDateModified();
?>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('MODIFIED_BY') ?></span>
					<span class="readonly"><b><?php echo $username ?></b></span>
					<input type="hidden" id="usr_modified_id" name="usr_modified_id" value="<?php echo $resultPozycjaBilansu->getUsrModifiedId() ?>"	>
				</li>
				<li>
					...
					<input type="hidden" id="date_modified" name="date_modified" value="<?php echo $resultPozycjaBilansu->getDateModified() ?>"	>
				</li>
<?php
	}
	if ($showFieldset == 1) {
...
?>
				...
				<div class="buttons form-actions">
						<input type="text" name="pbl_id_<?php echo $this->getId() ?>" id="pbl_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("POZYCJA_BILANSU"), "\\'".rawurlencode($resultPozycjaBilansu->getVirgoTitle())."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
...
// document.forms['portlet_form_<?php echo $this->getId() ?>'].elements['pbl_towar_<?php echo $resultPozycjaBilansu->getId() ?>'].focus();
</script>
	</div>
<?php
/* MILESTONE 1.2 Create */
	} elseif ($pozycjaBilansuDisplayMode == "CREATE") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
...
?>
<?php
		if (isset($resultPozycjaBilansu->pbl_id)) {
			$resultPozycjaBilansu->pbl_id = null;
		}
		$parentAjaxRendered = "0";
?>
<?php
...
if (isset($defaultValue) && trim($defaultValue) != "") {
	...
	$resultPozycjaBilansu->setTwrId($defaultValue);
}
$defaultValue = P('create_default_value_bilans_otwarcia');
if (isset($defaultValue) && trim($defaultValue) != "") {
	if ($defaultValue == -1) {
		$loggedInUserId = virgoUser::getUserId();
	...
	$resultPozycjaBilansu->setBotId($defaultValue);
}
	}
?>
																																					<?php
...
<?php
	...
	$parentCount = sealock\virgoTowar::getVirgoListSize($whereList);
	$showAjaxpbl = P('show_create_towar', "1") == "3" || $parentCount > 100;
	if (!$showAjaxpbl) {
?>
    						<select 
							class="inputbox  obligatory " 
...
?>
<?php
	if (class_exists('sealock\virgoBilansOtwarcia') && ((P('show_create_bilans_otwarcia', "1") == "1" || P('show_create_bilans_otwarcia', "1") == "2" || P('show_create_bilans_otwarcia', "1") == "3") && !isset($context["bot"]))) {
?>
<?php
	if (!$formsInTable) {
?>
...
//		$limit_bilans_otwarcia = $componentParams->get('limit_to_bilans_otwarcia');
		...
		$tmpId = sealock\virgoPozycjaBilansu::getParentInContext("sealock\\virgoBilansOtwarcia");
		$readOnly = "";
		if (isset($tmpId) || P('show_create_bilans_otwarcia', "1") == "2") {
// to powinno bylo juz byc zrobione w createGuiAware()			
//			if (!is_null($tmpId)) {
//				$resultPozycjaBilansu->pbl_bot__id = $tmpId;
//			}
...
<?php
	...
	$parentCount = sealock\virgoBilansOtwarcia::getVirgoListSize($whereList);
	$showAjaxpbl = P('show_create_bilans_otwarcia', "1") == "3" || $parentCount > 100;
	if (!$showAjaxpbl) {
?>
    						<select 
							class="inputbox  obligatory " 
...
?>
					<label nowrap class="fieldlabel  obligatory " for="pbl_ilosc_<?php echo $resultPozycjaBilansu->getId() ?>">
* <?php echo T('ILOSC') ?>
					</label>
<?php
	if (!$formsInTable) {
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "ilosc") {
				$resultPozycjaBilansu->setIlosc($virgoSelectedDay);
			}
		}
	}
...
/* MILESTONE 1.3 Search */
	} elseif ($pozycjaBilansuDisplayMode == "SEARCH") {
?>
...
<?php
	if (P('show_search_towar', '1') == "1") {
		if (isset($criteriaPozycjaBilansu["towar"])) {
			$fieldCriteriaTowar = $criteriaPozycjaBilansu["towar"];
		}
?>
				<li
					<?php echo (!$formsInTable && $floatingFields) ? "style='display: inline-block; float: left;'" : '' ?>
...
?>
<?php
	if (P('show_search_bilans_otwarcia', '1') == "1") {
		if (isset($criteriaPozycjaBilansu["bilans_otwarcia"])) {
			$fieldCriteriaBilansOtwarcia = $criteriaPozycjaBilansu["bilans_otwarcia"];
		}
?>
				<li
					<?php echo (!$formsInTable && $floatingFields) ? "style='display: inline-block; float: left;'" : '' ?>
				...
				<label align="right" nowrap class="fieldlabel"
					<?php echo (!$formsInTable && $floatingFields) ? "style='width: inherit;'" : '' ?>				
				><?php echo T('BILANS_OTWARCIA') ?> <?php echo T('') ?></label>
<?php
	$value = isset($fieldCriteriaBilansOtwarcia["value"]) ? $fieldCriteriaBilansOtwarcia["value"] : null;
?>
    <input type="text" class="inputbox " id="virgo_search_bilansOtwarcia" name="virgo_search_bilansOtwarcia" value="<?php echo $value ?>">
</span>
<?php
	if (P('empty_values_search', '0') == "1") {
?>
			...
			>
				<option value=""
<?php
		if (isset($fieldCriteriaBilansOtwarcia) && $fieldCriteriaBilansOtwarcia["is_null"] == 0) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="not_null"
<?php
		if (isset($fieldCriteriaBilansOtwarcia) && $fieldCriteriaBilansOtwarcia["is_null"] == 1) {
?>
				selected="selected"
<?php
		}
?>
				...
				<option value="null"
<?php
		if (isset($fieldCriteriaBilansOtwarcia) && $fieldCriteriaBilansOtwarcia["is_null"] == 2) {
?>
				selected="selected"
<?php
		}
...
<?php
/* MILESTONE 1.4 View */
	} elseif ($pozycjaBilansuDisplayMode == "VIEW") {
?>
	<div class="form_view">
<?php
...
<?php
			...
			$readOnly = "";
			if ($resultPozycjaBilansu->getId() == 0) {
// przesuac do createforgui				$resultPozycjaBilansu->pbl_twr__id = $tmpId;
			}
			$parentId = $resultPozycjaBilansu->getTowarId();
			$parentValue = sealock\virgoTowar::lookup($parentId);
				
?>
					<input
						type="hidden"
						id="pbl_towar_<?php echo isset($resultPozycjaBilansu->pbl_id) ? $resultPozycjaBilansu->pbl_id : '' ?>" 
						name="pbl_towar_<?php echo isset($resultPozycjaBilansu->pbl_id) ? $resultPozycjaBilansu->pbl_id : '' ?>" 						
...
?>
<?php
	if (class_exists('sealock\virgoBilansOtwarcia') && P('show_view_bilans_otwarcia', '1') == "1") { // && (isset($masterComponentName) && $masterComponentName != "bilans_otwarcia" || is_null($contextId))) {
?>
<?php
	if (!$formsInTable) {
?>
...
?>
					<span align="right" nowrap class="fieldlabel">Bilans otwarcia </span>
<?php
	if (!$formsInTable) {
?>
<?php
...
<?php
			$tmpId = null;
			if (!is_null($context) && isset($context['bot_id'])) {
				$tmpId = $context['bot_id'];
			}
			$readOnly = "";
			if ($resultPozycjaBilansu->getId() == 0) {
// przesuac do createforgui				$resultPozycjaBilansu->pbl_bot__id = $tmpId;
			}
			$parentId = $resultPozycjaBilansu->getBilansOtwarciaId();
			$parentValue = sealock\virgoBilansOtwarcia::lookup($parentId);
				
?>
					<input
						type="hidden"
						id="pbl_bilansOtwarcia_<?php echo isset($resultPozycjaBilansu->pbl_id) ? $resultPozycjaBilansu->pbl_id : '' ?>" 
						name="pbl_bilansOtwarcia_<?php echo isset($resultPozycjaBilansu->pbl_id) ? $resultPozycjaBilansu->pbl_id : '' ?>" 						
						value="<?php echo $parentId ?>"
					/>
					<span class="inputbox readonly"><?php echo $parentValue ?></span>
<?php
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("POZYCJA_BILANSU"), "\\'".$resultPozycjaBilansu->getVirgoTitle()."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '<?php echo isset($resultPozycjaBilansu) ? (is_array($resultPozycjaBilansu) ? $resultPozycjaBilansu['pbl_id'] : $resultPozycjaBilansu->getId()) : '' ?>');
...
<?php
/* MILESTONE 1.5 Table */
	} elseif ($pozycjaBilansuDisplayMode == "TABLE") {
PROFILE('TABLE');
		if (P('form_only') == "3") {
?>
<?php
	...
	$resultCount = -1;
	$filterApplied = false;
	$resultPozycjaBilansu->setShowPage(1); 
	$resultPozycjaBilansu->setShowRows('all'); 	
	$resultsPozycjaBilansu = $resultPozycjaBilansu->getTableData($resultCount, $filterApplied);
	$events = array();
	foreach ($resultsPozycjaBilansu as $resultPozycjaBilansu) {
		if (isset($resultPozycjaBilansu[$eventColumn]) && isset($events[substr($resultPozycjaBilansu[$eventColumn], 0, 10)])) {
			$eventsInDay = $events[substr($resultPozycjaBilansu[$eventColumn], 0, 10)];
		} else {
			$eventsInDay = array();
		}
		$eventObject = new virgoPozycjaBilansu($resultPozycjaBilansu['pbl_id']);
		$eventsInDay[] = $eventObject;
		$events[substr($resultPozycjaBilansu[$eventColumn], 0, 10)] = $eventsInDay;
	}
?>
<style type="text/css">
...
?>			
			...
			<tr class='virgo_calendar_week'>
				...
				<input type='hidden' name='pbl_id_<?php echo $this->getId() ?>' value=''/>
<?php
	$tmpWeekDay = 1;
	$safety = 0;
	if ($tmpDay["wday"] > 0) {
...
<?php
		...
		if (isset($events[$tmpStringDay])) {
			...
			foreach ($eventsInDay as $resultPozycjaBilansu) {
?>
<?php
PROFILE('token');
	if (isset($resultPozycjaBilansu)) {
...
<div class="button_wrapper button_wrapper_view inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '<?php echo isset($resultPozycjaBilansu) ? (is_array($resultPozycjaBilansu) ? $resultPozycjaBilansu['pbl_id'] : $resultPozycjaBilansu->getId()) : '' ?>');
 								...
 								form.target = '';
							" 
							value="<?php echo T($resultPozycjaBilansu->getVirgoTitle()) ?>"
						/><div class="button_right"></div></div><?php
	}
?>
<?php				
//				echo "<span class='virgo_calendar_event' onclick='var form=document.getElementById(\"portlet_form_".$this->getId()."\");form.portlet_action.value=\"View\";form.pbl_id_".$this->getId().".value=\"".$eventInDay->getId()."\";form.submit();'>" . $eventInDay->getVirgoTitle() . "</span>";
			}
		}
		echo "</td>";
...
?>
		<script type="text/javascript">
			var pozycjaBilansuChildrenDivOpen = '';
			
			function childrenButtonClicked(clickedDivId) {
				var div = document.getElementById(clickedDivId);
				if (clickedDivId == pozycjaBilansuChildrenDivOpen) {
					div.style.display = 'none';
					pozycjaBilansuChildrenDivOpen = '';
				} else {
					if (pozycjaBilansuChildrenDivOpen != '') {
						document.getElementById(pozycjaBilansuChildrenDivOpen).style.display = 'none';
					}
					div.style.display = 'block';
					pozycjaBilansuChildrenDivOpen = clickedDivId;
				}
			}
...
PROFILE('table_01');
PROFILE('table_02');
			$showPage = $resultPozycjaBilansu->getShowPage(); 
			$showRows = $resultPozycjaBilansu->getShowRows(); 
?>
						<input type="text" name="pbl_id_<?php echo $this->getId() ?>" id="pbl_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>			
				<tr><td colspan="99" class="nav-header"><?php echo T('Pozycja bilansus') ?></td></tr>
<?php
			}
?>			
<?php
PROFILE('table_02');
PROFILE('main select');
			$virgoOrderColumn = $resultPozycjaBilansu->getOrderColumn();
			$virgoOrderMode = $resultPozycjaBilansu->getOrderMode();
			$resultCount = -1;
			$filterApplied = false;
			$resultsPozycjaBilansu = $resultPozycjaBilansu->getTableData($resultCount, $filterApplied);
PROFILE('main select');
...
?>
<input name='pbl_Towar_id_<?php echo $this->getId() ?>' id='pbl_Towar_id_<?php echo $this->getId() ?>' type="hidden"/>
<?php		
	}
?>
				<th align="center" valign="middle" rowspan=" 2"><!-- bylo: nowrap -->
<?php
	$oc = $resultPozycjaBilansu->getOrderColumn(); 
	$om = $resultPozycjaBilansu->getOrderMode();
...
<?php
	if (P('show_table_filter', '0') == '1') {
		...
		if ($parentsCount < 51) {
			...
			$parentFilter = virgoPozycjaBilansu::getLocalSessionValue('VirgoFilterTowar', null);
?>
						<select 
...
<?php		
			...
			$parentFilter = virgoPozycjaBilansu::getLocalSessionValue('VirgoFilterTitleTowar', null);
?>
						<input
...
?>
<?php
if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoBilansOtwarcia'.DIRECTORY_SEPARATOR.'controller.php') && P('show_table_bilans_otwarcia', "1") != "0"  && !isset($parentsInContext['sealock\\virgoBilansOtwarcia'])  ) {
	if (P('show_table_bilans_otwarcia', "1") == "2") {
		$tmpLookupBilansOtwarcia = virgoBilansOtwarcia::getVirgoListStatic();
?>
<input name='pbl_BilansOtwarcia_id_<?php echo $this->getId() ?>' id='pbl_BilansOtwarcia_id_<?php echo $this->getId() ?>' type="hidden"/>
<?php		
	}
?>
				<th align="center" valign="middle" rowspan=" 2"><!-- bylo: nowrap -->
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder';
	_pSF(form, 'virgo_order_column', 'bilans_otwarcia');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php
	$customLabel = TE('BILANS_OTWARCIA_'); 
	echo !is_null($customLabel) ? $customLabel : T('BILANS_OTWARCIA') . '&nbsp;' . T('')
?>
							<?php echo ($oc == "bilans_otwarcia" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$parentsCount = virgoBilansOtwarcia::getVirgoListStatic('', true);
		if ($parentsCount < 51) {
			$parents = virgoBilansOtwarcia::getVirgoListStatic();
			$parentFilter = virgoPozycjaBilansu::getLocalSessionValue('VirgoFilterBilansOtwarcia', null);
?>
						<select 
							name="virgo_filter_bilans_otwarcia"
							class="virgo_filter"
							onChange="<?php echo JSFS(null, "Submit", true, array(), false, "SetVirgoTableFilter") ?>"
						>
							<option value=""></option>
...
<?php		
			...
			$parentFilter = virgoPozycjaBilansu::getLocalSessionValue('VirgoFilterTitleBilansOtwarcia', null);
?>
						<input
							name="virgo_filter_title_bilans_otwarcia"
							class="virgo_filter"
							onChange="<?php echo JSFS(null, "Submit", true, array(), false, "SetVirgoTableFilter") ?>"
							value="<?php echo $parentFilter ?>"
						/>
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'pbl_ilosc');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('ILOSC') ?>							<?php echo ($oc == "pbl_ilosc" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoPozycjaBilansu::getLocalSessionValue('VirgoFilterIlosc', null);
?>
						<input
...
<?php
			...
			if ($resultCount != 0) {
				if (((int)$showRows) * (((int)$showPage)-1) == $resultCount ) {
					...
					$resultPozycjaBilansu->setShowPage($showPage);
				}
				$index = 0;
PROFILE('table_04');
...
?>
<?php
$fileNameToInclude = PORTAL_PATH . "/portlets/sealock/virgoPozycjaBilansu/modules/renderTableRow_{$_SESSION['current_portlet_object_id']}.php";
...
<?php
			if (P('form_only') != "4" || (P('chessboard_width') != 0 && $index % P('chessboard_width') == 1)) {
				if (is_null($firstRowId)) {
					$firstRowId = $resultPozycjaBilansu['pbl_id'];
				}
				$displayClass = ' displayClass ';
				$tmpContextId = virgoPozycjaBilansu::getContextId();
				if (is_null($tmpContextId)) {
					$forceContextOnFirstRow = P('force_context_on_first_row', "1");
					if ($forceContextOnFirstRow == "1") {
						virgoPozycjaBilansu::setContextId($resultPozycjaBilansu['pbl_id'], false);
						$tmpContextId = $resultPozycjaBilansu['pbl_id'];
					}
				}
				if (isset($tmpContextId) && $resultPozycjaBilansu['pbl_id'] == $tmpContextId) {
					if (P('form_only') != "4") {
...
?>
			<tr 
				id="<?php echo $this->getId() ?>_<?php echo isset($resultPozycjaBilansu['pbl_id']) ? $resultPozycjaBilansu['pbl_id'] : "" ?>" 
...
?>
				<input type="checkbox" class="checkbox" style="float: right;" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultPozycjaBilansu['pbl_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
				<input type="checkbox" class="checkbox" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultPozycjaBilansu['pbl_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
			<a  
				...
				onclick="
					...
					form.portlet_action.value='View'; 
					form.pbl_id_<?php echo $this->getId() ?>.value='<?php echo isset($resultPozycjaBilansu['pbl_id']) ? $resultPozycjaBilansu['pbl_id'] : '' ?>'; 
					form.target = ''; 
<?php
					if (isset($ajax) && !$ajax) {
?>					
...
<?php 
	if (P('show_table_towar', "1") == "1") {
		if (isset($resultPozycjaBilansu['towar'])) {
			echo $resultPozycjaBilansu['towar'];
		}
	} else {
//		echo $resultPozycjaBilansu['pbl_twr_id'];
?>
		<select class="inline" onchange="
this.form.portlet_action.value='VirgoSetTowar';
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '<?php echo $resultPozycjaBilansu['pbl_id'] ?>');
	_pSF(form, 'pbl_Towar_id_<?php echo $this->getId() ?>', this.options[this.selectedIndex].value);
<?php
					if (isset($ajax) && !$ajax) {
?>					
						form.submit();
...
?>
			<option value="<?php echo $value ?>" <?php echo $value == $resultPozycjaBilansu['pbl_twr_id'] ? " selected " : "" ?>><?php echo $label ?></option>
<?php			
		}
?>
		</select>
...
?>
<?php
	if (class_exists('sealock\virgoBilansOtwarcia') && P('show_table_bilans_otwarcia', "1") != "0"  && !isset($parentsInContext["sealock\\virgoBilansOtwarcia"])  ) {
?>
<?php
			if (P('form_only') != "4") {
?>
				<td 
					align="left" 
					class="<?php echo ($index % 2 == 0 ? 'data_table_even' : 'data_table_odd') ?> <?php echo P('show_table_bilans_otwarcia', "1") == "1" ? " selectable " : " " ?> "
				>    
<?php
			} else {
?>
...
?>
<?php 
	if (P('show_table_bilans_otwarcia', "1") == "1") {
		if (isset($resultPozycjaBilansu['bilans_otwarcia'])) {
			echo $resultPozycjaBilansu['bilans_otwarcia'];
		}
	} else {
//		echo $resultPozycjaBilansu['pbl_bot_id'];
?>
		<select class="inline" onchange="
this.form.portlet_action.value='VirgoSetBilansOtwarcia';
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '<?php echo $resultPozycjaBilansu['pbl_id'] ?>');
	_pSF(form, 'pbl_BilansOtwarcia_id_<?php echo $this->getId() ?>', this.options[this.selectedIndex].value);
<?php
					if (isset($ajax) && !$ajax) {
?>					
						form.submit();
...
?>					
		">
<?php
		foreach ($tmpLookupBilansOtwarcia as $value => $label) {
?>
			<option value="<?php echo $value ?>" <?php echo $value == $resultPozycjaBilansu['pbl_bot_id'] ? " selected " : "" ?>><?php echo $label ?></option>
<?php			
		}
?>
		</select>
...
?>
		<span style="white-space: nowrap;" class="<?php echo 'displayClass' ?>">
			<?php echo 
				number_format($resultPozycjaBilansu['pbl_ilosc'], 2, ',', ' ')
			?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("POZYCJA_BILANSU"), "\\'".rawurlencode($resultPozycjaBilansu['pbl_virgo_title'])."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '<?php echo isset($resultPozycjaBilansu) ? (is_array($resultPozycjaBilansu) ? $resultPozycjaBilansu['pbl_id'] : $resultPozycjaBilansu->getId()) : '' ?>');
...
<?php
	...
	if (is_null($contextRowIdInTable)) {
		$forceContextOnFirstRow = P('force_context_on_first_row', "1");
		if ($forceContextOnFirstRow == "1") {
			virgoPozycjaBilansu::setContextId($firstRowId, false);
			if (P('form_only') != "4") {
?>
<script type="text/javascript">
...
<?php
				...
				unset($resultPozycjaBilansu);
				unset($resultsPozycjaBilansu);
				if (isset($contextIdOwn) && trim($contextIdOwn) != "") {
					if ($contextIdConfirmed == false) {
						$tmpPozycjaBilansu = new virgoPozycjaBilansu();
						$tmpCount = $tmpPozycjaBilansu->getAllRecordCount(' pbl_id = ' . $contextIdOwn);
						if ($tmpCount == 0) {
							virgoPozycjaBilansu::clearRemoteContextId($tabModeEditMenu);
						}
					}
				}
PROFILE('rows rendering');
...
?>
		$.getJSON('<?php echo $page->getUrl() ?>?portlet_action=SelectJson&pbl_id_<?php echo $this->getId() ?>=' + virgoId + '&invoked_portlet_object_id=<?php echo $this->getId() ?>&virgo_action_mode_json=T&_virgo_ajax=1', function(data) {
<?php
			if (P('form_only') != "4") {
?>
...
?>
		...
		form.pbl_id_<?php echo $this->getId() ?>.value=virgoId; 
		form.target = ''; 
<?php
...
<div class="button_wrapper button_wrapper_report inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								...
								for(var i = 0; i< children.length;i++) {
								  if (children[i].getAttribute('name') == 'pbl_id_<?php echo $this->getId() ?>') {
									found = 1;
								  }
								}
								if (found == 0) {
								  ...
								  input.setAttribute('name', 'pbl_id_<?php echo $this->getId() ?>');
								  input.setAttribute('value', '');
								  form.appendChild(input);
								} else {
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '');
								}
								form.action = '';
...
<?php
					$sessionSeparator = virgoPozycjaBilansu::getImportFieldSeparator();
...
<div class="button_wrapper button_wrapper_deleteselected inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								if (!nothingSelected(this.form, <?php echo $this->getId() ?>)) {
									if (!confirm(decodeURIComponent('<?php echo T('ARE_YOU_SURE_YOU_WANT_REMOVE', T('POZYCJA_BILANSUS'), "") ?>'))) return false;
								} else {
									var alertText = '<?php echo T('NOTHING_SELECTED') ?>';
									if (alertText.charAt(alertText.length-1)=='?') {
										if (!confirm(alertText)) return false;
...
/* MILESTONE 1.6 TableForm */
	} elseif ($pozycjaBilansuDisplayMode == "TABLE_FORM") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
	...
	<form method="post" style="display: inline;" action="" id="virgo_form_pozycja_bilansu" name="virgo_form_pozycja_bilansu" enctype="multipart/form-data">
						<input type="text" name="pbl_id_<?php echo $this->getId() ?>" id="pbl_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>
<?php
	if (P('show_table_bilans_otwarcia', "1") == "1" /* && ($masterComponentName != "bilans_otwarcia" || is_null($contextId)) */) {
?>
...
<?php			
				$resultsPozycjaBilansu = $resultPozycjaBilansu->getRecordsToEdit();
				$idsToCorrect = $resultPozycjaBilansu->getInvalidRecords();
				$index = 0;
PROFILE('rows rendering');
				foreach ($resultsPozycjaBilansu as $resultPozycjaBilansu) {
					$index = $index + 1;
...
<?php
	if ($resultPozycjaBilansu->pbl_id == 0 && R('virgo_validate_new', "N") == "N") {
?>
		style="display: none;"
<?php
	}
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsPozycjaBilansu) * 0;
?>
<?php
	if (!isset($resultPozycjaBilansu)) {
		$resultPozycjaBilansu = new sealock\virgoPozycjaBilansu();
...
?> 
						<input
							type="hidden"
							id="ilosc_<?php echo $resultPozycjaBilansu->pbl_id ?>" 
							name="ilosc_<?php echo $resultPozycjaBilansu->pbl_id ?>"
							value="<?php echo htmlentities($resultPozycjaBilansu->pbl_ilosc, ENT_QUOTES, "UTF-8") ?>"
						>
...
?>
				<td nowrap align="left" class="<?php echo ($index % 2 == 0 ? "data_table_even" : "data_table_odd") ?>">    
<?php
	$tabIndex = $index + sizeof($resultsPozycjaBilansu) * 2;
?>
<?php
//		$limit_bilans_otwarcia = $componentParams->get('limit_to_bilans_otwarcia');
		$limit_bilans_otwarcia = null;
...
?>
				<td>
<?php
	if (isset($idsToCorrect[$resultPozycjaBilansu->pbl_id])) {
		$errorMessage = $idsToCorrect[$resultPozycjaBilansu->pbl_id];
?>
					<div class="error">
...
/* MILESTONE 1.10 AddNewParents */
	} elseif ($pozycjaBilansuDisplayMode == "ADD_NEW_PARENT_TOWAR") {
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "kod") {
				$resultPozycjaBilansu->setKod($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultPozycjaBilansu->getKod(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="kod" name="twr_kod_<?php echo $resultTowar->getId() ?>" value="<?php echo htmlentities($resultPozycjaBilansu->getKod(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "nazwa") {
				$resultPozycjaBilansu->setNazwa($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultPozycjaBilansu->getNazwa(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="nazwa" name="twr_nazwa_<?php echo $resultTowar->getId() ?>" value="<?php echo htmlentities($resultPozycjaBilansu->getNazwa(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "stan_aktualny") {
				$resultPozycjaBilansu->setStanAktualny($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo number_format($resultPozycjaBilansu->getStanAktualny(), 2, ',', ' ') ?>
						&nbsp;</span>
						<input type="hidden" id="stanAktualny" name="twr_stanAktualny_<?php echo $resultTowar->getId() ?>" value="<?php echo htmlentities($resultPozycjaBilansu->getStanAktualny(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "stan_minimalny") {
				$resultPozycjaBilansu->setStanMinimalny($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo number_format($resultPozycjaBilansu->getStanMinimalny(), 2, ',', ' ') ?>
						&nbsp;</span>
						<input type="hidden" id="stanMinimalny" name="twr_stanMinimalny_<?php echo $resultTowar->getId() ?>" value="<?php echo htmlentities($resultPozycjaBilansu->getStanMinimalny(), ENT_QUOTES, "UTF-8") ?>"> 
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "produkt") {
				$resultPozycjaBilansu->setProdukt($virgoSelectedDay);
			}
		}
	}
...
>
<?php
	if (is_null($resultPozycjaBilansu->getProdukt()) || $resultPozycjaBilansu->getProdukt() == 2) {
		$booleanValue = T("NO_DATA");
	} elseif ($resultPozycjaBilansu->getProdukt() == 1) {
		$booleanValue = T("YES");
	} elseif ($resultPozycjaBilansu->getProdukt() === "0") {
		$booleanValue = T("NO");
	}
?>
...
//		$limit_grupa_towaru = $componentParams->get('limit_to_grupa_towaru');
		...
		$tmpId = sealock\virgoPozycjaBilansu::getParentInContext("sealock\\virgoGrupaTowaru");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#pbl_grupa_towaru_dropdown_<?php echo $resultPozycjaBilansu->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["gtw"])) {
		...
		$parentValue = $resultPozycjaBilansu->pbl_gtw_id;
	}
	
?>
				<input type="hidden" id="pbl_grupaTowaru_<?php echo $resultPozycjaBilansu->pbl_id ?>" name="pbl_grupaTowaru_<?php echo $resultPozycjaBilansu->pbl_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<?php
...
//		$limit_jednostka = $componentParams->get('limit_to_jednostka');
		...
		$tmpId = sealock\virgoPozycjaBilansu::getParentInContext("sealock\\virgoJednostka");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#pbl_jednostka_dropdown_<?php echo $resultPozycjaBilansu->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["jdn"])) {
		...
		$parentValue = $resultPozycjaBilansu->pbl_jdn_id;
	}
	
?>
				<input type="hidden" id="pbl_jednostka_<?php echo $resultPozycjaBilansu->pbl_id ?>" name="pbl_jednostka_<?php echo $resultPozycjaBilansu->pbl_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
...
?>
 <div class="button_wrapper button_wrapper_backfromparent inlineBlock"><input 							name="submit_button" 
	...
	$childId = R('pbl_id_' . $pobId);
?>
<input 
	type="hidden" 
	name="pbl_id_<?php echo $pobId ?>" 
	value="<?php echo $childId ?>"
/>
<input 
	type="hidden" 
	name="pbl_ilosc_<?php echo $childId ?>" 
	value="<?php echo R('pbl_ilosc_' . $childId) ?>"
/>
<input 
	type="hidden" 
	name="pbl_towar_<?php echo $childId ?>" 
	value="<?php echo R('pbl_towar_' . $childId) ?>"
/>
<input 
	type="hidden" 
	name="pbl_bilansOtwarcia_<?php echo $childId ?>" 
	value="<?php echo R('pbl_bilansOtwarcia_' . $childId) ?>"
/>
<?php		
/* MILESTONE 1.10 AddNewParents */
	} elseif ($pozycjaBilansuDisplayMode == "ADD_NEW_PARENT_BILANS_OTWARCIA") {
		$resultBilansOtwarcia = sealock\virgoBilansOtwarcia::createGuiAware();
		$resultBilansOtwarcia->loadFromRequest();
?>
<fieldset>
	<label>Dodaj nowy rekord Bilans otwarcia</label>
<?php
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "data") {
				$resultPozycjaBilansu->setData($virgoSelectedDay);
			}
		}
	}
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultPozycjaBilansu->getData(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="data" name="bot_data_<?php echo $resultBilansOtwarcia->getId() ?>" value="<?php echo htmlentities($resultPozycjaBilansu->getData(), ENT_QUOTES, "UTF-8") ?>"> 
...
//		$limit_status_bilans_otwarcia = $componentParams->get('limit_to_status_bilans_otwarcia');
		...
		$tmpId = sealock\virgoPozycjaBilansu::getParentInContext("sealock\\virgoStatusBilansOtwarcia");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#pbl_status_bilans_otwarcia_dropdown_<?php echo $resultPozycjaBilansu->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["sbo"])) {
		...
		$parentValue = $resultPozycjaBilansu->pbl_sbo_id;
	}
	
?>
				<input type="hidden" id="pbl_statusBilansOtwarcia_<?php echo $resultPozycjaBilansu->pbl_id ?>" name="pbl_statusBilansOtwarcia_<?php echo $resultPozycjaBilansu->pbl_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<?php
...
//		$limit_magazyn = $componentParams->get('limit_to_magazyn');
		$limit_magazyn = null;
		$tmpId = sealock\virgoPozycjaBilansu::getParentInContext("sealock\\virgoMagazyn");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#pbl_magazyn_dropdown_<?php echo $resultPozycjaBilansu->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
<?php
	if (isset($context["mgz"])) {
		...
		$parentValue = $resultPozycjaBilansu->pbl_mgz_id;
	}
	
?>
				<input type="hidden" id="pbl_magazyn_<?php echo $resultPozycjaBilansu->pbl_id ?>" name="pbl_magazyn_<?php echo $resultPozycjaBilansu->pbl_id ?>" value="<?php echo $parentValue ?>">
<?php
	}
?>
<input type="hidden" name="calling_view" value="<?php echo R('calling_view') ?>">
...
<?php
	$buttonRendered = false;
	if ($this->canExecute('StoreNewBilansOtwarcia')) { 		$buttonRendered = true;
		$anythingRendered = true;
?>
<div class="button_wrapper button_wrapper_storenewbilansotwarcia inlineBlock"><input 							name="submit_button" 
							class="button btn  " 
							type="submit" 
							onclick="
								var form = this.form;
								form.action = '';
								form.portlet_action.value='StoreNewBilansOtwarcia';
	_pSF(form, 'pbl_id_<?php echo $this->getId() ?>', '<?php echo isset($resultPozycjaBilansu) ? (is_array($resultPozycjaBilansu) ? $resultPozycjaBilansu['pbl_id'] : $resultPozycjaBilansu->getId()) : '' ?>');
...
?>
		<div class="<?php echo $pozycjaBilansuDisplayMode ?>">
<?php
?>
			<div class="buttons form-actions">
<?php
...
<div style="display: none; background-color:#FFFFFF; border:1px solid #000000; font-size:10px; margin:10px 0; padding:10px;"; id="extraFilesInfo_slk_pozycja_bilansu" style="font-size: 12px; " onclick="document.getElementById('extraFilesInfo_slk_pozycja_bilansu').style.display='none';">
...
<?php
	$infos = virgoPozycjaBilansu::getExtraFilesInfo();
