<?php
	...
	use sealock\virgoMagazyn;

//	setlocale(LC_ALL, '$messages.LOCALE');
	$componentParams = null; //&JComponentHelper::getParams('com_slk_magazyn');
...
?>
<link rel="stylesheet" href="<?php echo $live_site ?>/components/com_slk_magazyn/sealock.css" type="text/css" /> 
<?php
	}
?>
<?php
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoMagazyn'.DIRECTORY_SEPARATOR.'slk_mgz.css')) {
?>
<link rel="stylesheet" href="<?php echo $_SESSION['portal_url'] ?>/portlets/sealock/virgoMagazyn/slk_mgz.css" type="text/css" /> 
<?php
	}
?>
<style>
...
<div class="virgo_container_sealock virgo_container_entity_magazyn" style="border: none;">
...
//		$classNames["pozycja_dokumentu"] = "virgoPozycjaDokumentu";
		...
		$ancestors = array();
		$contextId = null;		
			$resultMagazyn = virgoMagazyn::createGuiAware();
			$contextId = $resultMagazyn->getContextId();
			if (isset($contextId)) {
				if (virgoMagazyn::getDisplayMode() != "CREATE" || R('portlet_action') == "Duplicate") {
					$resultMagazyn->load($contextId);
				}
			}
?>
<?php
...
<?php
	...
	if (isset($masterPobId)) {
		...
		if ($className == "virgoMagazyn") {
			$masterObject = new $className();
			$tmpId = $masterObject->getRemoteContextId($masterPobId);
			if (isset($tmpId)) {
				$resultMagazyn = new virgoMagazyn($tmpId);
				virgoMagazyn::setDisplayMode("FORM");
			} else {
				$resultMagazyn = new virgoMagazyn();
				virgoMagazyn::setDisplayMode("CREATE");
			}
		}
	} else {
		if (P('form_only', "0") == "5") {
			if (is_null($resultMagazyn->getId())) { 
				if (P('only_private_records', "0") == "1") {
					$allPrivateRecords = $resultMagazyn->selectAll();
					if (sizeof($allPrivateRecords) > 0) {
						$resultMagazyn = new virgoMagazyn($allPrivateRecords[0]['mgz_id']);
						$resultMagazyn->putInContext(false);
					} else {
						$resultMagazyn = new virgoMagazyn();
					}
				} else {
					...
					if (isset($customSQL) && trim($customSQL) != '') {
						...
						$records = $resultMagazyn->selectAll($customSQL);
						if (sizeof($records) > 0) {
							$resultMagazyn = new virgoMagazyn($records[0]['mgz_id']);
							$resultMagazyn->putInContext(false);
						} else {
							$resultMagazyn = new virgoMagazyn();
						}
			...
			$resultMagazyn = new virgoMagazyn(virgoUser::getUserId());
			$resultMagazyn->putInContext(false);
		}
	}
?>
...
?>
<?php
	$magazynDisplayMode = virgoMagazyn::getDisplayMode();
//	if ($magazynDisplayMode == "" || $magazynDisplayMode == "TABLE") {
//		$resultMagazyn = $resultMagazyn->portletActionForm();
//	}
?>
		<div class="form">
<?php
		$parentContextInfos = $resultMagazyn->getParentsInContext();
...
PROFILE('token');
	if (isset($resultMagazyn)) {
		$tmpId = is_array($resultMagazyn) ? $resultMagazyn['mgz_id'] : $resultMagazyn->getId();
		if (isset($tmpId)) {
?>
...
<?php
	if ($magazynDisplayMode != "TABLE") {
		...
		if (
			...
			$pob = $resultMagazyn->getMyPortletObject();
			...
			if (isset($invokedPortletId) && $invokedPortletId == $_SESSION['current_portlet_object_id'] && isset($reloadFromRequest) && $reloadFromRequest == "1") { 
				...
				if (P('form_only', "0") == "1" && isset($contextId)) {
					if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoMagazyn'.DIRECTORY_SEPARATOR.'create_store_message.php')) {
						require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoMagazyn'.DIRECTORY_SEPARATOR.'create_store_message.php');
						$magazynDisplayMode = "-empty-";
					}
				}
			}
		}
	}
if (!$resultMagazyn->hideContentDueToNoParentSelected()) {
...
<script language="JavaScript">
  ...
  function confirmExit() {
	  ...
	  for (var i=0; i < checks.length; i++) {
		  ...
		  if (check.form.name == "virgo_form_magazyn") {
			  if (check.value == 'T') {
...
?>
					<label 
						...
						for="mgz_nazwa_<?php echo $resultMagazyn->getId() ?>"
					>* <?php echo T('NAZWA') ?>
</label>
<?php
	if (!$formsInTable) {
...
?>
						<span class="inputbox readonly uneditable-input">
							<?php echo htmlentities($resultMagazyn->getNazwa(), ENT_QUOTES, "UTF-8") ?> 
						&nbsp;</span>
						<input type="hidden" id="nazwa" name="mgz_nazwa_<?php echo $resultMagazyn->getId() ?>" value="<?php echo htmlentities($resultMagazyn->getNazwa(), ENT_QUOTES, "UTF-8") ?>"> 
...
?>
					<label 
						...
						for="mgz_opis_<?php echo $resultMagazyn->getId() ?>"
...
<div 
	...
	id="opis" 
><?php echo htmlentities($resultMagazyn->getOpis(), ENT_QUOTES, "UTF-8") ?></div>
...
?>
<?php
	if (false) { //$componentParamsBilansOtwarcia->get('show_table_status_bilans_otwarcia') == "1" && ($masterComponentName != "status_bilans_otwarcia" || is_null($contextId))) {
?>
...
<?php			
				...
				$resultsBilansOtwarcia = $tmpBilansOtwarcia->selectAll(' bot_mgz_id = ' . $resultMagazyn->mgz_id);
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsMagazyn) * 0;
?>
<?php
...
?> 
						<input
							type="hidden"
							id="data_<?php echo $resultMagazyn->mgz_id ?>" 
							name="data_<?php echo $resultMagazyn->mgz_id ?>"
							value="<?php echo htmlentities($resultMagazyn->mgz_data, ENT_QUOTES, "UTF-8") ?>"
						>
...
//		$limit_status_bilans_otwarcia = $componentParams->get('limit_to_status_bilans_otwarcia');
		...
		$tmpId = sealock\virgoMagazyn::getParentInContext("sealock\\virgoStatusBilansOtwarcia");
		$readOnly = "";
...
?>						
<script type="text/javascript">
$('#mgz_status_bilans_otwarcia_dropdown_<?php echo $resultMagazyn->getId() ?>').qtip({position: {
		my: 'left center',
		at: 'right center'
	}});
</script>						
...
if (P('hide_audit', '0') == '0') {
	$showFieldset = 0;
	if ($resultMagazyn->getDateCreated()) {
		if ($resultMagazyn->getUsrCreatedId() == 0) {
			$username = "anonymous";
		} else {
			$createdByUser = new virgoUser((int)$resultMagazyn->getUsrCreatedId());
			$username = $createdByUser->getUsername();
		}
		$createdOn = $resultMagazyn->getDateCreated();
		$showFieldset = 1;
?>
		...
		<legend><?php echo T('AUDIT_DATA') ?>:</legend>
			<ul>
				<li>
					...
					<input type="hidden" id="usr_created_id" name="usr_created_id" value="<?php echo $resultMagazyn->getUsrCreatedId() ?>"	>
				</li>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('CREATION_DATE') ?></span>
					<span class="readonly"><i><?php echo $createdOn ?></i></span>
					<input type="hidden" id="date_created" name="date_created" value="<?php echo $resultMagazyn->getDateCreated() ?>"	>
				</li>
<?php
	}
?>
<?php
	if ($resultMagazyn->getDateModified()) {
		if ($resultMagazyn->getUsrModifiedId() == 0) {
			$username = "anonymous";
		} else {
			$modifiedByUser = new virgoUser((int)$resultMagazyn->getUsrModifiedId());
			$username = $modifiedByUser->getUsername();
		}
		$modifiedOn = $resultMagazyn->getDateModified();
?>
				<li>
					<span align="right" nowrap class="fieldlabel"><?php echo T('MODIFIED_BY') ?></span>
					<span class="readonly"><b><?php echo $username ?></b></span>
					<input type="hidden" id="usr_modified_id" name="usr_modified_id" value="<?php echo $resultMagazyn->getUsrModifiedId() ?>"	>
				</li>
				<li>
					...
					<input type="hidden" id="date_modified" name="date_modified" value="<?php echo $resultMagazyn->getDateModified() ?>"	>
				</li>
<?php
	}
	if ($showFieldset == 1) {
...
?>
				...
				<div class="buttons form-actions">
						<input type="text" name="mgz_id_<?php echo $this->getId() ?>" id="mgz_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
 ...
 <div class="button_wrapper button_wrapper_store inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'mgz_id_<?php echo $this->getId() ?>', '<?php echo isset($resultMagazyn) ? (is_array($resultMagazyn) ? $resultMagazyn['mgz_id'] : $resultMagazyn->getId()) : '' ?>');
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("MAGAZYN"), "\\'".rawurlencode($resultMagazyn->getVirgoTitle())."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
...
// document.forms['portlet_form_<?php echo $this->getId() ?>'].elements['mgz_nazwa_<?php echo $resultMagazyn->getId() ?>'].focus();
</script>
	</div>
<?php
/* MILESTONE 1.2 Create */
	} elseif ($magazynDisplayMode == "CREATE") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
...
?>
<?php
		if (isset($resultMagazyn->mgz_id)) {
			$resultMagazyn->mgz_id = null;
		}
		$parentAjaxRendered = "0";
?>
<?php
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "nazwa") {
				$resultMagazyn->setNazwa($virgoSelectedDay);
			}
		}
	}
	...
	if (S($virgoSelectedDay)) {
		if (P('form_only') == "3") {
			if (P('event_column') == "opis") {
				$resultMagazyn->setOpis($virgoSelectedDay);
			}
		}
	}
...
/* MILESTONE 1.3 Search */
	} elseif ($magazynDisplayMode == "SEARCH") {
?>
...
?>
<?php
	$record = new sealock\virgoMagazyn();
	$recordId = is_array($resultMagazyn) ? $resultMagazyn['mgz_id'] : $resultMagazyn->mgz_id;
...
<?php
/* MILESTONE 1.4 View */
	} elseif ($magazynDisplayMode == "VIEW") {
?>
	<div class="form_view">
<?php
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("MAGAZYN"), "\\'".$resultMagazyn->getVirgoTitle()."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'mgz_id_<?php echo $this->getId() ?>', '<?php echo isset($resultMagazyn) ? (is_array($resultMagazyn) ? $resultMagazyn['mgz_id'] : $resultMagazyn->getId()) : '' ?>');
...
<?php
/* MILESTONE 1.5 Table */
	} elseif ($magazynDisplayMode == "TABLE") {
PROFILE('TABLE');
		if (P('form_only') == "3") {
?>
<?php
	...
	$resultCount = -1;
	$filterApplied = false;
	$resultMagazyn->setShowPage(1); 
	$resultMagazyn->setShowRows('all'); 	
	$resultsMagazyn = $resultMagazyn->getTableData($resultCount, $filterApplied);
	$events = array();
	foreach ($resultsMagazyn as $resultMagazyn) {
		if (isset($resultMagazyn[$eventColumn]) && isset($events[substr($resultMagazyn[$eventColumn], 0, 10)])) {
			$eventsInDay = $events[substr($resultMagazyn[$eventColumn], 0, 10)];
		} else {
			$eventsInDay = array();
		}
		$eventObject = new virgoMagazyn($resultMagazyn['mgz_id']);
		$eventsInDay[] = $eventObject;
		$events[substr($resultMagazyn[$eventColumn], 0, 10)] = $eventsInDay;
	}
?>
<style type="text/css">
...
?>			
			...
			<tr class='virgo_calendar_week'>
				...
				<input type='hidden' name='mgz_id_<?php echo $this->getId() ?>' value=''/>
<?php
	$tmpWeekDay = 1;
	$safety = 0;
	if ($tmpDay["wday"] > 0) {
...
<?php
		...
		if (isset($events[$tmpStringDay])) {
			...
			foreach ($eventsInDay as $resultMagazyn) {
?>
<?php
PROFILE('token');
	if (isset($resultMagazyn)) {
...
<div class="button_wrapper button_wrapper_view inlineBlock"><input 							name="submit_button" 
	...
	_pSF(form, 'mgz_id_<?php echo $this->getId() ?>', '<?php echo isset($resultMagazyn) ? (is_array($resultMagazyn) ? $resultMagazyn['mgz_id'] : $resultMagazyn->getId()) : '' ?>');
 								...
 								form.target = '';
							" 
							value="<?php echo T($resultMagazyn->getVirgoTitle()) ?>"
						/><div class="button_right"></div></div><?php
	}
?>
<?php				
//				echo "<span class='virgo_calendar_event' onclick='var form=document.getElementById(\"portlet_form_".$this->getId()."\");form.portlet_action.value=\"View\";form.mgz_id_".$this->getId().".value=\"".$eventInDay->getId()."\";form.submit();'>" . $eventInDay->getVirgoTitle() . "</span>";
			}
		}
		echo "</td>";
...
?>
		<script type="text/javascript">
			var magazynChildrenDivOpen = '';
			
			function childrenButtonClicked(clickedDivId) {
				var div = document.getElementById(clickedDivId);
				if (clickedDivId == magazynChildrenDivOpen) {
					div.style.display = 'none';
					magazynChildrenDivOpen = '';
				} else {
					if (magazynChildrenDivOpen != '') {
						document.getElementById(magazynChildrenDivOpen).style.display = 'none';
					}
					div.style.display = 'block';
					magazynChildrenDivOpen = clickedDivId;
				}
			}
...
PROFILE('table_02');
	...
	if (file_exists(PORTAL_PATH.DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoMagazyn'.DIRECTORY_SEPARATOR.'controller.php')) require_once(PORTAL_PATH .DIRECTORY_SEPARATOR.'portlets'.DIRECTORY_SEPARATOR.'sealock'.DIRECTORY_SEPARATOR.'virgoMagazyn'.DIRECTORY_SEPARATOR.'controller.php');
			$showPage = $resultMagazyn->getShowPage(); 
			$showRows = $resultMagazyn->getShowRows(); 
?>
						<input type="text" name="mgz_id_<?php echo $this->getId() ?>" id="mgz_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
?>			
				<tr><td colspan="99" class="nav-header"><?php echo T('Magazyny') ?></td></tr>
<?php
			}
?>			
<?php
PROFILE('table_02');
PROFILE('main select');
			$virgoOrderColumn = $resultMagazyn->getOrderColumn();
			$virgoOrderMode = $resultMagazyn->getOrderMode();
			$resultCount = -1;
			$filterApplied = false;
			$resultsMagazyn = $resultMagazyn->getTableData($resultCount, $filterApplied);
PROFILE('main select');
...
<?php
	$oc = $resultMagazyn->getOrderColumn(); 
	$om = $resultMagazyn->getOrderMode();
...
?>
	...
	_pSF(form, 'virgo_order_column', 'mgz_nazwa');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('NAZWA') ?>							<?php echo ($oc == "mgz_nazwa" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoMagazyn::getLocalSessionValue('VirgoFilterNazwa', null);
?>
						<input
...
?>
						<span onclick="var form = document.getElementById('portlet_form_<?php echo $this->getId() ?>');
							form.portlet_action.value='ChangeOrder'; 
	_pSF(form, 'virgo_order_column', 'mgz_opis');
	_pSF(form, 'virgo_order_mode', '<?php echo ($om == 'asc' ? 'desc' : 'asc') ?>');
							<?php echo JSFS() ?>
							 " style="white-space: normal; cursor: pointer;" class="data_table_header">
<?php echo T('OPIS') ?>							<?php echo ($oc == "mgz_opis" ? ($om == "" ? $pagingDescHTML : ($om == "asc") ? $pagingDescHTML : $pagingAscHTML) : "") ?> 
						</span>
<?php
	if (P('show_table_filter', '0') == '1') {
		$tableFilter = virgoMagazyn::getLocalSessionValue('VirgoFilterOpis', null);
?>
<?php		
	}
?>
...
<?php
			...
			if ($resultCount != 0) {
				if (((int)$showRows) * (((int)$showPage)-1) == $resultCount ) {
					...
					$resultMagazyn->setShowPage($showPage);
				}
				$index = 0;
PROFILE('table_04');
...
?>
<?php
$fileNameToInclude = PORTAL_PATH . "/portlets/sealock/virgoMagazyn/modules/renderTableRow_{$_SESSION['current_portlet_object_id']}.php";
...
<?php
			if (P('form_only') != "4" || (P('chessboard_width') != 0 && $index % P('chessboard_width') == 1)) {
				if (is_null($firstRowId)) {
					$firstRowId = $resultMagazyn['mgz_id'];
				}
				$displayClass = ' displayClass ';
				$tmpContextId = virgoMagazyn::getContextId();
				if (is_null($tmpContextId)) {
					$forceContextOnFirstRow = P('force_context_on_first_row', "1");
					if ($forceContextOnFirstRow == "1") {
						virgoMagazyn::setContextId($resultMagazyn['mgz_id'], false);
						$tmpContextId = $resultMagazyn['mgz_id'];
					}
				}
				if (isset($tmpContextId) && $resultMagazyn['mgz_id'] == $tmpContextId) {
					if (P('form_only') != "4") {
...
?>
			<tr 
				id="<?php echo $this->getId() ?>_<?php echo isset($resultMagazyn['mgz_id']) ? $resultMagazyn['mgz_id'] : "" ?>" 
...
?>
				<input type="checkbox" class="checkbox" style="float: right;" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultMagazyn['mgz_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
				<input type="checkbox" class="checkbox" name="DELETE_<?php echo $this->getId() ?>_<?php echo $resultMagazyn['mgz_id'] ?>">
<?php
			if (P('form_only') != "4") {
?>
			</td>
...
?>
			<a  
				...
				onclick="
					...
					form.portlet_action.value='View'; 
					form.mgz_id_<?php echo $this->getId() ?>.value='<?php echo isset($resultMagazyn['mgz_id']) ? $resultMagazyn['mgz_id'] : '' ?>'; 
					form.target = ''; 
<?php
					if (isset($ajax) && !$ajax) {
?>					
...
?>
		<span 
			class="<?php echo 'displayClass' ?>">
				<?php echo htmlentities($resultMagazyn['mgz_nazwa'], ENT_QUOTES, "UTF-8") ?>
		</span>
<?php
	if (P('show_details_method') == "1") { 
?>
...
?>
		<?php
			$len = strlen($resultMagazyn['mgz_opis']);
			$shortTextSize = P('short_text_size_opis', 30);
		?>
		<span 
			class="<?php echo 'displayClass' ?>" <?php print ($len > $shortTextSize ? 'title="' . htmlentities($resultMagazyn['mgz_opis'], ENT_QUOTES, "UTF-8") . '"' : '') ?>>
			<?php
				if ($shortTextSize > 0 && $len > $shortTextSize) {
					echo htmlentities(substr($resultMagazyn['mgz_opis'], 0, $shortTextSize), ENT_QUOTES, "UTF-8") . "...";
				} else {
					if ($shortTextSize == 0) {
						echo "<pre style='white-space: pre-line;'>";
					}
					echo htmlentities($resultMagazyn['mgz_opis'], ENT_QUOTES, "UTF-8");
					if ($shortTextSize == 0) {
						echo "</pre>";
					}
				}				 
...
<div class="button_wrapper button_wrapper_delete inlineBlock"><input 							name="submit_button" 
							...
							type="submit" 
							onclick="
									if (!confirm(decodeURIComponent('<?php echo T("ARE_YOU_SURE_YOU_WANT_REMOVE", T("MAGAZYN"), "\\'".rawurlencode($resultMagazyn['mgz_virgo_title'])."\\'") ?>'))) return false;
								var form = this.form;
								form.action = '';
								form.portlet_action.value='Delete';
	_pSF(form, 'mgz_id_<?php echo $this->getId() ?>', '<?php echo isset($resultMagazyn) ? (is_array($resultMagazyn) ? $resultMagazyn['mgz_id'] : $resultMagazyn->getId()) : '' ?>');
...
<?php
	...
	if (is_null($contextRowIdInTable)) {
		$forceContextOnFirstRow = P('force_context_on_first_row', "1");
		if ($forceContextOnFirstRow == "1") {
			virgoMagazyn::setContextId($firstRowId, false);
			if (P('form_only') != "4") {
?>
<script type="text/javascript">
...
<?php
				...
				unset($resultMagazyn);
				unset($resultsMagazyn);
				if (isset($contextIdOwn) && trim($contextIdOwn) != "") {
					if ($contextIdConfirmed == false) {
						$tmpMagazyn = new virgoMagazyn();
						$tmpCount = $tmpMagazyn->getAllRecordCount(' mgz_id = ' . $contextIdOwn);
						if ($tmpCount == 0) {
							virgoMagazyn::clearRemoteContextId($tabModeEditMenu);
						}
					}
				}
PROFILE('rows rendering');
...
?>
		$.getJSON('<?php echo $page->getUrl() ?>?portlet_action=SelectJson&mgz_id_<?php echo $this->getId() ?>=' + virgoId + '&invoked_portlet_object_id=<?php echo $this->getId() ?>&virgo_action_mode_json=T&_virgo_ajax=1', function(data) {
<?php
			if (P('form_only') != "4") {
?>
...
?>
		...
		form.mgz_id_<?php echo $this->getId() ?>.value=virgoId; 
		form.target = ''; 
<?php
...
<div class="button_wrapper button_wrapper_report inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								...
								for(var i = 0; i< children.length;i++) {
								  if (children[i].getAttribute('name') == 'mgz_id_<?php echo $this->getId() ?>') {
									found = 1;
								  }
								}
								if (found == 0) {
								  ...
								  input.setAttribute('name', 'mgz_id_<?php echo $this->getId() ?>');
								  input.setAttribute('value', '');
								  form.appendChild(input);
								} else {
	_pSF(form, 'mgz_id_<?php echo $this->getId() ?>', '');
								}
								form.action = '';
...
<?php
					$sessionSeparator = virgoMagazyn::getImportFieldSeparator();
...
<div class="button_wrapper button_wrapper_deleteselected inlineBlock"><input 							name="submit_button" 
							...
							onclick="
								if (!nothingSelected(this.form, <?php echo $this->getId() ?>)) {
									if (!confirm(decodeURIComponent('<?php echo T('ARE_YOU_SURE_YOU_WANT_REMOVE', T('MAGAZYNY'), "") ?>'))) return false;
								} else {
									var alertText = '<?php echo T('NOTHING_SELECTED') ?>';
									if (alertText.charAt(alertText.length-1)=='?') {
										if (!confirm(alertText)) return false;
...
/* MILESTONE 1.6 TableForm */
	} elseif ($magazynDisplayMode == "TABLE_FORM") {
?>
<script language="JavaScript">
  window.onbeforeunload = confirmExit;
  function confirmExit() {
	...
	<form method="post" style="display: inline;" action="" id="virgo_form_magazyn" name="virgo_form_magazyn" enctype="multipart/form-data">
						<input type="text" name="mgz_id_<?php echo $this->getId() ?>" id="mgz_id_<?php echo $this->getId() ?>" value="" style="display: none;"/>
						<input type="hidden" name="virgo_order_column" id="virgo_order_column"/>
						<input type="hidden" name="virgo_order_mode" id="virgo_order_mode"/>						
...
<?php			
				$resultsMagazyn = $resultMagazyn->getRecordsToEdit();
				$idsToCorrect = $resultMagazyn->getInvalidRecords();
				$index = 0;
PROFILE('rows rendering');
				foreach ($resultsMagazyn as $resultMagazyn) {
					$index = $index + 1;
?>
			<tr id="virgo_tr_id_<?php echo $resultMagazyn->mgz_id ?>" class="<?php echo ($index % 2 == 0 ? "data_table_even" : "data_table_odd") ?>"
<?php
	if ($resultMagazyn->mgz_id == 0 && R('virgo_validate_new', "N") == "N") {
?>
		style="display: none;"
<?php
	}
...
?> 
						<input
							type="hidden"
							id="nazwa_<?php echo $resultMagazyn->mgz_id ?>" 
							name="nazwa_<?php echo $resultMagazyn->mgz_id ?>"
							value="<?php echo htmlentities($resultMagazyn->mgz_nazwa, ENT_QUOTES, "UTF-8") ?>"
						>
...
<td>
<?php
	$tabIndex = $index + sizeof($resultsMagazyn) * 1;
?>
<?php
	if (!isset($resultMagazyn)) {
		$resultMagazyn = new sealock\virgoMagazyn();
...
?> 
						<input
							type="hidden"
							id="opis_<?php echo $resultMagazyn->mgz_id ?>" 
							name="opis_<?php echo $resultMagazyn->mgz_id ?>"
							value="<?php echo htmlentities($resultMagazyn->mgz_opis, ENT_QUOTES, "UTF-8") ?>"
						>
...
?>
				<td>
<?php
	if (isset($idsToCorrect[$resultMagazyn->mgz_id])) {
		$errorMessage = $idsToCorrect[$resultMagazyn->mgz_id];
?>
					<div class="error">
						<?php echo $errorMessage ?>
					</div>
...
?>
		<div class="<?php echo $magazynDisplayMode ?>">
<?php
?>
			<div class="buttons form-actions">
<?php
...
<div style="display: none; background-color:#FFFFFF; border:1px solid #000000; font-size:10px; margin:10px 0; padding:10px;"; id="extraFilesInfo_slk_magazyn" style="font-size: 12px; " onclick="document.getElementById('extraFilesInfo_slk_magazyn').style.display='none';">
...
<?php
	$infos = virgoMagazyn::getExtraFilesInfo();
